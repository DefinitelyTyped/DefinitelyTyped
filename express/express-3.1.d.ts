...
interface ImportConnect {
     ...
     * @param options
     */
    bodyParser(options?: any): Handler;
     ...
     * @param key
     */
    methodOverride(key: string): Handler;
     ...
     * @param secret
     */
    cookieParser(secret: string): Handler;
     ...
     * @param realm
     */
    basicAuth(callback: Function, realm: string);

    basicAuth(callback: string, realm: string);
     ...
     * @param options
     */
    cookieSession(options?: any): Handler;
     ...
     * @param options
     */
    directory(root: string, options?: any): Handler;
     ...
     * @param options
     */
    favicon(path?: string, options?: any);
     ...
     *       .use(handleImageUpload)
     */
    limit(bytes: number): Handler;

    limit(bytes: string): Handler;
     ...
     *       connect.logger.format('name', 'string or function')
     */
    logger(options: string): Handler;

    logger(options: Function): Handler;

    logger(options: any): Handler;
     ...
     *   - `maxLength`  max cache object length 256kb
     */
    staticCache(options: any): Handler;
     ...
     * @param server
     */
    vhost(hostname: string, server: any): Handler;
}

interface ExpressApplication extends ImportConnect {
    /**
     * Initialize the server.
     *
     *   - setup default configuration
