...
expect<Type>({ a: 1, b: 2, c: 3 }).to.include({ a: 1 });
expect<Type>({ a: 1, b: 2, c: 3 }).to.include({ a: 1, c: 3 });
expect<Type>({ a: 1, b: 2, c: 3 }).to.part.include({ a: 1, d: 4 });
expect({ a: 1, b: 2, c: 3 }).to.only.include({ a: 1, b: 2, c: 3 });
expect<Type2>({ a: [1], b: [2], c: [3] }).to.include({ a: [1], c: [3] });
...
settings.comparePrototypes = true;
expect(bar).to.equal({});
