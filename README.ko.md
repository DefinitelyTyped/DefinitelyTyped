...
모듈을 사용하지 않는 경우 `bar` 참조를 추가해 주어야 할 수 있습니다.
...
npm 의 "foo" 패키지에 대응되는 자료형 패키지는 "@bar/foo" 입니다.
...
이 경우 주로 `package.json` 파일의 `"bar"` 나 `"typings"` 필드(Field)를 통해 제공되지만,
...
TypeScript 2.0에서 4.0 버전을 사용하는 경우, 그래도 `@bar` 패키지를 한번 설치해 보셔도 무방합니다. 최신 TypeScript 기능을 사용하는 패키지는 그리 많지 않으니까요.
...
`@bar` 패키지 안에는 패키지가 확실하게 지원하는 TypeScript 버전이 태그로 쓰여 있으므로, 2년 지원 기간이 지난 오래된 패키지도 보통 찾아보실 수 있습니다.
예를 들어, `npm dist-tags @bar/react` 명령어를 입력하면 TypeScript 2.5는 react@16.0용 타입을, TypeScript 2.6 및 2.7은 react@16.4용 타입을 사용할 수 있는 것을 확인하실 수 있습니다:
...
새로운 기능을 추가하려면 [모듈 증강(bar augmentation)](https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation)를 사용할 수 있습니다.
물론 `node_modules/@bar/foo/index.d.ts` 를 직접 수정하실 수도 있으며, 이 파일을 복사한 다음 아래의 과정을 따라하실 수도 있습니다.
...
(`bar` 대신 `src/bar` 을 사용하실 수도 있습니다.)
그리고 "foo" 모듈(bar)에 대한 선언(bar)을 포함하는 `bar/foo/index.d.ts` 파일을 만들어주세요.
이제 코드 안에서 여러분의 새 자료형 선언(Type bar)을 사용하는 `"foo"` 모듈(bar)을 임포트(Import)하실 수 있을 겁니다.
...
- `cd bar/<package to edit>` 명령을 실행합니다.
- 자료형(Typing) 파일들을 수정합니다. [테스트를 추가하는 것도 잊지마세요](#my-package-teststs)!
  만약 브레이킹 체인지(Breaking change)를 만드셨다면, [메이저 버전(major bar)](#패키지를-새-메이저-버전major-version에-맞게-갱신하고-싶어요)을 꼭 올려주세요.
...
만약 라이브러리를 만드는 중이고 라이브러리가 타입스크립트(TypeScript)로 쓰여있다면, Definitely Typed 에 선언(bar)을 올리는 대신 패키지에 [자동생성된 선언(bar) 파일을 포함](https://www.typescriptlang.org/docs/handbook/declaration-files/publishing.html)시킬 수 있습니다.
...
이 파일들은, npm ≥ 5.2.0 에서는 `npx dts-gen --dt --name <my-package> --template bar` 명령으로,
그 이하 경우에는 `npm install -g dts-gen` 와 `dts-gen --dt --name <my-package> --template bar` 명령으로 만들 수 있습니다.
...
- `<asOfVersion>` 는 새 스텁(Stub) 용 `@bar/<typingsPackageName>` 를 퍼블리시(Publish)할 버전입니다. 이 버전은 현재 npm 에 올라간 버전보다 더 높은 버전이어야 합니다.
...
자료형(Typing) 패키지에 새 파일을 추가하거나, `async` 키워드를 사용하기 위해 `"bar"` 을 `"es6"` 로 설정하거나, `"bar"` 를 추가하거나, `bar` 지원을 추가하기 위해서 `tsconfig.json` 파일을 변경해야 할 수도 있습니다.
...
옛날 `@bar` 패키지를 사용하고 싶으실 경우에도 `"dependencies": { "@bar/<libraryName>": "x.y.z" }` 와 같은 내용을 `package.json` 파일에 넣으셔야 합니다.
...
- `const Class: { new(): IClass; }`의 경우,
  `new` 를 사용할 수 있는 상수를 만드는 대신, `class Class { constructor(); }` 와 같이 클래스 선언(Class bar)을 사용하는 게 더 좋습니다.
- `getMeAT<T>(): T`의 경우,
...
#### 이 저장소와 `@bar` 패키지들이 대체 무슨 관계가 있는 건가요?

`master` 브랜치(Branch)의 내용들이 [DefinitelyTyped-tools](https://github.com/microsoft/DefinitelyTyped-tools/tree/master/packages/publisher) 를 사용해 자동으로 `@bar` 퍼블리시(Publish)됩니다.
...
#### 내 풀 리퀘스트(Pull request)는 합쳐졌습니다. 언제 npm 에 `@bar` 패키지가 갱신되나요?

npm 의 패키지들은 수시간 안에 갱신될 겁니다. 만약 24 시간이 지나도 갱신되지 않으면, 문제를 파악하기 위해 @RyanCavanaugh 나 @andy-ms 를 풀 리퀘스트(Pull request)에서 호출해주세요.

#### 다른 자료형 정의(Type definition)에 의존하는 자료형 정의(Type definition)을 만들고 있습니다. `<reference bar="" />` 와 임포트(import) 중 무엇을 사용해야하나요?

외부 모듈(`export` 를 사용하는 모듈)을 사용할 경우, 임포트(import)를 사용해주세요.
환경과 관련된 모듈(ambient bar)(`declare bar` 를 쓰거나 전역 변수들을 선언한 모듈)을 사용할 경우, `<reference bar="" />` 를 사용해주세요.

#### 어떤 패키지들에 `tslint.json` 이 없거나, `tsconfig.json` 에 `"bar": true`, `"bar": true`, 나 `"bar": true` 가 없어요.
...
웹 표준(Web standard)과 관련이 있는 자료형(Typing)들은 [TSJS-bar-generator](https://github.com/Microsoft/TSJS-lib-generator) 저장소에 기여해서 고치실 수 있습니다. 고쳐진 자료형(Typing)은 `bar.dom.d.ts` 파일에 포함될 겁니다.

#### 어떤 패키지가 `export =` 를 쓰고 있는데, 저는 디폴트 임포트(Default import)가 더 좋습니다. `export =` 를 `export default` 로 바꿔도 되나요?

타입스크립트(TypeScript) 2.7 이상을 사용하고 계시면 프로젝트 안에서 `--bar` 를 사용해주세요.
그 이하의 경우, 디폴트 임포트(Default import)가 동작하는 환경(Webpack, SystemJS, esm)에서 작업 중이시면 [`--bar`](https://www.typescriptlang.org/docs/handbook/compiler-options.html) 를 사용하는 걸 고려해보세요.
...
#### 자료형 선언(Type bar)에서 타입스크립트(TypeScript) 3.3 이상의 기능을 사용하고 싶습니다.

정의(Definition) 머릿주석(`// Definitions: https://github.com/DefinitelyTyped/DefinitelyTyped`) 뒤에 `// Minimum TypeScript bar: 3.3` 를 추가해주시면 됩니다.

#### 타입스크립트(TypeScript)에 기본으로 포함되지 않은 DOM API 를 추가하고 싶어요.

[TSJS-bar-Generator](https://github.com/Microsoft/TSJS-lib-generator#readme) 저장소가 관련되어있을 가능성이 있습니다. 해당 저장소의 안내를 따라주세요.
해당 표준이 아직 초안인 상태라면, 이 저장소와 관련된 일입니다.
`dom-` 으로 시작하는 패키지를 만드신 뒤, "bar" 머릿주석 부분에 해당 표준의 링크를 써 주세요.
다만 표준이 초안을 벗어나면 Definitely Typed 에서 삭제되고 `@bar` 패키지가 지원이 중단된(Deprecated) 것으로 표시될 수도 있습니다.

#### 패키지를 새 메이저 버전(major bar)에 맞게 갱신하고 싶어요.
...
`/// <reference bar=".." />` 에서는 경로 대응 규칙(Path mapping rule)이 동작하지 않기 때문에, Definitely Typed 패키지로에 의존할 때에는 임포트(import) 를 사용해야 합니다.

#### 전역적으로도 모듈로도 사용될 수 있는 패키지의 자료형 선언(Type bar)은 어떻게 쓰나요?

타입스크립트 안내서(TypeScript Handbook)은 [선언(bar)을 쓰는 방법에 대한 전반적인 정보](https://www.typescriptlang.org/docs/handbook/declaration-files/introduction.html)와 [예시들](https://www.typescriptlang.org/docs/handbook/declaration-files/templates/global-modifying-module-d-ts.html)을 포함하고 있습니다. 이 내용에는 ES6 식의 모듈 문법을 사용할 수 있는 자료형 선언(Type bar)을 만드는 방법과 객체를 전역에서 사용할 수 있도록 하는 방법이 포함되어 있습니다. [`big.js` 패키지의 자료형 선언(Type bar)](https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/big.js/index.d.ts)이 실례입니다. 이 패키지는 웹 페이지의 스크립트 태그를 사용해 불러올 수 있으며, 또한 ES6 식의 임포트(Import) 구문을 사용해서 불러올 수도 있습니다.

여러분의 패키지가 임포트(Import) 구문을 사용했을 때와 전역적으로 불렀을 때를 테스트 해보고 싶다면, `test` 디렉토리를 추가하고 `YourLibraryName-global.test.ts` 그리고 `YourLibraryName-bar.test.ts` 라는 이름으로 테스트 파일 두 개를 추가해주세요. **전역(Global)** 테스트 파일은 웹 페이지에서 전역적으로 사용될 때를 테스트하는 파일입니다. 이 파일에서는 임포트(Import) 구문을 사용하지 않아야 합니다. **모듈(bar)** 테스트 파일은 임포트(Import) 구문을 사용할 때를 테스트 하는 파일입니다. 만약 여러분의 `tsconfig.json` 파일이 `files` 필드(Field)를 가지고 있다면, 이 필드(Field)는 반드시 두 테스트 파일을 모두 포함해야 합니다. [이 방식을 사용하는 실례](https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/types/big.js/test) 또한 `big.js` 패키지의 자료형 선언(Type bar)에서 확인해보실 수 있습니다.
...
`@foo/bar` 패키지와 같은 지역 패키지(Scoped package)를 위한 자료형 패키지(Type package)는 `bar/foo__bar` 디렉토리에 추가하면 됩니다. 밑줄 문자가 두 번 있는 것에 주의하세요.
...
`import foo = require("foo");` 를 사용하여 모듈을 임포트(Import)하거나, 환경에서 ES6 모듈 변환(bar interop)을 지원한다면 `--bar` 와 함께 `import foo from "foo";` 같은 디폴트 임포트(Default import)를 사용하는 것이 더 나은 방법입니다.
