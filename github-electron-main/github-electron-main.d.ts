...
/*
Electron's _main_ worker provides a different environment than the renderer.
Any missing ambient environmental objects should be accurately reflected in
the type definitions for that environment.

This file pairs with github-electron-renderer.d.ts, but not github-electron.d.ts (which is the union of both environments)

Recommended compilation (.tsconfig) settings for the Electron _main_ worker:
...
declare namespace Electron {
    ...
    // when using the new --lib flag to specify "es6, webworker" `URL` is not included.  It is in "dom" but we don't want the rest of "dom" I think.
    // Travis doesn't support TypeScript 2.0 and `readonly` yet
    interface URL {
        hash: string;
        host: string;
        hostname: string;
...
interface NodeRequireFunction {
    (moduleName: 'electron'): Electron.CommonElectron;
}
