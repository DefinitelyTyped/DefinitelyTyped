...
declare namespace __Highcharts {
    ...
    /**
     * The heatmap series type. This series type is available both in  and Highmaps.
    ...
    interface GlobalOptions extends Options {
         ...
         * .setOptions to set it before any chart is initiated.
         */
        lang?: LangObject;
    }
    ...
    interface Chart {
         ...
         * This is the constructor for creating a new chart object.
         * @param {Options} options The chart options
         * @param callback A function to execute when the chart object is finished loading and rendering. In most cases the chart is built in one thread, but in Internet Explorer version 8 or less the chart is sometimes initiated before the document is ready, and in these cases the chart object will not be finished directly after callingnew .Chart(). As a consequence, code that relies on the newly built Chart object should always run in the callback. Defining a chart.event.load handler is equivalent.
         * @return {ChartObject}
         */
    ...
    interface ElementObject {
         ...
         * specific to , like zIndex and rotation.
         *
         ...
         *
         * Attributes frequently used in  are fill, stroke, stroke-width.
    ...
    /**
     * Allows direct access to the  rendering layer in order to draw primitive shapes like circles,
     ...
     * use var renderer = new .Renderer(parentNode, width, height); where parentNode is the HTML element where you
     * want to add it.
     *
    ...
    interface RendererObject {
        ...
        /**
         * Draw a label, which is an extended text element with support for border and background.  creates a g
    ...
    interface SeriesObject {
         ...
         * @param [boolean] updatePoints - When the updated data is the same length as the existing data, points will be updated instead of replace. This option prevents this, and makes setData behave like it did prior to  3.0.10.
         * @since 1.2.0
         */
    ...
    var Color: (color: string | Gradient) => string | Gradient;
    var chart: {
        (options: Options, callback?: (chart: ChartObject) => void): ChartObject;
        (renderTo: string | HTMLElement, options: Options, callback?: (chart: ChartObject) => void): ChartObject;
    }
    var charts: ChartObject[];
    var dateFormat: (format: string, time?: number, capitalize?: boolean) => string;
    var dateFormats: DateFormatSpecifiers;
    var numberFormat: (value: number, decimals?: number, decimalPoint?: string, thousandsSep?: string) => string;
    var setOptions: (options: GlobalOptions) => Options;
    var getOptions: () => Options;
    var map: (array: any[], fn: Function) => any[];
}
