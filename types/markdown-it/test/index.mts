...
import ParserInline from "markdown-it/lib/parser_inline.mjs";
...
{
    const md = MarkdownIt().set({ html: true, breaks: true }).set({ typographer: true });
}

{
    const md = MarkdownIt().configure("default").configure("zero").configure("commonmark");
}
...

{
    ...
    const plugin4: PluginWithOptions<{ foo: boolean; bar: number }> = (md, options) => {};
    const plugin5: PluginWithParams = (md, flag?: boolean, str?: string, num?: number) => {};
