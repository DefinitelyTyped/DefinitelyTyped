...
{
    var md = MarkdownIt().use(plugin1).use(plugin2, opts).use(plugin3);
}
...
{
    var md = MarkdownIt().disable(['link', 'image']).enable(['link']).enable('image');
...
{
    var md = MarkdownIt().set({ html: true, breaks: true }).set({ typographer: true });
}
