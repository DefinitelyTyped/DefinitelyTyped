...
{
    fs.writev(1, [Buffer.from('123')] as ReadonlyArray<ArrayBufferView>, (err: ErrnoException | null, bytesWritten: number, buffers: ArrayBufferView[]) => {
    });
    const bytesWritten = fs.writevSync(1, [Buffer.from('123')] as ReadonlyArray<ArrayBufferView>);
}
...
{
    fs.readvSync(123, [Buffer.from('wut')] as ReadonlyArray<ArrayBufferView>);
    fs.readv(123, [Buffer.from('wut')] as ReadonlyArray<ArrayBufferView>, 123, (err: ErrnoException | null, bytesRead: number, buffers: ArrayBufferView[]) => {
    });
}
