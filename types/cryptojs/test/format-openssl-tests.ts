        ...
        Y.Test.Runner.add(
            new Y.Test.Case({
                ...
                testSaltedToString: function () {
                    Y.Assert.areEqual(
                        C.enc.Latin1.parse('Salted__')
                            .concat(this.data.salt)
                            .concat(this.data.ciphertext)
                            .toString(C.enc.Base64),
                        C.format.OpenSSL.stringify(
                            C.lib.CipherParams.create({ ciphertext: this.data.ciphertext, salt: this.data.salt }),
                        ),
                    );
                },
                ...
                testSaltedFromString: function () {
                    var openSSLStr = C.format.OpenSSL.stringify(
                        C.lib.CipherParams.create({ ciphertext: this.data.ciphertext, salt: this.data.salt }),
                    );
                ...
                testUnsaltedFromString: function () {
                    var openSSLStr = C.format.OpenSSL.stringify(
                        C.lib.CipherParams.create({ ciphertext: this.data.ciphertext }),
                    );
                    var cipherParams = C.format.OpenSSL.parse(openSSLStr);
