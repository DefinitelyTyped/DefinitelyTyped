        ...
        new Y.Test.Case({
            ...
            testDecryptKeySize128: function() {
                Y.Assert.areEqual(
                    ...
                    C.AES.decrypt(
                        C.lib.CipherParams.create({ ciphertext: C.enc.Hex.parse("69c4e0d86a7b0430d8cdb78070b4c55a") }),
                        C.enc.Hex.parse("000102030405060708090a0b0c0d0e0f"),
                        { mode: C.mode.ECB, padding: C.pad.NoPadding },
                    ).toString(),
                );
            ...
            testDecryptKeySize192: function() {
                Y.Assert.areEqual(
                    "00112233445566778899aabbccddeeff",
                    C.AES.decrypt(
                        C.lib.CipherParams.create({ ciphertext: C.enc.Hex.parse("dda97ca4864cdfe06eaf70a0ec0d7191") }),
                        C.enc.Hex.parse("000102030405060708090a0b0c0d0e0f1011121314151617"),
                        { mode: C.mode.ECB, padding: C.pad.NoPadding },
                    ).toString(),
                );
            ...
            testDecryptKeySize256: function() {
                Y.Assert.areEqual(
                    "00112233445566778899aabbccddeeff",
                    C.AES.decrypt(
                        C.lib.CipherParams.create({ ciphertext: C.enc.Hex.parse("8ea2b7ca516745bfeafc49904b496089") }),
                        C.enc.Hex.parse("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"),
                        { mode: C.mode.ECB, padding: C.pad.NoPadding },
                    ).toString(),
                );
            ...
            testHelper: function() {
                ...
                Y.Assert.areEqual(
                    ...
                    C.AES.encrypt("Hi There", C.SHA256("Jefe"), { mode: C.mode.ECB, padding: C.pad.NoPadding })
                        .ciphertext.toString(),
                );
                Y.Assert.areEqual(
