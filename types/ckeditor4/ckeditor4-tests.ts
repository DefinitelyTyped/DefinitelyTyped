...
function test_ckEditor() {
    ...
    let event: CKEDITOR.event = new ckEditor.event()
...
function test_dom_event() {
    ...
    element.on<CKEDITOR.dom.event>('mousemouse', (ev) => {
        const pageOffset = ev.data.getPageOffset()
        alert(pageOffset.x)
        alert(pageOffset.y)
    })
    element.on<CKEDITOR.dom.event<Element, CKEDITOR.dom.element>>(
        'click',
        (ev) => {
...
function test_dom_node() {
    ...
    prev = node.getPreviousSourceNode(
        ...
        (current: CKEDITOR.dom.node) => false
    )
    ...
    next = node.getNextSourceNode(
        ...
        (current: CKEDITOR.dom.node) => true
    )
...
function test_editable() {
    ...
    editable
        .attachListener(editable, 'event', (evt: CKEDITOR.eventInfo) =>
            console.log('listen')
        )
        .removeListener()
    editable
        .attachListener(
            new ckEditor.event(),
            'event',
            (evt: CKEDITOR.eventInfo) => console.log('listen')
        )
        .removeListener()
    editable
        .attachListener(
