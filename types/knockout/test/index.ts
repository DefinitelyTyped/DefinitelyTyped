...
function test_bindings() {
    ...
    ko.bindingHandlers.slideVisible = {
        update: function (element, valueAccessor, allBindingsAccessor) {
            var value = valueAccessor(),
                allBindings = allBindingsAccessor ? allBindingsAccessor() : null;
    ...
    ko.bindingHandlers.hasFocus = {
        ...
        update: function (element, valueAccessor) {
            var value = valueAccessor();
            if (ko.utils.unwrapObservable(value)) element.focus();
            else element.blur();
        },
...
function test_more() {
    ...
    function AppViewModel3() {
        ...
        this.throttledValue = ko.computed(this.instantaneousValue).extend({ throttle: 400 });
    ...
    observableArray.subscribe(
        (changes) => {
            changes.forEach((change) => {
                if (change.status == 'added') count++;
                else if (change.status == 'deleted') count--;
            });
        },
        null,
        'arrayChange',
