...
fetch('../controllers/php-post.php', {
    ...
    .then((data: any) => {
        if (data.email_check === 'valid') {
            $('#message_ajax').html(
                `<div class='successMessage'>${data.email} is a valid e-mail address. Thank you, ${data.name}.</div>`
            );
            $('#message_ajax').append(`<p>${data.msg}</p>`);
        } else {
            $('#message_ajax').html(
                `<div class='errorMessage'>Sorry ${data.name}, ${
                    data.email
                } is NOT a valid e-mail address. Try again.</div>`
            );
        }
    });
// PUT:
