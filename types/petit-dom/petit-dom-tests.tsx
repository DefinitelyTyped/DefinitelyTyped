...
export function testHtmlElementWithMixedContent() {
    // HyperScript syntax returns an ElementNode<T> object, with typed properties
    const formNode = h('form', { key: 1, method: 'POST', onsubmit: eventHandler }, 'Hello ', h('span', null, 'World'));
...
export function testFunctionComponentWithProps() {
    function FunctionComponentWithProps(props: CustomProps): JSX.Element {
        ...
        return <div className={name} tabIndex={count} onclick={onSomeEvent} />;
    }
...
export function testComponentClass() {
    class ComponentClass {
        ...
        mount(): Element {
            return mount(<div className="some-class" />);
        }
        ...
        patch(
            ...
            patch(<div {...oldProps}>{oldContent}</div>, <div {...newProps}>{newContent}</div>);
            return element;
        }
