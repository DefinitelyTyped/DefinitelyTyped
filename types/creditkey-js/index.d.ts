declare namespace _default {
	...
	export class CartItem {
		constructor(
			merchantProductId: string, name: string, price: number,
			sku: string | null, quantity: number, size: string | null, color: string | null)
	...
	export class Address {
		constructor(
			...
			address1: string, address2: string | null, city: string, state: string, zip: string, phone_number: string)
	...
	export type CheckoutResponse = { id: string, checkout_url: string };
	export type CustomerResponse = { status: Status, amount: number, amount_available: number };
	export class Client {
		constructor(key: string, platform?: Platform)
		begin_checkout(
			cartItems: CartItem[], billingAddress: Address, shippingAddress: Address,
			charges: Charges, remoteId: string, customerId: string,
			returnUrl: string, cancelUrl: string, mode: Mode, merchantData: any): Promise<CheckoutResponse>
