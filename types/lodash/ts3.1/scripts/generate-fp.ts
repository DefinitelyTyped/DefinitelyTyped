...
async function main() {
    ...
    for (const subfolder of subfolders) {
        promises.push(
            new Promise<InterfaceGroup[]>((resolve, reject) => {
    ...
    const interfaces = _.uniqBy(
        _.flatMap(interfaceGroups, (g) => g.interfaces),
        (i) => i.name,
    );
    ...
    fs.writeFile(path.join('..', 'fp.d.ts'), fpFile, (err) => {
        if (err) console.error('Failed to write fp.d.ts: ', err);
    });
    ...
    const tsconfig = tsconfigFile
        .split(lineBreak)
        .filter((row) => !row.includes('fp/') || row.includes('fp/convert.d.ts'));
    const newRows = interfaceGroups
        .map((g) => `        "fp/${g.functionName}.d.ts",`)
    ...
    if (!tsconfig[insertIndex - 1].endsWith(',')) tsconfig[insertIndex - 1] += ',';
    tsconfig.splice(insertIndex, 0, ...newRows);

    fs.writeFile(tsconfigPath, tsconfig.join(lineBreak), (err) => {
        if (err) console.error(`Failed to write ${tsconfigPath}: `, err);
    });
}
...
async function processDefinitions(filePaths: string[], commonTypes: string[]): Promise<InterfaceGroup[]> {
    ...
    for (const filePath of filePaths) {
        ...
        for (const definition of definitions) {
            definition.overloads = definition.overloads.filter(
                (o) => !o.params.some((p) => p.includes('StringIterator')),
            );
            if (
                definition.overloads.every(
                    (o) => o.params.length <= 1 && (o.returnType === 'typeof _' || o.returnType === 'LoDashStatic'),
                )
            ) {
    ...
    const functionNames = Object.keys(builderFp).filter(
        (key) => key !== 'convert' && typeof builderFp[key] === 'function',
    );
    const interfaceGroups: InterfaceGroup[] = functionNames.map(
        (functionName): InterfaceGroup => ({
            functionName,
    ...
    for (const functionName of functionNames) {
        const output = [`import { ${functionName} } from "../fp";`, `export = ${functionName};`, ''].join(lineBreak);
        const targetFile = `../fp/${functionName}.d.ts`;
        fs.writeFile(targetFile, output, (err) => {
            if (err) console.error(`failed to write file: ${targetFile}`, err);
        });
    }
...
async function parseFile(filePath: string, commonTypes: string[]): Promise<Definition[]> {
    ...
    while (newCommonType) {
        if (!commonTypes.includes(newCommonType[1])) commonTypes.push(newCommonType[1]);
    ...
    while (lodashStaticMatch) {
        ...
        if (endIndex === -1) {
            ...
            console.warn(
                `Failed to find end of interface 'LoDashStatic' (starting at ${filePath} line ${lineNumber}).`,
            );
            break;
        }
        const parsedDefinitions = parseDefinitions(
            definitionString,
            startIndex + lodashStaticMatch[0].length,
            endIndex,
            filePath,
            commonTypes,
        );
...
function parseDefinitions(
    ...
    for (; overloadMatch && overloadMatch.index < endIndex; overloadMatch = overloadRegExp.exec(definitionString)) {
        ...
        if (!currentDefinition || name !== currentDefinition.name) {
            if (currentDefinition && !_.isEmpty(currentDefinition.overloads)) definitons.push(currentDefinition);
            ...
            if (
                jsdocStartIndex !== -1 &&
                jsdocStartIndex > startIndex &&
                jsdocEndIndex !== -1 &&
                jsdocEndIndex < overloadStartIndex
            ) {
                ...
                if (overloadTestIndex === overloadStartIndex)
                    currentDefinition.jsdoc = definitionString
                        .substring(jsdocStartIndex, jsdocEndIndex + 2)
                        .replace(/    /g, '');
            }
        }
        ...
        if (name === 'chain' || name === 'mixin' || name.startsWith('prototype.')) continue;
        ...
        if (paramStartIndex !== -1) {
            ...
            if (previousLine.startsWith('// tslint:disable')) overload.tslintDisable = previousLine;
                    ...
                    overload.typeParams = overloadString
                        ...
                        .map(
                            (tpString): TypeParam => {
                                ...
                                if (parts[1]) typeParam.equals = parts[1].trim();
                                parts = tpString.split(' extends ');
                                if (parts[1]) typeParam.extends = parts[1].trim();
            ...
            if (!interfaceName.startsWith('{')) {
                ...
                if (ignoreInterfaceNames.includes(interfaceName)) continue;
                const interfaceRegExp = new RegExp(`( *)interface ${interfaceName} {`);
                const interfaceMatch = interfaceRegExp.exec(definitionString);
                if (!interfaceMatch) {
                    const lineNumber = getLineNumber(definitionString, overloadStartIndex);
                    console.warn(
                        `Failed to parse function '${name}': interface '${interfaceName}' not found. (${filePath} line ${lineNumber})`,
                    );
                    continue;
                }
                ...
                if (interfaceEndIndex === -1) {
                    ...
                    console.warn(
                        `Failed to find end of interface '${interfaceName}' (starting at ${filePath} line ${lineNumber}).`,
                    );
                    continue;
                }
            ...
            if (definition) {
                if (currentDefinition.jsdoc)
                    for (const overload of definition.overloads) overload.jsdoc = currentDefinition.jsdoc;
    ...
    if (parentName && currentDefinition) {
        ...
        for (
            let constantMatch = constantRexExp.exec(definitionString);
            constantMatch && constantMatch.index < endIndex;
            constantMatch = constantRexExp.exec(definitionString)
        ) {
...
function curryDefinition(
    ...
    let overloads = _.uniqWith(
        _.cloneDeep(definition.overloads),
        (a, b) =>
            _.isEqual(a.params, b.params) &&
            _.isEqual(getUsedTypeParams(a.params, a.typeParams), getUsedTypeParams(b.params, b.typeParams)),
    );
    ...
    if (!isFixed) {
        ...
        for (const overload of overloads) overload.params = overload.params.map((p) => p.replace(/\?:/g, ':')); // No optional parameters
        return [
            {
    ...
    if (spreadIndex !== -1) {
        ...
        if (spreadIndex === arity - 1) {
            for (let i = 0; i < overloads.length; ++i) {
                ...
                if (
                    overload.params.length === arity &&
                    overload.params[spreadIndex] &&
                    overload.params[spreadIndex].startsWith('...')
                ) {
                    // case 1
                    overload.params[spreadIndex] = overload.params[spreadIndex].replace('...', '');
                } else if (
                    overload.params.length === arity + 1 &&
                    overload.params[spreadIndex + 1] &&
                    overload.params[spreadIndex + 1].startsWith('...')
                ) {
                    // case 2
                    ...
                    overload.params.length >= arity &&
                    overload.params[spreadIndex] &&
                    !overload.params.some((p) => p.startsWith('...'))
                ) {
                    // case 3
    ...
    let filteredOverloads = overloads.filter(
        (o) =>
            o.params.length >= arity &&
            o.params.slice(arity).every((p) => p.includes('?') && !p.startsWith('iteratee')) &&
            o.params.every((p) => !p.startsWith('...') && !p.startsWith('guard:')),
    );
    if (filteredOverloads.length === 0)
        filteredOverloads = overloads.filter(
            (o) =>
                o.params.length >= arity &&
                o.params.slice(arity).every((p) => p.includes('?') || p.startsWith('...')) &&
                o.params.every((p) => !p.startsWith('guard:')),
        );
    if (filteredOverloads.length === 0)
        filteredOverloads = overloads.filter(
            (o) =>
                o.params.length > 0 && o.params.length <= arity + 1 && o.params[o.params.length - 1].startsWith('...'),
        );
    if (filteredOverloads.length === 0)
        console.warn(`No matching overloads found for ${functionName} with arity ${arity}`);

    const restOverloads = overloads.filter(
        (o) => o.params.length > 0 && o.params[o.params.length - 1].startsWith('...'),
    );
    for (const restOverload of restOverloads) {
        ...
        if (restOverload.params.length < arity) {
            ...
            const copiedParams = _.range(2, arity - restOverload.params.length + 2).map((i) =>
                paramToCopy.replace(/(^.+?):/, `$1${i}:`),
            );
            restOverload.params.push(...copiedParams);
        }
    }

    for (const overload of filteredOverloads) preProcessOverload(overload, functionName, arity);
    ...
    for (const overload of mainInterface.overloads) {
        const others = mainInterface.overloads.filter(
            (o2) =>
                o2 !== overload &&
        ...
        for (const otherOverload of others) {
            for (let i = 0; i < overload.params.length; ++i) {
                ...
                if (paramNames.length > 1) {
                    ...
                    overload.params[i] = `${paramNames[0]}Or${paramNames
                        .slice(1)
                        .map(_.upperFirst)
                        .join('Or')}: ${paramType}`;
                }
            ...
            if (otherOverload.returnType === overload.returnType) {
                ...
                const otherReturnInterface = interfaces.find((i) =>
                    new RegExp(`\\b${i.name}\\b`).test(otherOverload.returnType),
                );
                if (returnInterface && otherReturnInterface) {
                    _.remove(otherReturnInterface.overloads, (o) =>
                        new RegExp(`\\b${otherReturnInterface.name}\\b`).test(o.returnType),
                    );
                    ...
                    for (const overload of _.flatMap(interfaces, (i) => i.overloads))
                        overload.returnType = overload.returnType.replace(
                            new RegExp(`\\b${otherReturnInterface.name}\\b`),
                            returnInterface.name,
                        );
                }
            }
        }
    ...
    for (const interfaceDef of interfaces) interfaceDef.overloads = mergeSimilarOverloads(interfaceDef.overloads);
...
function preProcessOverload(overload: Overload, functionName: string, arity: number): void {
    overload.params = overload.params.slice(0, arity).map((p) =>
        p
            .replace(/\?:/g, ':') // No optional parameters
            .replace(/: *(?:Array<(.+)>|([^ |]+)\[\]|\((.+)\)\[\])$/, ': ReadonlyArray<$1$2$3>'),
    ); // Convert Array to ReadonlyArray because lodash/fp treats everything as immutable
    // Cap the number of callback arguments
    for (let i = 0; i < overload.params.length; ++i) overload.params[i] = capCallback(overload.params[i], functionName);
}
...
function capCallback(parameter: string, functionName: string): string {
    ...
    if (
        ...
        functionName !== 'unzipWith' &&
        /(?:iteratee|predicate|callback): *\((?:[^,)]+,[^)]+| *\.\.\..+)\) ?=>/.test(parameter)
    )
    ...
    if (functionName === 'mapKeys') {
        parameter = parameter
            .replace(
                /(iteratee|predicate|callback): *(?:Array|List|NumericDictionary|String)Iteratee<(.+)>$/g,
                '$1: ValueIteratee<number>',
            )
            .replace(
                /(iteratee|predicate|callback): *(?:Dictionary|Object)Iteratee<(.+)>$/g,
                '$1: ValueIteratee<string>',
            )
            .replace(
                /(iteratee|predicate|callback): *(?:Array|List|NumericDictionary|String)Iteratee(?:Custom)?<(.+?),(.+)>$/g,
                '$1: ValueIterateeCustom<number,$3>',
            )
            .replace(
                /(iteratee|predicate|callback): *(?:Dictionary|Object)Iteratee(?:Custom)?<(.+?),(.+)>$/g,
                '$1: ValueIterateeCustom<string,$3>',
            );
    }
    ...
    if (functionName === 'reduceRight') {
        parameter = parameter.replace(
            /(iteratee|predicate|callback): *Memo(Void)?(?:List|Object|Dictionary)Iterator<([^,>]+),([^,>]+)(?:,[^,>]+?(<T>)?)?>/g,
            '$1: Memo$2IteratorCappedRight<$3,$4>',
        );
    }

    return parameter
        .replace(
            /(iteratee|predicate|callback): *(?:Array|List|Dictionary|NumericDictionary|String)Iteratee<(.+)>$/g,
            '$1: ValueIteratee<$2>',
        )
        .replace(
            /(iteratee|predicate|callback): *(?:Array|List|Dictionary|NumericDictionary|String)Iteratee(?:Custom)?<(.+?),(.+)>$/g,
            '$1: ValueIterateeCustom<$2,$3>',
        )
        .replace(
            /(iteratee|predicate|callback): *(?:Array|List|Dictionary|NumericDictionary|String)IteratorTypeGuard<(.+?),(.+)>$/g,
            '$1: ValueIteratorTypeGuard<$2,$3>',
        )
        .replace(/(iteratee|predicate|callback): *ObjectIteratee<(.+)>$/g, '$1: ValueIteratee<$2[keyof $2]>')
        .replace(
            /(iteratee|predicate|callback): *ObjectIterateeCustom<(.+?),(.+)>$/g,
            '$1: ValueIterateeCustom<$2[keyof $2],$3>',
        )
        .replace(
            /(iteratee|predicate|callback): *ObjectIteratorTypeGuard<(.+?),(.+)>$/g,
            '$1: ValueIteratorTypeGuard<$2[keyof $2],$3>',
        )
        .replace(
            /(iteratee|predicate|callback): *(?:Array|List|Dictionary|NumericDictionary)Iterator<(.+?), *(.+)>$/g,
            '$1: (value: $2) => $3',
        )
        ...
        .replace(
            /(iteratee|predicate|callback): *Memo(Void)?(?:Array|List|Object|Dictionary)Iterator<([^,>]+),([^,>]+)(?:,[^,>]+?(<T>)?)?>/g,
            '$1: Memo$2IteratorCapped<$3,$4>',
        )
        .replace(
            /(iteratees|predicates|callbacks): *Many<(?:Array|List|Dictionary|NumericDictionary|String)Iteratee<(.+?)>>/g,
            '$1: Many<ValueIteratee<$2>>',
        )
        .replace(
            /(iteratees|predicates|callbacks): *Many<ObjectIteratee<(.+?)>>/g,
            '$1: Many<ValueIteratee<$2[keyof $2]>>',
        )
        .replace(
            /(iteratees|predicates|callbacks): *Many<(?:Array|List|Dictionary|NumericDictionary)Iterator<(.+?), *(.+?)>>/g,
            '$1: Many<(value: $2) => $3>',
        )
        .replace(
            /(iteratees|predicates|callbacks): *Many<ObjectIterator<(.+?), *(.+?)>>/g,
            '$1: Many<(value: $2[keyof $2]) => $3>',
        );
}
...
function curryOverload(overload: Overload, functionName: string, overloadId: number): Interface[] {
    ...
    for (const interfaceDef of interfaces) {
        for (const overload of interfaceDef.overloads) {
            const objectTypeParam = overload.typeParams.find(
                (tp) => tp.name === 'T' && (tp.extends === 'object' || !tp.extends),
            );
            if (
                objectTypeParam &&
                overload.params.some((p) => p.includes('T[keyof T]')) &&
                !overload.params.some((p) => /T(?!]|\[keyof T])/.test(p))
            ) {
                delete objectTypeParam.extends;
                overload.params = overload.params.map((p) =>
                    p.replace(/\bArray<T\[keyof T]>/g, ' T[]').replace(/\bT\[keyof T]/g, 'T'),
                );
                const fixInterfaceName = overload.returnType.split('<')[0];
                const fixInterface = interfaces.find((i) => i.name === fixInterfaceName);
                if (fixInterface) {
                    const fixTypeParam = fixInterface.typeParams.find(
                        (tp) => tp.name === objectTypeParam.name && (tp.extends === 'object' || !tp.extends),
                    );
                    if (fixTypeParam) {
                        ...
                        for (const fixOverload of fixInterface.overloads) {
                            ...
                            const typeParamSearch = new RegExp(
                                `\\b${fixTypeParam.name}\\b(?:(?!(?:\\[keyof ${fixTypeParam.name})?\\])|$)`,
                            );
                            const matches = fixOverload.params
                                .concat(fixOverload.returnType)
                                .filter((value) => typeParamSearch.test(value));
                            let replacement = 'object';
                            if (matches.length >= 2) {
                                let i = 1;
                                const newTypeParam: TypeParam = { name: 'T' + i, extends: replacement };
                                while (
                                    fixInterface.typeParams.some((t) => t.name === newTypeParam.name) ||
                                    fixOverload.typeParams.some((t) => t.name === newTypeParam.name)
                                )
                                    newTypeParam.name = 'T' + ++i;
                                fixOverload.typeParams.push(newTypeParam);
                                replacement = newTypeParam.name;
                            }
                            fixOverload.params = fixOverload.params.map((p) =>
                                p
                                    .replace(new RegExp(typeParamSearch, 'g'), replacement)
                                    .replace(
                                        new RegExp(`\\bArray<${fixTypeParam.name}\\[keyof ${fixTypeParam.name}]>`, 'g'),
                                        fixTypeParam.name + '[]',
                                    )
                                    .replace(
                                        new RegExp(`\\b${fixTypeParam.name}\\[keyof ${fixTypeParam.name}]`, 'g'),
                                        fixTypeParam.name,
                                    ),
                            );
                    ...
                    console.warn(
                        `Fixed T[keyof T] overload, but could not find corresponding interface '${fixInterfaceName}'.`,
                    );
                }
            }
        }
...
function curryParams(
    ...
    for (let i = 1; i <= lastOverload; ++i) {
        ...
        while (currentParams.length > 0 && _.last(currentParams)!.endsWith('__')) currentParams.pop(); // There's no point in passing a placeholder as the last parameter, so don't allow it.
        ...
        const currentReturnType =
            i < lastOverload
                ? getInterfaceName(baseName, sourceOverloadId, combinedIndex, passTypeParams)
                : sourceOverload.returnType;
    ...
    for (const typeParam of interfaceDef.typeParams) {
        ...
        if (!_.startsWith(typeParam.extends, 'keyof ') && typeParam.extends !== 'any[]') delete typeParam.extends;
    }
    return interfaceDef;
}
...
function getUsedTypeParams(params: string[], typeParams: TypeParam[]) {
    ...
    usedTypeParams.unshift(
        ...typeParams.filter(
            (tp) =>
                !usedTypeParams.includes(tp) &&
                usedTypeParams.some((tp2) => !!tp2.extends && new RegExp(`\\b${tp.name}\\b`).test(tp2.extends)),
        ),
    );
...
function getInterfaceTypeParams(overload: Overload, interfaceIndex: number) {
    ...
    const interfaceTypeParams = passedTypeParams.filter((tp) =>
        currentParams.concat(overload.returnType).some((p) => new RegExp(`\\b${tp.name}\\b`).test(p)),
    );
    interfaceTypeParams.unshift(
        ...overload.typeParams.filter(
            (tp) =>
                !interfaceTypeParams.includes(tp) &&
                interfaceTypeParams.some(
                    (tp2) => !!tp2.extends && new RegExp(`^keyof ${tp.name}\\b`).test(tp2.extends),
                ),
        ),
    );
...
function mergeInterfaces(interfaces: Interface[]): void {
    ...
    mainInterface.overloads = _.uniqWith(
        _.flatMap(interfaces, (i) => i.overloads),
        _.isEqual,
    );
}
...
function mergeSimilarOverloads(overloads: Overload[]): Overload[] {
    ...
    for (const overload of newOverloads) {
        ...
        const others = newOverloads.filter(
            (o2) =>
                o2 !== overload &&
                _.isEqual(overload.typeParams, o2.typeParams) &&
        ...
        const differingParamIndexes = _(others)
            ...
            .uniq()
            .value();
        if (differingParamIndexes.length > 1) continue; // Only one param is different, but it's a different param for some overloads, so we can't merge
        const similarOverloads = [overload].concat(others);
        for (let i = 0; i < overload.params.length; ++i) {
            const paramNames = _.uniq(similarOverloads.map((o) => getParamName(o.params[i])));
            const newParamName =
                paramNames.length > 1
                    ? `${paramNames[0]}Or${paramNames.slice(1).map(_.upperFirst).join('Or')}`
                    : paramNames[0];
...
function combineIndexes(i1: number, i2: number): number {
    ...
    for (let i2mask = 1; i2mask <= i2; i2mask <<= 1) {
        while (result & resultMask) resultMask <<= 1;
        if (i2 & i2mask) result += resultMask;
        resultMask <<= 1;
    }
    return result;
}
...
function getInterfaceName(baseName: string, overloadId: number, index: number, typeParams: TypeParam[]): string {
    ...
    if (index > 0) interfaceName += `${overloadId}x${index}${typeParamsToString(typeParams, false)}`;
...
function interfaceToString(interfaceDef: Interface): string {
    if (_.isEmpty(interfaceDef.overloads)) {
        ...
        const overloadStrings =
            interfaceDef.overloads.map((o) => lineBreak + tab(overloadToString(o), 1)).join('') +
            interfaceDef.constants.map((c) => `${lineBreak}${tab(c, 1)};`).join('');
        return `interface ${interfaceDef.name}${typeParamsToString(
            interfaceDef.typeParams,
        )} {${overloadStrings}${lineBreak}}`;
    }
}
...
function overloadToString(overload: Overload, arrowSyntax = false): string {
    ...
    let jsdoc = '';
    if (jsdoc) jsdoc += lineBreak;
    if (overload.tslintDisable) jsdoc += overload.tslintDisable + lineBreak;
    return `${jsdoc}${typeParamsToString(overload.typeParams)}(${joinedParams})${arrowSyntax ? ' =>' : ':'} ${
        overload.returnType
    };`;
}
