...
namespace TestFlatten {
...

    {
        ...
        result = _({ 0: 1, 1: [2, 3], length: 2 })
            .chain()
            .flatten<number>();
        result = _({ 0: 1, 1: [2, [3]], length: 2 })
            .chain()
            .flatten<number>(true);
        result = _({ 0: 1, 1: [2, [3]], 2: [[4]], length: 3 })
            .chain()
            .flatten<number>(true);
    }
...
namespace TestAny {
    ...
    let numericDictionaryIterator: (
        value: SampleObject,
        key: number,
        collection: _.NumericDictionary<SampleObject>,
    ) => boolean;
...
result = <{ a: number }[][]>(
    _.partition<{ a: number }, { a: number }>({ 0: { a: 1 }, 1: { a: 2 }, length: 2 }, { a: 2 })
);
...
result = <string[][]>_('abcd')
    .partition((n) => n < 'c')
    .value();
result = <string[][]>_(['a', 'b', 'c', 'd'])
    .partition((n) => n < 'c')
    .value();
result = <number[][]>_([1, 2, 3, 4])
    .partition((n) => n < 3)
    .value();
result = <number[][]>_({ 0: 1, 1: 2, 2: 3, 3: 4, length: 4 })
    .partition<number>((n) => n < 3)
    .value();
result = <number[][]>_({ a: 1, b: 2, c: 3, d: 4 })
    .partition<number>((n) => n < 3)
    .value();
result = <{ a: number }[][]>_([{ a: 1 }, { a: 2 }])
    .partition<{ a: number }>({ a: 2 })
    .value();
result = <{ a: number }[][]>_({ 0: { a: 1 }, 1: { a: 2 }, length: 2 })
    .partition<{ a: number }, { a: number }>({ a: 2 })
    .value();
result = <{ a: number }[][]>_({ 0: { a: 1 }, 1: { a: 2 } })
    .partition<{ a: number }, { a: number }>({ a: 2 })
    .value();
result = <{ a: number }[][]>_([{ a: 1 }, { a: 2 }])
    .partition('a')
...
result = <IStoogesAge[]>_.sortByAll(
    stoogesAges,
    (stooge) => Math.sin(stooge.age),
    (stooge) => stooge.name.slice(1),
);
...
result = <IFoodOrganic[]>_(foodsOrganic)
    .sortByAll('organic', (food) => food.name, { organic: true })
    .value();
...
namespace TestSortByOrder {
...

    {
        let iteratees: (
            value: SampleObject,
        ) => any | string | { a: number } | ((value: SampleObject) => any | string | { a: number })[];
        let result: SampleObject[];
...
result = <IStoogesCombined[]>_(stoogesCombined)
    .where({ quotes: ['Poifect!'] })
    .value();
...
result = <string[]>_(testReargFn)
    .rearg<TestReargResultFn>([2, 0, 1])
    .value()('b', 'c', 'a');
...
namespace TestClone {
...

    {
        ...
        result = _({ a: { b: 42 } })
            .chain()
            .clone<{ a: { b: string } }>(false, customizer);
        result = _({ a: { b: 42 } })
            .chain()
            .clone<{ a: { b: string } }>(false, customizer, any);
        result = _({ a: { b: 42 } })
            .chain()
            .clone<{ a: { b: string } }>(customizer);
        result = _({ a: { b: 42 } })
            .chain()
            .clone<{ a: { b: string } }>(customizer, any);
    }
}
...
namespace TestCloneDeep {
...

    {
        ...
        result = _({ a: { b: 42 } })
            .chain()
            .cloneDeep<{ a: { b: string } }>(customizer);
        result = _({ a: { b: 42 } })
            .chain()
            .cloneDeep<{ a: { b: string } }>(customizer, any);
    }
}
...
namespace TestAssign {
...

    {
        ...
        result = _.assign<Obj, { a: number; b: number; c: number; d: number; e: number }>(
            obj,
            s1,
            s2,
            s3,
...

    {
        ...
        result = _(obj).assign<{ a: number; b: number; c: number; d: number; e: number }>(
            s1,
            s2,
            s3,
            s4,
...

    {
        ...
        result = _(obj)
            .chain()
            .assign<S1, S2, S3, S4, { a: number; b: number; c: number; d: number }>(s1, s2, s3, s4, customizer);
        result = _(obj)
            .chain()
            .assign<S1, S2, S3, S4, { a: number; b: number; c: number; d: number }>(s1, s2, s3, s4, customizer, any);
    }

    {
        ...
        result = _(obj)
            .chain()
            .assign<{ a: number; b: number; c: number; d: number; e: number }>(s1, s2, s3, s4, s5, customizer);
        result = _(obj)
            .chain()
            .assign<{ a: number; b: number; c: number; d: number; e: number }>(s1, s2, s3, s4, s5, customizer, any);
    }
}
...
namespace TestDefaults {
...

    {
        ...
        result = _(obj)
            .chain()
            .defaults<S1, S2, S3, S4, { a: string; b: number; c: number; d: number }>(s1, s2, s3, s4);
    }
...
namespace TestFindKey {
...

    {
        ...
        result = _<{ a: string }>({ a: '' })
            .chain()
            .findKey<string>(predicateFn);
        result = _<{ a: string }>({ a: '' })
            .chain()
            .findKey<string>(predicateFn, any);
    }
}
...
namespace TestFindLastKey {
...

    {
        ...
        result = _<{ a: string }>({ a: '' })
            .chain()
            .findLastKey<string>(predicateFn);
        result = _<{ a: string }>({ a: '' })
            .chain()
            .findLastKey<string>(predicateFn, any);
    }
}
...
namespace TestMerge {
    ...
    var complicatedResult: ComplicatedExpectedType = _.merge(
        { a: 1 },
        { b: 'string' },
        { c: {} },
        { d: [1] },
    ...
    var overriddenResult: ExpectedTypeAfterOverriding = _.merge(
        { a: 1 },
        { a: 'string' },
        { a: {} },
        { a: [1] },
    ...
    var complicatedResult: ComplicatedExpectedType = _({ a: 1 })
        .merge({ b: 'string' }, { c: {} }, { d: [1] }, { e: true })
        .value();

    // Test for type overriding with chaining

    var overriddenResult: ExpectedTypeAfterOverriding = _({ a: 1 })
        .merge({ a: 'string' }, { a: {} }, { a: [1] }, { a: true })
        .value();
}
...
namespace TestResult {
...

    {
        ...
        result = _('abc')
            .chain()
            .result<_.LoDashExplicitWrapper<string>>(['0'], () => '_');
        result = _('abc')
            .chain()
            .result<_.LoDashExplicitWrapper<string>>(['0'], () => '_');
    }

    {
        ...
        result = _([42])
            .chain()
            .result<_.LoDashExplicitWrapper<number>>('0', () => -1);
        result = _([42]).chain().result<_.LoDashExplicitWrapper<number>>(['0']);
        result = _([42]).chain().result<_.LoDashExplicitWrapper<number>>(['0'], -1);
        result = _([42])
            .chain()
...

    {
        ...
        result = _({ a: true })
            .chain()
            .result<_.LoDashExplicitWrapper<boolean>>('a', () => false);
        result = _({ a: true }).chain().result<_.LoDashExplicitWrapper<boolean>>(['a']);
        result = _({ a: true }).chain().result<_.LoDashExplicitWrapper<boolean>>(['a'], false);
        result = _({ a: true })
            .chain()
...
namespace TestConstant {
    ...
    {
        let result: _.LoDashExplicitObjectWrapper<() => { a: string }> = _({ a: 'a' })
            .chain()
            .constant<{ a: string }>();
    }
}
