...
function testWithHandlers() {
    ...
    const InnerComponent: React.StatelessComponent<InnerProps & HandlerProps & OutterProps> = ({
        onChange,
        onSubmit,
        foo,
...
function testRenameProp() {
    ...
    const innerComponent: React.StatelessComponent<InnerProps> = ({ c, b }: InnerProps) => (
        <div>
            {c}, {b}
        </div>
    );
...
function testRenameProps() {
    ...
    const innerComponent: React.StatelessComponent<InnerProps> = ({ c, d }: InnerProps) => (
        <div>
            {c}, {d}
        </div>
    );
...
function testFlattenProp() {
    ...
    const innerComponent: React.StatelessComponent<InnerProps> = (props: InnerProps) => <div />;
...
function testWithState() {
    ...
    const InnerComponent: React.StatelessComponent<InnerProps> = props => <div onClick={() => props.setCount(0)} />;
...
function testWithStateHandlers() {
    ...
    // The inner props should be fully inferrable
    const enhancer2 = withStateHandlers((props: OutterProps) => ({ counter: props.initialCounter }), {
        add: (state, props) => n => ({ ...state, counter: state.counter + n ** props.power }),
    });
...
function testBranch() {
    ...
    const enhancer2 = branch((props: OutterProps) => props.toggled, renderComponent(innerComponent));
    const enhanced: React.ComponentClass<OutterProps> = enhancer(innerComponent);
