...
declare namespace OneSignalCordovaPlugin {
	interface OneSignalCordovaPlugin {
		...
		/** Get the current notification and permission state. Returns an object of OSPermissionSubscriptionState type described below. */
		...
		/** Sets a In-App Message clicked handler. The instance will be called when an In-App Message action is tapped on. */
		...
		/** You can call this method with false to opt users out of receiving all notifications through OneSignal. You can pass true later to opt users back into notifications. This unsubscribes the user from OneSignal, but the device will still show as subscribed to push. This method is helpful if you need to unsubscribe users momentarily or provide the option in a "settings" page. */
