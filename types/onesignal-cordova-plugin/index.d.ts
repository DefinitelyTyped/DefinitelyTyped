...
declare namespace OneSignalCordovaPlugin {
	interface OneSignalCordovaPlugin {
		...
		addPermissionObserver(
			callback: (change: { from: OSPermissionState; to: OSPermissionState }) => void
		): void;
		...
		/** Add a key-value Object of triggers, may show an In-App Message if its
		...
		/** Deletes a single tag that was previously set on a user with sendTag or
		 * sendTags. Use deleteTags if you need to delete more than one. */
		...
		/** By default OneSignal plays the system's default notification sound when
		 * the device's notification system volume is turned on. You may also set
		 * custom sounds on notifications. Passing false means that the device will
		 * only vibrate unless the device is set to a total silent mode. */
		enableSound(enable: boolean): void;
		/** By default OneSignal always vibrates the device when a notification is
		 * displayed unless the device is in a total silent mode. Passing false
		 * means that the device will only vibrate lightly when the device is in
		 * it's vibrate only mode. */
		enableVibrate(enable: boolean): void;
		getIds(IdsReceivedCallBack: (id: { userId: string; pushToken: string }) => void): void;
		/** Get the current notification and permission state. Returns an object of
		 * OSPermissionSubscriptionState type described below. */
		...
		/** Sets a In-App Message clicked handler. The instance will be called when
		 * an In-App Message action is tapped on. */
		handleInAppMessageClicked(handler: (action: OSNotificationAction) => void): void;
		/** If your app implements logout functionality, you can call logoutEmail to
		 * dissociate the email from the device: */
		logoutEmail(onSuccess: (success: any) => void, onFailure: (error: any) => void): void;
		/** Allows you to temporarily pause all In-App Messages. You may want to do
		 * this while the user is watching a video playing a match in your game to
		 * make sure they don't get interrupted at a bad time. */
		pauseInAppMessages(pause: boolean): void;
		/** Allows you to send notifications from user to user or schedule ones in
		 * the future to be delivered to the current device. */
		postNotification(
			notificationObj: Partial<OSNotification>,
			onSuccess: (json: any) => void,
			onFailure: (json: any) => void
		): void;
		/** Prompt the user for notification permissions. Callback fires as soon as
		 * the user accepts or declines notifications. */
		promptForPushNotificationsWithUserResponse(callback: (accepted: boolean) => void): void;
		/** Prompts the user for location permissions. This allows for geotagging
		 * so you can send notifications to users based on location. */
		promptLocation(): void;
		/** If your application is set to require the user's privacy consent, you
		 * can provide this consent using this method. Until you call
		 * provideUserConsent(true), the SDK will not fully initialize and will not
		 * send any data to OneSignal. */
		provideUserConsent(granted: boolean): void;
		registerForPushNotifications(): void;
		/** If your user logs out of your app and you would like to disassociate
		 * their custom user ID from your system with their OneSignal user ID, you
		 * will want to call this method. */
		removeExternalUserId(): void;
		/** Removes a single trigger for the given key, may show an In-App Message
		 * if its triggers conditions were met. */
		removeTriggerForKey(key: string): void;
		/** Removes a list of triggers based on a collection of keys, may show an
		 * In-App Message if its triggers conditions were met. */
		removeTriggersForKeys(keys: string[]): void;
		/** Tag a user based on an app event of your choosing so later you can
		 * create segments in Segments to target these users. Use sendTags if you
		 * need to set more than one tag on a user at a time. */
		 ...
		 * authentication token and send it to your app. The following code also
		 * includes callbacks: */
		setEmail(
			email: string,
		...
		/** If your system assigns unique identifiers to users, it can be annoying to
		 * have to also remember their OneSignal user ID's as well. To make things
		 * easier, OneSignal now allows you to set an external_id for your users.
		 * Simply call this method, pass in your custom user ID (as a string), and
		 * from now on when you send a push notification, you can use
		 * include_external_user_ids instead of include_player_ids. */
		 ...
		 * OneSignal. The logging levels are available with increasingly more
		 ...
		 * provides privacy consent. The SDK will not be fully initialized until the
		 * provideUserConsent(true) method is called. If you set this to be true,
		 * the SDK will not fully initialize until consent is provided. You can
		 * still call OneSignal methods, but nothing will happen, and the user will
		 * not be registered for push notifications. */
		setRequiresUserPrivacyConsent(required: boolean): void;
		/** You can call this method with false to opt users out of receiving all
		 * notifications through OneSignal. You can pass true later to opt users back
		 * into notifications. This unsubscribes the user from OneSignal, but the
		 * device will still show as subscribed to push. This method is helpful if
		 * you need to unsubscribe users momentarily or provide the option in a
		 * "settings" page. */
		...
		/** Accepts a callback, which returns a boolean variable indicating if the
		 * user has given privacy consent yet. */
