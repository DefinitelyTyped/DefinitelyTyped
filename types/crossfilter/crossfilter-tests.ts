...
var total_payments: number = payments
    .groupAll<number>()
    .reduce(
        function (p: number, v: any) {
            return (p += v.total);
        },
        function (p: number, v: any) {
            return (p -= v.total);
...
{
    var p: Payment = bottomPayments[0];
...
{
    var s: string = topTypes[0].key;
} // the top payment type (e.g., "tab")
{
    var n: number = topTypes[0].value;
} // the payment volume for that type (e.g., 900)
