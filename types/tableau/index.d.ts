// Type definitions for tableau v2.2.1
...
declare module tableau {
    interface VizCreateOptions {
        hideTabs?: boolean /** Undoes action on sheet, defaults to a single undo unless optional parameters is specified. */
        hideToolbar?: boolean /** Indicates whether the toolbar is hidden or shown. */
        ...
        onFirstInteractive?: (e: TableauEvent) => void /** Callback function that is invoked when the Viz object first becomes interactive. This is only called once, but it’s guaranteed to be called. If the Viz object is already interactive, it will be called immediately, but on a separate "thread." */
        onFirstVizSizeKnown?: (e: VizResizeEvent) => void /** Callback function that's invoked when the size of the Viz object is known. You can use this callback to perform tasks such as resizing the elements surrounding the Viz object once the object's size has been established. */
        [filter: string]: any /** Apply a filter that you specify to the view when it is first rendered. For example, if you have an Academic Year filter and only want to display data for 2017, you might enter "Academic Year": "2016". For more information, see Filtering. */
    }
    ...
    enum FieldAggregationType {
        SUM, AVG, MIN, MAX, STDEV, STDEVP, VAR, VARP, COUNT, COUNTD, MEDIAN, ATTR, NONE, YEAR, QTR, MONTH, DAY, HOUR, MINUTE, SECOND, WEEK, WEEKDAY, MONTHYEAR, MDY, END, TRUNC_YEAR, TRUNC_QTR, TRUNC_MONTH, TRUNC_WEEK, TRUNC_DAY, TRUNC_HOUR, TRUNC_MINUTE, TRUNC_SECOND, QUART1, QUART3, SKEWNESS, KURTOSIS, INOUT, USER
    }
    ...
    class Viz {
        /** Creates a new Tableau Viz inside of the given HTML container, which is typically a <div> element. Each option as well as the options parameter is optional. If there is already a Viz associated with the parentElement, an exception is thrown. Before reusing the parentElement you must first call dispose(). */
        ...
        /** Shows or hides the iframe element hosting the visualization. */
        hide(): void
        /** Cleans up any resources associated with the visualization, removes the visualization from the VizManager instance, and removes any DOM elements from the parentElement object. In effect, this method restores the page to what it was before a Viz object was instantiated. */
        dispose(): void
        ...
        /** Shows the Export Data dialog, which is currently a popup window. The worksheetInDashboard parameter is optional. If not specified, the currently active Worksheet is used. */
        ...
        /** Sets the size of the iframe element, which causes the visualization to expand or collapse to fit the iframe element if the visualization size (current sheet's size) is set to AUTOMATIC. */
    ...
    class Worksheet {
        ...
        /** Gets aggregated data for the fields used in the currently active sheet and returns it as an object. You can specify options with an optional parameter. This can only be called on sheets of the WORKSHEET type. */
        getSummaryDataAsync(options: getSummaryDataOptions): Promise<DataTable>
        /** Gets data for all fields in the data source used by the currently active sheet and returns it as an object. You can specify options with an optional parameter. This can only be called on sheets of the WORKSHEET type. */
    ...
    class CategoricalFilter extends Filter {
        ...
        /** Gets the collection of values that are currently set on the filter. This is a native JavaScript array and not a keyed collection. Note that only the first 200 values are returned. */
