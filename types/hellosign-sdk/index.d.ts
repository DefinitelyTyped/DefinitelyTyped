...
export interface SignatureRequestModule {
    ...
    download(requestId: string, options: { file_type: string }, callback: (err: Error, response: any) => void): void;
...
export interface EmbeddedRequestOptions {
    test_mode?: number;
    template_id: string;
    cc_roles?: any[];
    merge_fields?: any[];
...
export interface ReportsRequestOptions {
    test_mode?: number;
    start_date: string;
    end_date: string;
    report_type: string[];
}
export interface Report extends Omit<ReportsRequestOptions, 'test_mode'> {
    success?: string;
}
...
export interface OAuth extends GenericObject {
...
export interface AccountIdOrEmailRequestOptions {
    account_id: string;
}
export interface AccountIdOrEmailRequestOptions {
    email_address: string;
...
export interface TeamModule {
    ...
    removeMember(options: { new_owner_email_address?: string } & AccountIdOrEmailRequestOptions): Promise<TeamResponse>;
}
...
export interface UnclaimedDraft {
    signature_request_id: string;
    claim_url: string;
    signing_redirect_url: string;
...
export interface UnclaimedDraftRequestOptions {
    test_mode?: number;
    file?: string[];
    file_url?: string[];
    type: string;
...
export interface UnclaimedDraftModule {
    ...
    editAndResend(requestId: string, options: Partial<UnclaimedDraftRequestOptions>): Promise<UnclaimedDraftResponse>;
}

export type HellosignEvents =
...
export interface EventResponse extends SignatureRequestResponse, Event {}
...
export interface ApiAppRequestOptions {
    name?: string;
    domain?: string;
    callback_url?: string;
