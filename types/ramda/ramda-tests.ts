...
() => {
    ...
    const a: Dictionary<number> = R.pipe(R.filter<number, 'object'>(isEven))({ a: 0, b: 1 }); // => { a: 0 }

    const b: number[] = R.pipe(R.filter<number, 'array'>(isEven))([0, 1]); // => [0]

    const c: Dictionary<number> = R.pipe(R.reject<number, 'object'>(isEven))({ a: 0, b: 1 }); // => { b: 1 }
...
() => {
    ...
    const getMemberTitle: (email: string) => Promise<string> = R.pipe(makeQuery, fetchMember, R.then(getTitleAsync));
};
...
() => {
    const sortByNameCaseInsensitive = R.sortBy(
        R.compose<Record<'name', string>, string, string>(R.toLower, R.prop('name')),
    );
    const alice = {
        name: 'ALICE',
        age: 101,
