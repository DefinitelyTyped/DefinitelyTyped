 ...
 */
{
    var choices = Array.apply(0, new Array(26)).map((_x: number, y: number) => String.fromCharCode(y + 65));
 ...
 */
{
    var observe = Observable.create(function(obs: Observer<inquirer.Question<{ phone: string }>>) {
        obs.next({
            type: 'input',
            name: 'first_name',
            message: "What's your first name",
 ...
 */
{
    ...
    const likesFood = (aFood: keyof Answers) => (answers: Answers) => answers[aFood];
