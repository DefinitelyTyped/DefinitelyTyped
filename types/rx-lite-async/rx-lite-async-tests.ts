...
function toAsync() {
    ...
    obsStr = <any>(
        Rx.Observable.toAsync((a1: string, a2: number, a3: Date) => a1 + a2.toFixed(0) + a3.toDateString())(
            '',
            1,
            new Date(),
        )
    ...
    obsStr = <any>(
        Rx.Observable.toAsync(
            (a1: string, a2: number, a3: Date, a4: boolean) => a1 + a2.toFixed(0) + a3.toDateString() + (a4 ? 1 : 0),
        )('', 1, new Date(), false)
    );
}
