...
console.log(fs.absolute(path.join('~', 'does', 'not', 'exist')) === path.join(homeDir, 'does', 'not', 'exist'));

console.log(fs.normalize('~/foo') === path.join(homeDir, 'foo'));

console.log(fs.tildify(homeDir) === '~');

console.log(fs.getAppDataDirectory() === path.join(fs.getHomeDirectory(), 'Library', 'Application Support'));
