...
parallel([http.get('/foo.json'), http.get('/bar.json'), http.get('/baz.json')]).next(function (results) {
    alert(results);
});
...
parallel([$.get('README.markdown'), $.get('ChangeLog')]).next(function (values) {
...
parallel({ html: $.get('README.markdown'), js: $.get('ChangeLog') }).next(function (values) {
