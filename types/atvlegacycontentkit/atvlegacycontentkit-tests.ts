...
function device() {
    ...
    console.log(atv.device.screenFrame.height, atv.device.screenFrame.width, atv.device.screenFrame.x, atv.device.screenFrame.y);
...
function fullScreenMediaBrowser() {
    var mediaBrowser = new atv.FullScreenMediaBrowser();
    mediaBrowser.type = 'commentsScreenOnly'

    mediaBrowser.onLoadMetadata = function (photoId) {
        // Do nothing
        console.log("on load metadata " + photoId)
        mediaBrowser.updateMetadata(photoId, { "liked": true, "likeStatus": "liked this", "comments": [{ "text": "Test comment", "footer": "Test footer" }] })
    };
...
atv.onExecuteQuery = (query, callback) => {
    ...
    callback.success([{ id: 'test', type: 'photo', 'assets': [{ width: 10, height: 10, src: 'https://avatars.githubusercontent.com/u/3637556' }] }]);
...
function player() {
    if (atv.player) {
        console.log(atv.player.states.FastForwarding, atv.player.states.Loading, atv.player.states.Paused, atv.player.states.Rewinding, atv.player.states.Playing, atv.player.states.Stopped);
        console.log(atv.player.events.FFwd, atv.player.events.Pause, atv.player.events.Play, atv.player.events.Rew, atv.player.events.SkipFwd, atv.player.events.SwipBack);
...
if (atv.player) {
    ...
    atv.player.onTimedMetdataChanged = (m) => console.log(m, m.dataValue, m.dateValue, m.duration, m.extraAttributes, m.key, m.numberValue, m.startDate, m.stringValue, m.time, m.value);
...
function storeFront() {
    ...
    req.onProductsRequestDidReceiveResponse = (res) => {
        ...
        res.products.forEach((p) => console.log(p.localizedDescription, p.localizedPrice, p.localizedTitle, p.price, p.priceLocale, p.priceLocale.localeIdentifier, p.productIdentifier));
    };
    req.onRequestDidFinish = () => console.log("finish");
    req.onRequestDidFailWithError = (error) => console.log(error, error.code, error.error, error.message, error.transaction);
    req.start();
    req.cancel();

    var obs : SKPaymentTransactionObserver = {
        updatedTransactions: (t) => { 
            console.log(t);
            t.forEach((tr) => console.log(tr.originalTransaction, tr.payment, tr.transactionDate, tr.transactionIdentifier, tr.transactionReceipt, tr.transactionState));
    ...
    const payment = { product: { localizedPrice: "$0.00", localizedDescription: "test", localizedTitle: "test 2", price: 0, productIdentifier: "test", priceLocale: { localeIdentifier: "" } }, quantity: 1, requestParameters: {} };
    atv.SKDefaultPaymentQueue.addPayment(payment);
    console.log(atv.SKDefaultPaymentQueue.canMakePayments);
    atv.SKDefaultPaymentQueue.finishTransaction({payment: payment, transactionDate: new Date(), transactionIdentifier: "", transactionReceipt: {}, transactionState: atv.SKPaymentTransactionStatePurchased});

    console.log(atv.SKPaymentTransactionStateDeferred, atv.SKPaymentTransactionStateFailed, atv.SKPaymentTransactionStatePurchased, atv.SKPaymentTransactionStatePurchasing, atv.SKPaymentTransactionStateRestored);
    ...
    if (atv.SKDefaultPaymentQueue.transactions != null) {
        atv.SKDefaultPaymentQueue.transactions.forEach((tr) => console.log(tr.originalTransaction, tr.payment, tr.transactionDate, tr.transactionIdentifier, tr.transactionReceipt, tr.transactionState));
    }

    console.log(atv.SKErrorUnknown, atv.SKErrorClientInvalid, atv.SKErrorPaymentCancelled, atv.SKErrorPaymentInvalid, atv.SKErrorPaymentNotAllowed, atv.SKErrorStoreProductNotAvailable);
...
function slideShow() {
    ...
    const photos: MediaBrowserPhoto[] = [{ id: 'test', type: 'photo', assets: [ { width: 10, height: 10, src: "https://avatars.githubusercontent.com/u/3637556?s=200&v=4" } ] }];
