// Type definitions for Noteflight Client API v2.0
// Project: Noteflight Client API v2.0
...
/**
 * The global Noteflight API object. After the page has loaded, you must call the NFClient.init() function to initialize the Client API before using any other functions. 
 * @see https://www.noteflight.com/info/api/client_doc_v2
 */
declare var NFClient: {
    /**
     * The init() function performs additional API initialization and then invokes an optional callback function that is passed to it. The callback function, when called, is in turn passed an object that describes the API's properties and capabilities.
     */
    init: ( callback: (info: {
        version: string;
    }) => void) => void,
    /**
     * To replace an element with a Noteflight embedded document, use the API to create a new NFClient.ScoreView object, passing the element ID of the score and a special options object that describes the score to be created.
...
type Options = {
    /**
     * The hostname of the Noteflight site containing the embedded score. The default hostname is "www.noteflight.com".
     */
    host?: string,
    /**
     * The width of the embedded score in pixels. Default value: 800.
     */
    width?: number,
    /**
    ...
    viewParams?: {
        /**
         * This parameter optionally provides the visual magnification factor to be applied to the score in the embedded view. A value of 1 indicates no magnification (equivalent to a Zoom value of 100%). A value of 1.5 would make the score view larger (equivalent to 150%) while a value of 0.5 would make it smaller (50%). This magnification overrides any Zoom slider settings in the regular Noteflight view of the score. If omitted, the default factor is 1 (again, not necessarily the factor chosen in the Noteflight full page view).
         */
        scale?: number,
        /**
         * This optional parameter may be used to control how playback is handled in the embed, and also whether instrument sounds are downloaded or not. There are three settings for this parameter. If this option is not provided, then the playback mode is determined by the default setting in the score.
         */
        playback?: 'normal' | 'audioTrack' | 'echo' | 'silent',
        /**
         * This optional parameter specifies the user's role with respect to the document. Only two settings are permitted. The default setting is reader.
         */
        ...
        /**
         * This parameter hides the playback controls. Default is false.
         */
        hidePlaybackControls?: boolean,
        /**
         * This parameter hides the full window button in the header of an embed. It also suppresses the display of links to the full score in the Sharing panel. Default is false.
         */
...
type NoteflightEvent<T> = {
    ...
    embedld?: string,
    /**
     * This event is dispatched whenever an embed instance finishes loading a score from the Noteflight server and is ready to accept user interaction. Document methods should not be called until this event has been received.
     */
    scoreId: T extends 'scoreDataLoaded' ? string : T extends 'any' ? string | undefined : undefined,
    /**
     * If this property is absent, indicates that there is no active selection. Otherwise, has one of the following values:
     ...
     */
    kind: T extends 'selectionChange' | 'any' ? 'object' | 'measure' | 'range' | undefined : undefined,
    /**
     * The zero-based index of the measure at the start of the selected range.
     */
    startIndex: T extends 'selectionChange' ? number : T extends 'any' ? number | undefined : undefined,
    /**
     ...
     */
    startOffset: T extends 'selectionChange' ? number : T extends 'any' ? number | undefined : undefined,
    /**
     * The zero-based index of the measure which includes the end of the selected range. When kind has the value "measure" this will be a exclusive end index analogous to the argument to selectMeasures().
     */
    endIndex: T extends 'selectionChange' ? number : T extends 'any' ? number | undefined : undefined,
    /**
     ...
     */
    endOffset: T extends 'selectionChange' ? number : T extends 'any' ? number | undefined : undefined,
    /**
     * Optional property only present when kind has the value "object" or "range", providing an array of zero-based staff indices, counting down from the topmost staff in the score. If given, only the given staves are included in the selection.
     */
    staffIndices: T extends 'selectionChange' ? number[] : T extends 'any' ? number[] | undefined : undefined,
    /**
     * A deep-linking fragment representing the selection which can be passed to the selectFragment() function if desired.
     */
    fragment: T extends 'selectionChange' ? string : T extends 'any' ? string | undefined : undefined,
    /**
     * The zero-based index of the measure at which playback should start.
     */
    index: T extends 'playbackRequest' | 'playbackStop' ? number : T extends 'any' ? number | undefined : undefined,
    /**
     * The offset in quarter notes from the beginning of the start measure at which playback is to start. This value may range from zero to the number of quarter notes in the measure.
     */
    offset: T extends 'playbackRequest' | 'playbackStop' ? number : T extends 'any' ? number | undefined : undefined,
}
 ...
 */
type DocumentMethodPromise<Result> = {
    /**
     * To wait for a method to complete or to obtain its return value if it has one, call done() on the Promise object returned by the method and pass a callback that will receive the return value of the method (if any).
     */
    done: (callback: (result: Result) => void | ((result: Result) => void)[]) => DocumentMethodPromise<Result>,
    /**
...
type NoteSet = {
    /**
     * an array of Objects representing the notes in a note set. If empty, this indicates that the note set is a rest. If one element is present, indicates a regular note. If multiple elements, indicates a chord.
     */
    notes: Note[],
    /**
...
type KeySignature = {
    ...
    tonic: string,
    /**
     * 	the key signature's distance from C major / A minor by fifths, expressed as a number between -7 and 7. If positive, equal to the number of sharps in the key signature; if negative, equal to the number of flats.
     */
    fifths: number
}
...
type ScoreView = {
    ...
    addEventListener: <T extends EventType>(
        eventType: `${T}`,
        callback: (event: NoteflightEvent<typeof eventType>) => void,
    ) => void,
    /**
    ...
    /**
     * Causes a contiguous run of measures in the document to be selected, starting with the measure at startIndex and ending at the measure just before endIndex. If endIndex is -1, it is interpreted as the number of measures in the score.
     * @param startIndex The inclusive zero-based index of the measure that begins the selected range.
     * @param endIndex f positive, the exclusive zero-based index of the measure following the selected range. If -1, this parameter is taken as the number of measures in the score, as a convenience for selecting the entire score.
     */
    selectMeasures: (startIndex: number, endIndex: number) => DocumentMethodPromise<undefined>,
    /**
     * Causes a contiguous time range in the document to be selected, covering some or all of the staves in the document. The starting point of the range is defined by startIndex and startOffset, while the end is defined by endIndex and endOffset. The optional staffIndices argument allows specification of a specific set of staves to be selected.
     * @param startIndex The inclusive zero-based index of the measure that begins the selected range.
     * @param startOffset The offset in quarter notes from the beginning of the start measure at which the range begins. This value may range from zero to the number of quarter notes in the measure.
     * @param endIndex The zero-based index of the measure which includes the end of the selected range.
     * @param endOffset The offset in quarter notes from the beginning of the end measure at which the range ends. This offset may range from zero to the number of quarter notes in the measure.
     * @param staffIndices Optional argument providing an array of zero-based staff indices, counting down from the topmost staff in the score. If given, only the given staves are included in the selection.
     */
    ...
    /**
     * Returns a JSON-like structure describing the entire score in the embed. For modifiable embeds, permits programmatic access to their contents. The returned data is a copy of the score in the embed; it has no live connection to the actual score. Therefore, modifying the returned objects has no effect on the embed instance's score.
     */
    ...
    /**
     * Controls the behavior whereby Noteflight automatically advances the note entry location after a note is entered (the default, corresponding to an argument of true). This can be selectively disabled in situations where the focus is on chord entry.
     ...
     * @param mode This parameter may be used to control how playback is handled in the embed, and also whether instrument sounds are downloaded or not.
     */
     ...
     *  @param partIndicies 	An array containing one or more zero-based indices of parts in the score to be shown. If an empty array is given, all parts in the score are displayed.
     */
    setVisibleParts: (partIndicies: number[]) => DocumentMethodPromise<undefined>,
    /**
     *  Loads an entire MusicXML score into the embed client. Usage Restrictions: This function is only available to communities that are enabled for content injection into Noteflight embeds.
    ...
    /**
     * Loads an entire NoteflightXML score into the embed client. Usage Restrictions: This function is only available to communities that are enabled for content injection into Noteflight embeds.
    ...
    /**
     * Pastes a sequence of note sets into the document at a given staff, measure and metrical offset. Notes will be automatically split and tied as needed to fit the metrical structure of the score, which will also extend to accommodate the pasted sequence. Accidentals will be adjusted automatically.
     * @param noteSets An array of note set objects, where each note set's position property is taken as relative to the starting musical position at which notes are to be pasted.
    ...
    /**
     * Deletes any selected objects, staves or measures from the score. Note that attempting to delete the entire score will retain a single empty measure at the start of the score.
     */
     ...
     * @returns A JavaScript Array of objects with a key signature property structure. Additionally an optional alternateKey property may be present, pointing to an alternate, enharmonic key signature that may be obtained by setting the alternateKey option when calling setTransposeParts().
     */
    getTranspositions: () => DocumentMethodPromise<(KeySignature & {alternateKey?: KeySignature})[]>,
    /**
     ...
     * @returns A float indicating the current playback speed adjustment, as log2(k) where k is the speed scaling factor. Thus 0 indicates no speed adjustment, -1 is 50%, and +1 is 200%.
     */
    ...
    /**
     *  Sets a background image for the score. If smaller than the score, the image will be tiled. Note: in order to load an image from your a domain, the domain must allow cross-domain image data by serving a crossdomain.xml from its root. See Adobe's documentation for more detail.
     ...
     * @returns A string: "normal" for internal synthesis, "audioTrack" for a synchronized track, "silent" for no playback, or "echo" for playing selection only.
     */
