...
const columns_d = dt.cells(':contains("21")').indexes().pluck('column').sort().unique();
...
alert('Column index 0 is ' + (dt.column(0).visible() ? 'visible' : 'not visible'));
for (let i = 0; i < 4; i++) {
...
dt.row(':eq(0)').child(['First child row', 'Second child row', 'Third child row']).show();
...
const defaults_2: DataTables.Settings = {
    dom: "<'row'<'col-lg-5'f><'col-lg-7'Bl>>" + "<'row'<'col-lg-12't>>" + "<'row'<'col-lg-5'i><'col-lg-7'p>>Ox",
    paging: true,
