...
async function run() {
    ...
    const cursor1: AggregationCursor<EmployeeName> = collection.aggregate<Employee>().project<EmployeeName>({
    ...
    const cursor2: AggregationCursor<DepartmentSummary> = collection.aggregate<Employee>().group<DepartmentSummary>({
        _id: '$department',
        count: { $sum: 1 },
    });
}
