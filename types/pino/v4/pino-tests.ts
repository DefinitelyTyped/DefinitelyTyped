...
const useFormatter = true;
const formatterFunc = (
    log: pino.LogDescriptor,
    options: pino.UtilityFunctionOption
) => {
    return `${log.hostname}-${options.chalk.blue("test")}`;
};
const loggerWithFormatter: pino.Logger = pino({
    level: "trace",
    prettyPrint: useFormatter ? { formatter: formatterFunc } : false
});
loggerWithFormatter.info("withFormatter...");
