// cspell: ignore recalc, Boberson, Jimmerson, Jillerson, hayley, Betterson,
// freetext, datetime

import {CalculationComponent, CellComponent, ColumnComponent, ColumnDefinition, ColumnDefinitionSorterParams, DataTreeModule, GroupComponent, InputParams, JSONRecord, ListEditorParams, MenuObject, MenuSeparator, Module, NumberParams, Options, type RangeComponent, Renderer, RowComponent, SortDirection, SorterFromTable, Tabulator, TabulatorFull, TextAreaParams, TooltipModule, Validator,} from 'tabulator-tables';
...
table.setFilter(
    (data, filterParams) => {
      ...
      return data.name === 'bob' &&
          data.height <
          filterParams
              .height;  // must return a boolean, true if it passes the filter.
    },
    {height: 3},
);
...
colDef.editor = (cell, onRendered, success, cancel, editorParams) => {
  ...
  // success - function to call to pass the successfully updated value to
  // Tabulator cancel - function to call to abort the edit and return to a
  // normal cell editorParams - params object passed into the editorParams
  ...
  // set focus on the select box when the editor is selected (timeout allows for
  // editor to be added to DOM)
  onRendered(() => {
    editor.focus();
...
colDef.headerFilterFunc = (headerValue, rowValue, rowData, filterParams) => {
  return rowData.name === filterParams.name &&
      rowValue <
      headerValue;  // must return a boolean, true if it passes the filter.
};
...
colDef.tooltip =
    (event: MouseEvent, cell: CellComponent,
...
options.ajaxConfig = {
  ...
  credentials:
      'same-origin',  // send cookies with the request from the matching origin
  headers: {
    Accept: 'application/json',            // tell the server we need JSON back
    'X-Requested-With': 'XMLHttpRequest',  // fix to help some frameworks
                                           // respond correctly to request
    'Content-type':
        'application/json; charset=utf-8',  // set the character encoding of the
                                            // request
    'Access-Control-Allow-Origin':
        'http:// you-site.com',  // the URL origin of the site making the
                                 // request
  },
};
options.ajaxConfig = {
  ...
  headers: {
    'Content-type':
        'application/json; charset=utf-8',  // set specific content type
  },
};
...
options.groupValues = [
  [
    'red', 'blue', 'green'
  ],  // create groups for color values of "red", "blue", and "green",
...
options.groupHeader = [
  ...
  (value, count, data) => {
    ...
    return `${value} <span style='color:#0dd; margin-left:10px;'>(${
        count}item)</span>`;
  },
];
...
table = new Tabulator('#example-table', {
  persistence: {
    columns: [
      'width', 'visible', 'frozen'
    ],  // persist changes to the width, visible and frozen properties
  },
});
...
table.updateColumnDefinition('age', {title: 'Updated Title', frozen: true})
    .then(
        () => {
            // run code after column has been scrolled to
        })
...
row.addTreeChild(
    {name: 'Billy Bob', age: '12', gender: 'male', height: 1}, true);
column.isVisible();
...
table = new Tabulator('#test', {
  columns: [
    {
      ...
      editorParams: {
        ...
        format:
            'dd/MM/yyyy',  // the format of the date value stored in the cell
        elementAttributes: {
  ...
  downloadEncoder: (fileContents, mimeType) => {
    ...
    return new Blob([fileContents], {
      type: mimeType
    });  // must return a blob to proceed with the download, return false to
         // abort download
  },
});
...
table = new Tabulator('#testPagination', {
  ...
  pagination: true,
  paginationCounter:
      (pageSize, currentRow, currentPage, totalRows, totalPages) => {
        return `${pageSize}, ${currentRow}, ${currentPage}, ${totalRows}, ${
            totalPages}`;
      },
});
...
table = new Tabulator('#example-table', {
  ...
  rowHeader:
      {field: '_id', hozAlign: 'center', headerSort: false, frozen: true},

  editTriggerEvent:
      'dblclick',  // change edit trigger mode to make cell navigation smoother
  editorEmptyValue:
      undefined,  // ensure empty values are set to undefined so they arent
                  // included in spreadsheet output data
