namespace dataTests {
    ...
    namespace mpakTests {
        ...
        {
            const a: adone.collection.ByteArray = new mpak.Encoder([])
                .encode('hello', new adone.collection.ByteArray(10))
                .flip();
        }
        ...
        {
            const a: adone.data.mpak.Serializer = new mpak.Serializer().registerEncoder(
                10,
                (x) => true,
                (x) => new adone.collection.ByteArray(10),
            );
        }
        {
            const a: adone.data.mpak.Serializer = new mpak.Serializer().registerDecoder(
                10,
                (buf: adone.collection.ByteArray) => null,
            );
        }
        {
            const a: adone.data.mpak.Serializer = new mpak.Serializer().register(
                10,
                Date,
                (x: Date) => x,
                (buf: adone.collection.ByteArray) => null,
    ...
    namespace yamlTests {
        ...
        namespace schemaTests {
            ...
            new schema.Schema({
                implicit: [yaml.type.Int],
            });
            new schema.Schema({
                explicit: [yaml.type.Int],
            });
            {
