namespace adoneTests.collection.LRU {
    ...
    new LRU<string, number>().rforEach(
        function (value: number, key: string) {
            const b: number = this.a;
        },
        { a: 1 },
    );
    new LRU<string, number>().forEach((value: number, key: string) => null);
    new LRU<string, number>().forEach(
        function (value: number, key: string) {
            const b: number = this.a;
        },
        { a: 1 },
    ...
    {
        const a: adone.collection.LinkedList<adone.collection.I.LRU.Entry<string, number>> = new LRU<
            string,
            number
        >().dumpLru();
        const b = a.shift();
