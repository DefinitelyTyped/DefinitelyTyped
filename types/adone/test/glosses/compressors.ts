namespace compressorsTests {
    ...
    namespace gzTests {
        ...
        gz.compress(Buffer.from('hello'), { info: true }).then(
            (x: { buffer: Buffer; engine: nodestd.zlib.Gzip }) => {}
        );
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.Gzip } = gz.compressSync(Buffer.from('hello'), {
                info: true,
            });
        }
        {
        ...
        gz.decompress(Buffer.from('hello'), { info: true }).then(
            (x: { buffer: Buffer; engine: nodestd.zlib.Gunzip }) => {}
        );
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.Gunzip } = gz.decompressSync(Buffer.from('hello'), {
                info: true,
            });
        }
        {
    ...
    namespace deflateTests {
        ...
        deflate
            .compress(Buffer.from('hello'), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.Deflate }) => {});
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.Deflate } = deflate.compressSync(Buffer.from('hello'), {
                info: true,
            });
        }
        {
        ...
        deflate
            .rawCompress(Buffer.from('hello'), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.DeflateRaw }) => {});
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.DeflateRaw } = deflate.rawCompressSync(
                Buffer.from('hello'),
                { info: true }
            );
        }
        ...
        deflate
            .decompress(Buffer.from('hello'), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.Inflate }) => {});
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.Inflate } = deflate.decompressSync(Buffer.from('hello'), {
                info: true,
            });
        }
        {
        ...
        deflate
            .rawDecompress(Buffer.from('hello'), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.InflateRaw }) => {});
        ...
        {
            const a: { buffer: Buffer; engine: nodestd.zlib.Inflate } = deflate.rawDecompressSync(
                Buffer.from('hello'),
                { info: true }
            );
        }
    ...
    namespace lzmaTests {
        ...
        lzma.singleStringCoding(lzma.createStream('streamDecoder'), 'hello').then((x: Buffer) => {});

        lzma.singleStringCoding(lzma.createStream('streamDecoder'), Buffer.from('hello')).then((x: Buffer) => {});

        lzma.singleStringCoding(lzma.createStream('streamDecoder'), 'hello', (err: any, data: Buffer) => {}).then(
            (x: Buffer) => {}
        );
    ...
    namespace xzTests {
        ...
        xz.singleStringCoding(xz.createStream('streamDecoder'), 'hello').then((x: Buffer) => {});

        xz.singleStringCoding(xz.createStream('streamDecoder'), Buffer.from('hello')).then((x: Buffer) => {});

        xz.singleStringCoding(xz.createStream('streamDecoder'), 'hello', (err: any, data: Buffer) => {}).then(
            (x: Buffer) => {}
        );
