...
module DXDemo {
    ...
    export function onIndexOfferCloseClick(index: number): void {
        ...
        var sibPanel = <ASPxClientDockPanel>(
            ASPxClientControl.GetControlCollection().GetByName('OfferDockPanel' + (index == 1 ? 2 : 1))
        );
        panel.Hide();
        sibPanel.MakeFloat();
    ...
    export function onBackNavButtonClick(s: ASPxClientButton, e: ASPxClientButtonClickEventArgs): void {
        ...
        switch (getCurrentPage()) {
            ...
            case 'Booking':
                ...
                else showPage('ShowRooms', params, false);
                break;
            case 'ShowRooms':
    ...
    export function onNextBookingStepButtonClick(step: number): void {
        var valid = true;
        var validationGroup = '';
        if (step == 1) validationGroup = 'Account';
        if (step == 2) validationGroup = 'RoomDetails';
        if (step == 3) validationGroup = 'PaymentDetails';
    ...
    export function onRatingControlItemClick(
        s: ASPxClientRatingControl,
        e: ASPxClientRatingControlItemClickEventArgs,
    ): void {
    ...
    export function onInputKeyDown(s: ASPxClientTextBox, e: ASPxClientEditKeyEventArgs): void {
        ...
        if (keyCode == 13) (<HTMLElement>s.GetInputElement()).blur();
    }

    function getCurrentPage(): string {
        var hfPage = <string>hiddenField.Get('page');
        if (hfPage) return hfPage;
    ...
    function hideMenu(): void {
        leftPanel.Collapse();
        if (menuButton.GetMainElement() && menuButton.GetChecked()) menuButton.SetChecked(false);
    }
    ...
    function getCurrentQueryParams(): { [key: string]: any } {
        ...
        if (hfParams) return getParamsByQueryString(hfParams);
    ...
    function getQueryString(params: { [key: string]: any }): string {
        ...
        if (queryItems.length > 0) return queryItems.join('&');
        return '';
    }
