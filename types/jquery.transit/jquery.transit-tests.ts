...
function test_opacity() {
    ...
    TestObject.transition({
        ...
        duration: 1,
        complete: function() {
            Assert.Equal(TestObject.attr('style'), 'opacity: 75;', 'Opacity transition test');
        },
    });
}
...
function test_scale() {
    ...
    TestObject.transition({
        ...
        duration: 1,
        complete: function() {
            Assert.Equal(TestObject.attr('style'), 'transform: scale(2, 2);', 'Scale transition test');
        },
    });
    ...
    TestObject.transition({
        ...
        duration: 1,
        complete: function() {
            Assert.Equal(TestObject.attr('style'), 'transform: scale(2, 3);', 'Scale transition test');
        },
    });
}
...
function test_duration() {
    ...
    TestObject.transition({
        ...
        duration: 1000,
        complete: function() {
            Assert.Equal(TestObject.attr('style'), 'opacity: 75;', 'Duration post-transition test');
        },
    });
    // Test the transitions state partway through and assert that we're not to our final state yet.
    setTimeout(function() {
        Assert.NotEqual(TestObject.attr('style'), 'opacity: 75;', 'Duration intra-transition test');
    }, 300);
}
