...
export default class Http {
   ...
   * @return  {Promise}
   */
  static xmlHttpRequest(method, url, data): Promise<any> {
   ...
   * @return  {Promise}
   */
  static requestPromise(method: string, url: string, data: Record<string, any>, files: Record<string, any>, useMultipartFormData: Boolean = false, showHeader: Boolean = false): Promise<any> {
    const options = {
      method: method,
      url: url,
      baseURL: Api.GRAPH,
      ...
      headers: {
        'User-Agent': `fbbizsdk-nodejs-v${Api.SDK_VERSION}`
      },
      data: Object,
    ...
    return axios(options).catch((response: Record<string, any>) => {
      throw response;
    });
  }
