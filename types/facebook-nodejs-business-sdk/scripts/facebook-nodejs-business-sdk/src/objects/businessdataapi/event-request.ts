...
export default class EventRequest {
   ...
   */
  constructor(access_token: string, page_id: string, events: Array<Event> = [], partner_agent: string | null | undefined = null) {
  ...
  execute(): Promise<EventResponse> {
    ...
    const pagePromise = new Page(this._page_id).createBusinessDatum([], params);
    return pagePromise.then(response => {
      return new EventResponse(response._data['events_received'], response._data['events_dropped'], response._data['message']);
    });
  }
