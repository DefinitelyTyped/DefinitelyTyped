...
export default class EventRequest {
   ...
   */
  constructor(access_token: string, pixel_id: string, page_id: string, events: Array<SignalEvent> = [], partner_agent: string | null | undefined = null, test_event_code: string | null | undefined = null, namespace_id: string | null | undefined = null, upload_id: string | null | undefined = null, upload_tag: string | null | undefined = null, upload_source: string | null | undefined = null, debug_mode_flag: boolean = false, http_service: HttpServiceInterface | null | undefined = null) {
    const business_data_events = events.map(event => event.business_data_event);
    this._business_data_event_request = new BusinessDataEventRequest(access_token, page_id, business_data_events, partner_agent);
    const server_events = events.map(event => event.server_event);
    this._server_event_request = new ServerEventRequest(access_token, pixel_id, server_events, partner_agent, test_event_code, namespace_id, upload_id, upload_tag, upload_source, debug_mode_flag, http_service);
  }
  ...
  execute(): Promise<Record<string, any>> {
    return Promise.all([this._server_event_request.execute().catch(Error), this._business_data_event_request.execute().catch(Error)]).then(response => {
      return Utils.constructResponse(response[0], response[1]);
    });
  }
