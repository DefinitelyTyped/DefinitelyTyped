...
class VideoUploadStartRequestManager extends VideoUploadRequestManager {
   ...
   **/
  async sendRequest(context: VideoUploadRequestContext): Record<string, any> {
...
class VideoUploadRequest {
  ...
  send(path: string | Array<string>): Record<string, any> {
    return new Promise((resolve, reject) => {
      this._api.call('POST', path, this._params, this._files, true, // use multipart/form-data
      FacebookAdsApi.GRAPH_VIDEO // override graph.facebook.com
      ).then(response => resolve(JSON.parse(response))).catch(error => reject(error));
    });
  }

  setParams(params: Record<string, any>, files: Record<string, any> = null) {
    this._params = params;
    this._files = files;
  }
...
class VideoEncodingStatusChecker {
  static async waitUntilReady(api: FacebookAdsApi, videoId: number, interval: number, timeout: number): Promise<void> {
