import * as OpenTimestamps from "opentimestamps";
const fileOts = Buffer.from(
    "",
    "hex",
);
const detached = OpenTimestamps.DetachedTimestampFile.deserialize(fileOts); // $ExpectType DetachedTimestampFile
const infoResult = OpenTimestamps.info(detached); // $ExpectType string
