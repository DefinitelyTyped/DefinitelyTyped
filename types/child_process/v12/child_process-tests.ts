...
import { Pipe, Readable, ReadableStream, Writable, WritableStream } from 'stream';
...

{
    ...
    cp = cp.addListener("close", (code, signal) => {
        ...
        const _signal: process.Signals = signal;
    });
    ...
    cp = cp.addListener("exit", (code, signal) => {
        ...
        const _signal: process.Signals | null  = signal;
    });
...
}
{
    ...
    const stdin: ReadableStream = process.stdin;
    console.log(stdin instanceof net.Socket);
    console.log(stdin instanceof fs.ReadStream);

    const stdout: WritableStream = process.stdout;
