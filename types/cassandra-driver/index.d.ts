...
export namespace types {
    ...
    function timeuuid(
        options?: { msecs: number | Date; node: Buffer; clockseq: number; nsecs: number },
        buffer?: Buffer,
        offset?: number
    ): string;
...
export interface Client extends events.EventEmitter {
    ...
    batch(
        queries: string[] | Array<{ query: string; params?: any }>,
...
export namespace mapping {
    ...
    interface ResultStatic {
        new (
            ...
            rowAdapter: (row: types.Row, info: ModelMappingInfo) => { [key: string]: any }
        ): Result;
    }
    interface Result {
