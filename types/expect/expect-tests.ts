...
describe('expect.extend', () => {
    const ColorAssertions: Extension = {
        toBeAColor() {
            assert(this.actual.match(/^#[a-fA-F0-9]{6}$/), 'expected %s to be an HTML color', this.actual);
        },
    };
...
describe('restoreSpies', () => {
    ...
    describe('with multiple spies', () => {
        ...
        const targets = [{ method: originals[0] }, { method: originals[1] }];
...
describe('toInclude', () => {
    ...
    it('does not throw when an object contains an expected object in a deep key', () => {
        expect(() => {
            expect({ a: 1, b: 2, c: { d: 4, e: { f: 5, g: 6, h: 7 } } }).toInclude({
                b: 2,
                c: { e: { f: 5, g: 6, h: 7 } },
            });
        }).toNotThrow();
...
describe('expect(array).toMatch', () => {
    it('does not throw when the array contains an object that matches the pattern', () => {
        const array = [{ one: 'one' }, { two: 'two' }, { three: 'three' }];
...
describe('withContext', () => {
    ...
    const fn = function fn(arg: any) {
        if (this.check && typeof arg === 'undefined') throw new Error('context found');

        if (this.check && arg === 'good') throw new Error('context and args found');
    };
    ...
    it('calls function with context and args', () => {
        expect(() => {
            expect(fn)
                .withContext(context)
                .withArgs('good')
                .toThrow(/context and args found/);
        }).toNotThrow();
    });
