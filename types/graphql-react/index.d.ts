...
export type GraphQLFetchOptionsOverride = (options: GraphQLFetchOptions) => void;
...
export class GraphQL {
    ...
    on(type: 'reset', handler: (event: { exceptCacheKey: GraphQLCacheKey }) => void): void;

    on(
        type: 'cache',
        handler: (event: { cacheKey: GraphQLCacheKey; cacheValue: GraphQLCacheValue<any> }) => void,
    ): void;

    on(
        type: 'fetch',
        handler: (event: { cacheKey: GraphQLCacheKey; cacheValuePromise: Promise<GraphQLCacheValue<any>> }) => void,
    ): void;

    off(type: 'reset', handler: (event: { exceptCacheKey: GraphQLCacheKey }) => void): void;
...
export function ssr(grapphql: GraphQL, node: ReactNode, render?: (element: ReactNode) => string): void;
