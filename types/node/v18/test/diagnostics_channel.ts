...
import { Channel, channel, hasSubscribers, subscribe, unsubscribe, tracingChannel } from "node:diagnostics_channel";
...

{
    ...
    channels.traceSync(function (a, b) {
        this; // $ExpectType string
        a; // $ExpectType number
        b; // $ExpectType string
        return 'something'
    }, { requestId: 42 }, thisArg, a, b);

    channels.traceSync(function () {
        return 'something'
    }, { requestId: 42 });

    channels.tracePromise(async function (a, b) {
        this; // $ExpectType string
        a; // $ExpectType number
        b; // $ExpectType string
        return 'something'
    ...
    channels.traceCallback(function (arg1, callback) {
        callback(null, 'result');
    }, 1, {
        requestId: 42,
    }, 'thisArg', 42, callback);

    channels.traceCallback(function (callback) {
        callback(null, 'result');
    }, undefined, {
        requestId: 42,
    }, undefined, callback);
