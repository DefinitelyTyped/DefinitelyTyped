...
const createTypeDefinition = (type: schema.Type, domain: string): string[] => {
    return [
        ...
        ...(type.type === 'object'
            ? [
                  `interface ${type.id} {`,
                  ...createFieldsForInterface(type.properties || [], domain).map(line => `${INDENT}${line}`),
                  '}',
              ]
...
const createPostFunctions = (command: schema.Command, domain: string): string[] => {
    ...
    result.push(
        [`${fnName}(`, `method: "${domain}.${command.name}", `, `callback?: ${callbackStr}`, '): void;'].join('')
    );
    return result;
};
...
export const generateSubstituteArgs = (protocol: schema.Schema): { [propName: string]: string[] } => {
    ...
    const postOverloads: string[] = protocol.domains
        .map(item =>
            (item.commands || []).map(command => createPostFunctions(command, item.domain)).reduce(flattenArgs(''), [])
        )
        .reduce(flattenArgs(''), []);
    ...
    const eventOverloads: string[] = createListeners(
        protocol.domains
            ...
            .reduce((acc, next) => acc.concat(next), [
                {
                    comment: ['/**', ' * Emitted when any notification from the V8 Inspector is received.', ' */'],
