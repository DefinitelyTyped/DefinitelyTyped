...
function testBulkDocs() {
    ...
    const isError = (result: PouchDB.Core.Response | PouchDB.Core.Error): result is PouchDB.Core.Error => {
...
function heterogeneousGenericsDatabase(db: PouchDB.Database) {
    ...
    db.allDocs<Cat>({ startkey: 'cat/', endkey: 'cat/\uffff', include_docs: true }).then((cats) => {
    ...
    db.allDocs<Boot>({ startkey: 'boot/', endkey: 'boot/\uffff', include_docs: true }).then((boots) => {
