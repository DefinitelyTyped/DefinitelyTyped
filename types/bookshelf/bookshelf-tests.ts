...
exports.down = (knex: Knex) => {
    return knex.schema.dropTable('books').dropTable('pages');
};
...
new User({ id: 1 }).destroy().then((model) => {
    // ...
});
...
new Book({ 'ISBN-13': '9780440180296' }).fetch().then((model) => {
...
});
{
    ...
    new Book({ 'ISBN-13': '9780440180296' })
        .fetch({
            withRelated: ['genre', 'editions', { chapters: (query) => query.orderBy('chapter_number') }],
        })
        .then((book) => {
...
new Author({ id: 1, first_name: 'User' }).save({ bio: 'Short user bio' }, { patch: true }).then((model) => {
    // ...
});

// Save with no arguments
bookshelf.Model.forge<Author>({ id: 5, firstName: 'John', lastName: 'Smith' })
    .save()
    .then(() => {
        //...
    });
...
bookshelf.Model.forge<Author>({ id: 5 })
    .save({ firstName: 'John', lastName: 'Smith' })
    .then(() => {
        //...
    });
...
// or
model
    .where('favorite_color', 'in', ['red', 'green'])
    .fetch()
    .then(() => {
        // ...
    });
...
var accounts = Accounts.forge<Accounts>([{ name: 'Person1' }, { name: 'Person2' }]);
...

{
    ...
    Promise.all([admin1.save(), admin2.save()]).then(() => {
        return Promise.all([
...

{
    ...
    new Author({ id: 1 }).fetch({ require: true, withRelated: ['books'] }).then((author) => {
