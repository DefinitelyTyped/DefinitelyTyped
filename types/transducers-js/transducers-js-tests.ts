...
function transduceExample() {
    ...
    const stringAppendThenLengthTransformer = completing(stringAppendFn, s => s.length);
    const lengthsString1: string = transduce(t.map((s: string) => s.length), stringAppendFn, '', ['a', 'b']);
    const lengthsString2: string = transduce(t.map((s: string) => s.length), stringAppendTransformer, '', ['a', 'b']);
...
function advancedIntoExample() {
    const array: number[] = into([], t.map((s: string) => s.length), ['a', 'b']);
    const string: string = into('', t.map((s: string) => s + s), ['a', 'b']);
    const object1: { [key: string]: number } = into({}, t.map((s: string) => [s, s.length] as [string, number]), [
        'a',
        'b',
    ]);
...
function compExample() {
    ...
    const fn2: t.Transducer<number, string> = comp(filter(isEven), map((x: number) => '' + x));
}
