...
class PlatformInfo {
    ...
    constructor(public platform: WEBCL.WebCLPlatform, public deviceInfos: DeviceInfo[] = new Array<DeviceInfo>()) {
...
class KernelWorkGroupInfo {
    ...
    constructor(kernel: WEBCL.WebCLKernel, device: WEBCL.WebCLDevice) {
        this.KERNEL_COMPILE_WORK_GROUP_SIZE = kernel.getWorkGroupInfo(
            device,
...
class ArgCLBuffer {
    ...
    makeCLBuffer(context: DeviceContext): void {
        ...
        this.buffer = context.context.createBuffer(
            rwflag,
            this.hostArray.length * this.hostArray.BYTES_PER_ELEMENT,
            this.hostArray,
        ); // make the CLBuffer for the host array
    }

    queueGPU2CPU() {
        if (this.gpu2cpu) {
            this.helper.queue.enqueueReadBuffer(
                this.buffer,
                false,
                0,
                this.hostArray.length * this.hostArray.BYTES_PER_ELEMENT,
                this.hostArray,
            ); // queue up a write from the host mem to the GPU mem
        }
    }

    queueCPU2GPU() {
        if (this.cpu2gpu) {
            this.helper.queue.enqueueWriteBuffer(
                this.buffer,
                false,
                0,
                this.hostArray.length * this.hostArray.BYTES_PER_ELEMENT,
...
class Kernel {
    ...
    queueExecution() {
        ...
        this.helper.queue.enqueueNDRangeKernel(
            this.kernel,
...
*/
     ...
     */
    setDeviceContext(
        deviceTypes: WEBCL.DeviceTypeBits[] = [WEBCL.DeviceTypeBits.DEVICE_TYPE_DEFAULT], // optional, if empty default
    ): DeviceContext {
