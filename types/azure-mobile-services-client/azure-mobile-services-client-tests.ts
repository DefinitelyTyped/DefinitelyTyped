...
if (client.currentUser === null) {
    client
        ...
        .done(
            () => alert('USER: ' + client.currentUser.userId),
            (e) => alert('ERROR: ' + e),
        );
} else {
    client.logout();
...
var query = tableTodoItems.select('text', 'id').where({ complete: false }).orderBy('text');
...
query
    .where(function (len?: number) {
        return this.text != null && this.text.length > len;
    }, minlength)
    .orderByDescending('id')
    .skip(2)
    .take(3) //some other ordering and paging filters
    .select(function () {
        return { abc: this.text + '|' + this.id };
    }) //Projection
    .read()
    .done(printOut); //return 3 object {abd: 'ttttttttttttttt|ID'}
