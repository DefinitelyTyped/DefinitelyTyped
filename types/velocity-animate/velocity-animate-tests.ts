function basics_arguments() {
    ...
    $el.velocity({ top: 50 }, 1000, function() {
        alert('Hi');
    });
    $.Velocity.mock = 1;
...
function command_scroll() {
    var $el: JQuery;
    $el.velocity('scroll', { duration: 1500, easing: 'spring' }).velocity({ opacity: 1 });
    ...
    $el.velocity('scroll', { duration: 750, offset: -50 })
...
function command_fadeIn_fadeOut() {
    var $el: JQuery;
    $el.velocity('fadeIn', { duration: 1500 }).velocity('fadeOut', { delay: 500, duration: 1500 });
}

function command_slideDown_slideUp() {
    var $el: JQuery;
    $el.velocity('slideDown', { duration: 1500 }).velocity('slideUp', { delay: 500, duration: 1500 });
}
...
function advanced_forcefeeding() {
    ...
    $el.velocity({ translateX: [500, 0] }).velocity({ translateX: 1000 });
}
...
function ui_pack_registration() {
    ...
    $.Velocity.RegisterEffect('callout.pulse', {
        defaultDuration: 900,
        calls: [[{ scaleX: 1.1 }, 0.5], [{ scaleX: 1 }, 0.5]],
    });
    $element.velocity('callout.pulse');

    $.Velocity.RegisterEffect('transition.flipXIn', {
        defaultDuration: 700,
        calls: [[{ opacity: 1, rotateY: [0, -55] }]],
    }).RegisterEffect('transition.flipXOut', {
        defaultDuration: 700,
        calls: [[{ opacity: 0, rotateY: 55 }]],
        reset: { rotateY: 0 },
    });
    $element.velocity('transition.flipXIn').velocity('transition.flipXOut', { delay: 1000 });
}
