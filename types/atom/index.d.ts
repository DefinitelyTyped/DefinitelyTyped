...
declare namespace AtomCore {
	...
	/** The static side to the Color class. */
	interface ColorStatic {
		/** Parse a string or Object into a Color. */
		parse(value: string|{ red: number, green: number, blue: number, alpha: number }): Color;
	}
	...
	interface Config {
		...
		onDidChange(callback: (values: {
			newValue: any, oldValue: any }) => void): EventKit.Disposable;
		/** Add a listener for changes to a given key path. If keyPath is not specified, your
		 *  callback will be called on changes to any key.
		 */
	...
	/** The static side to the ScopeDescriptor class. */
	interface ScopeDescriptorStatic {
		new (object: { scopes: string[] }): ScopeDescriptor;
	}
	...
	interface TaskStatic {
		...
		// for this appearing in the middle of the parameter list, which isn't aligned
		// with the ES6 spec.
	...
	interface TextEditorStatic {
		new (params?: {
			softTabs?: boolean,
			initialScrollTopRow?: number,
			initialScrollLeftColumn?: number,
			initialLine?: number,
			initialColumn?: number,
			tabLength?: number,
			softWrapped?: boolean,
			decorationManager?: any, // TODO: unimplemented class DecorationManager.
			selectionsMarkerLayer?: TextBuffer.DisplayMarkerLayer,
			...
			tokenizedBuffer?: any, // TODO: unimplemented class TokenizedBuffer.
			displayLayer?: TextBuffer.DisplayMarkerLayer,
			invisibles?: Structures.Invisibles,
			showIndentGuide?: boolean,
			softWrapAtPreferredLineLength?: boolean,
			preferredLineLength?: number,
			showCursorOnSelection?: boolean
		}): TextEditor;
	}
	...
	interface TextEditor extends Model {
		 ...
		 *  ::scan functions as the replace method as well via the replace.
		 */
		scan(regex: RegExp, iterator: (match: object, matchText: string, range: TextBuffer.Range,
		 ...
		 */
		scanInBufferRange(regex: RegExp, range: TextBuffer.RangeLike, iterator: (match: object,
			matchText: string, range: TextBuffer.Range, stop: Function, replace: Function) =>
			void): void;
		/** Scan regular expression matches in a given range, calling the given iterator.
		 *  function on each match.
		 */
		scanInBufferRange(regex: RegExp, range: [TextBuffer.PointLike, TextBuffer.PointLike],
			iterator: (match: object, matchText: string, range: TextBuffer.Range, stop: Function,
		...
		backwardsScanInBufferRange(regex: RegExp, range: TextBuffer.RangeLike, iterator:
			(match: object, matchText: string, range: TextBuffer.Range, stop: Function, replace:
			Function) => void): void;
		/** Scan regular expression matches in a given range in reverse order, calling the
		 *  given iterator function on each match.
		 */
	...
	interface TooltipManager {
		...
		add(target: HTMLElement, options: {
			...
			html?: boolean,
			item?: Structures.View|Element,
			class?: string,
			placement?: "top"|"bottom"|"left"|"right"|"auto"|Function,
	...
	interface ViewRegistry {
		 ...
		 */
		addViewProvider(createView: () => HTMLElement|undefined): EventKit.Disposable;
		/** Add a provider that will be used to construct views in the workspace's view
		 *  layer based on model objects in its model layer.
		 */
		addViewProvider(modelConstructor: Function|object, createView: (instance: object) =>
			HTMLElement|undefined): EventKit.Disposable;
	...
	interface Workspace {
		 ...
		 *  will be created.
		 */
		createItemForURI(uri: string): Promise<object>;
		...
		addOpener(opener: (uri: string, options: object) => any): EventKit.Disposable;
...
declare module "atom" {
	 ...
	 *  Node's ChildProcess.
	 */
	const BufferedProcess: AtomCore.BufferedProcessStatic;

	/** Like BufferedProcess, but accepts a Node script as the command to run.
	 *  This is necessary on Windows since it doesn't support shebang #! lines.
	 */
	const BufferedNodeProcess: AtomCore.BufferedNodeProcessStatic;
	...
	const Directory: PathWatcher.DirectoryStatic;
	...
	const File: PathWatcher.FileStatic;

	/** Represents the underlying git operations performed by Atom. */
	const GitRepository: AtomCore.GitRepositoryStatic;

	/** A notification to the user containing a message and type. */
	const Notification: AtomCore.NotificationStatic;

	/** Represents a point in a buffer in row/column coordinates. */
	const Point: TextBuffer.PointStatic;

	/** Represents a region in a buffer in row/column coordinates. */
	const Range: TextBuffer.RangeStatic;

	/** Run a node script in a separate process. */
	const Task: AtomCore.TaskStatic;

	/** A mutable text container with undo/redo support and the ability to annotate
	 *  logical regions in the text.
	 */
	const TextBuffer: TextBuffer.TextBufferStatic;

	/** This class represents all essential editing state for a single TextBuffer,
	 *  including cursor and selection positions, folds, and soft wraps.
	 */
	const TextEditor: AtomCore.TextEditorStatic;

	/** Invoke a callback with each filesystem event that occurs beneath a specified
	 *  path. If you only need to watch events within the project's root paths, use
	 *  Project::onDidChangeFiles instead.
	 */
	// export function watchPath(rootPath, options, eventCallback):
		// Promise<AtomCore.PathWatcher>
}
