...
export interface BufferWatchErrorEvent {
    /** The error object. */
    error: Error;
...
export interface DecorationLayerProps extends SharedDecorationProps {
    /** One of several supported decoration types. */
    type?: "line"|"line-number"|"highlight"|"block";
}

export interface DecorationProps extends SharedDecorationProps {
    /** One of several supported decoration types. */
...
export interface SharedDecorationProps {
...
/** An object that is capable of being disposed of. */
...
export type RangeCompatible = RangeLike
    |[PointLike, PointLike]
...
export type TestRunner = (params: TestRunnerArgs) => Promise<number>;
...
export interface AtomEnvironment {
     ...
     */
    onWillThrowError(callback: (event: PreventableExceptionThrownEvent) =>
        void): Disposable;
...
export interface CommandRegistry {
    ...
    add(target: string|Node, commands: { [key: string]: (event:
        CommandEvent) => void }): CompositeDisposable;
...
export interface ContextMenuManager {
    ...
    add(itemsBySelector: {
        [key: string]: ReadonlyArray<ContextMenuOptions>
    }): Disposable;
}
...
export interface Cursor {
    ...
    onDidChangePosition(callback: (event: CursorPositionChangedEvent) => void):
        Disposable;
...
export interface Decoration {
    ...
    onDidChangeProperties(callback: (event: DecorationPropsChangedEvent) => void):
        Disposable;
...
export interface DisplayMarker {
    ...
    setHeadScreenPosition(screenPosition: PointCompatible, options?:
        { clipDirection: "backward"|"forward"|"closest" }): void;
    ...
    setTailScreenPosition(screenPosition: PointCompatible, options?:
        { clipDirection: "backward"|"forward"|"closest" }): void;
...
export interface DisplayMarkerLayer {
    ...
    markScreenRange(range: RangeCompatible, options?: { reversed?: boolean, invalidate?:
    ...
    markBufferRange(range: RangeCompatible, options?: { reversed?: boolean, invalidate?:
...
export interface Dock {
    ...
    onDidAddPaneItem(callback: (event: PaneItemObservedEvent) => void):
        Disposable;

    /** Invoke the given callback when a pane item is about to be destroyed, before the user is
     *  prompted to save it.
     */
    onWillDestroyPaneItem(callback: (event: PaneItemObservedEvent) => void):
        Disposable;

    /** Invoke the given callback when a pane item is destroyed. */
    onDidDestroyPaneItem(callback: (event: PaneItemObservedEvent) => void):
        Disposable;
...
export class GitRepository {
     ...
     */
    onDidChangeStatus(callback: (event: RepoStatusChangedEvent) => void):
      Disposable;
...
export interface Gutter {
     ...
     *  the marker's state.
     */
    decorateMarker(marker: DisplayMarker, decorationParams: DecorationProps):
        Decoration;
}
...
export interface KeymapManager {
    ...
    onDidMatchBinding(callback: (event: FullKeybindingMatchEvent) => void):
        Disposable;
    ...
    onDidReloadKeymap(callback: (event: KeymapLoadedEvent) => void):
        Disposable;

    /** Invoke the given callback when a keymap file is unloaded. */
    onDidUnloadKeymap(callback: (event: KeymapLoadedEvent) => void):
        Disposable;

    /** Invoke the given callback when a keymap file not able to be loaded. */
    onDidFailToReadFile(callback: (error: FailedKeymapFileReadEvent) => void):
        Disposable;
    ...
    addKeystrokeResolver(resolver: (event: AddedKeystrokeResolverEvent) => string):
        Disposable;
...
export interface LayerDecoration {
    ...
    getProperties(): DecorationLayerProps;

    /** Set this decoration's properties. */
    setProperties(newProperties: DecorationLayerProps): void;

    /** Override the decoration properties for a specific marker. */
    setPropertiesForMarker(marker: DisplayMarker|Marker, properties:
        DecorationLayerProps): void;
}
...
export interface Marker {
     ...
     */
    setRange(range: RangeCompatible, params?: { reversed?: boolean, exclusive?: boolean }):
        boolean;
...
export interface MarkerLayer {
    ...
    markPosition(position: PointCompatible, options?: { invalidate?:
        "never"|"surround"|"overlap"|"inside"|"touch", exclusive?: boolean }): Marker;
...
export class Notification {
    ...
    constructor(type: "warning"|"info"|"success", message: string, options?:
        NotificationOptions);
...
export interface Pane {
    ...
    onDidAddItem(callback: (event: PaneListItemShiftedEvent) => void):
        Disposable;

    /** Invoke the given callback when an item is removed from the pane. */
    onDidRemoveItem(callback: (event: PaneListItemShiftedEvent) => void):
        Disposable;

    /** Invoke the given callback before an item is removed from the pane. */
    onWillRemoveItem(callback: (event: PaneListItemShiftedEvent) => void):
        Disposable;

    /** Invoke the given callback when an item is moved within the pane. */
    onDidMoveItem(callback: (event: PaneItemMovedEvent) => void):
        Disposable;
    ...
    onWillDestroyItem(callback: (event: PaneListItemShiftedEvent) => void):
        Disposable;
...
export interface Selection {
    ...
    onDidChangeRange(callback: (event: SelectionChangedEvent) => void):
        Disposable;
    ...
    setScreenRange(screenRange: RangeCompatible, options?: { preserveFolds?: boolean,
    ...
    setBufferRange(bufferRange: RangeCompatible, options?: { preserveFolds?: boolean,
        autoscroll?: boolean }): void;
...
export class TextBuffer {
    ...
    onWillThrowWatchError(callback: (errorObject: BufferWatchErrorEvent) => void):
        Disposable;
    ...
    setTextInRange(range: RangeCompatible, text: string, options?: { normalizeLineEndings?:
        boolean, undo?: "skip" }): Range;
    ...
    markRange(range: RangeCompatible, properties?: { reversed?: boolean, invalidate?:
        "never"|"surround"|"overlap"|"inside"|"touch", exclusive?: boolean }): Marker;
     ...
     */
    backwardsScan(regex: RegExp, iterator: (params: BufferScanResult) => void):
        void;
    /** Scan regular expression matches in the entire buffer in reverse order,
     *  calling the given iterator function on each match.
     */
     ...
     *  iterator function on each match.
     */
    scanInRange(regex: RegExp, range: RangeCompatible, options: ScanContextOptions, iterator:
        (params: ContextualBufferScanResult) => void): void;
...
export class TextEditor {
    ...
    insertText(text: string, options?: { select?: boolean, autoIndent?: boolean,
        ...
        normalizeLineEndings?: boolean, undo?: "skip" }): Range|boolean;
    ...
    decorateMarkerLayer(markerLayer: MarkerLayer|DisplayMarkerLayer,
        decorationParams: DecorationLayerProps): LayerDecoration;

    /** Get all decorations. */
    getDecorations(propertyFilter?: DecorationProps): Decoration[];

    /** Get all decorations of type 'line'. */
    getLineDecorations(propertyFilter?: DecorationProps): Decoration[];

    /** Get all decorations of type 'line-number'. */
    getLineNumberDecorations(propertyFilter?: DecorationProps): Decoration[];

    /** Get all decorations of type 'highlight'. */
    getHighlightDecorations(propertyFilter?: DecorationProps): Decoration[];

    /** Get all decorations of type 'overlay'. */
    getOverlayDecorations(propertyFilter?: DecorationProps): Decoration[];
     ...
     */
    markBufferRange(range: RangeCompatible, properties?: { maintainHistory?: boolean,
     ...
     *  the buffer changes.
     */
    markScreenRange(range: RangeCompatible, properties?: { maintainHistory?: boolean,
        reversed?: boolean, invalidate?: "never"|"surround"|"overlap"|"inside"|"touch" }):
        DisplayMarker;
     ...
     */
    setSelectedBufferRange(bufferRange: RangeCompatible, options?: { reversed?: boolean,
        preserveFolds?: boolean}): void;
     ...
     *  ::scan functions as the replace method as well via the replace.
     */
    scan(regex: RegExp, iterator: (params: BufferScanResult) => void):
        void;
     ...
     */
    backwardsScanInBufferRange(regex: RegExp, range: RangeCompatible, iterator:
        (params: BufferScanResult) => void): void;
...
export interface Workspace {
    ...
    onDidAddTextEditor(callback: (event: TextEditorObservedEvent) => void):
        Disposable;
