...
declare let tooltips: AtomCore.Tooltip[];
...
// .views
sub = atom.views.addViewProvider(TextEditor, (textEditor): HTMLElement => {
	return div;
});
...
displayMarker = displayMarker.copy({});
displayMarker = displayMarker.copy({ type: null });
...
bool = displayMarker.matchesProperties(obj);
...
displayMarkers = displayMarkerLayer.findMarkers({ containsBufferRange: range });
...
declare let decorationLayerProps: AtomCore.Structures.DecorationLayerProps;
...
pane.activateItem(obj);
pane.activateItem(obj, { pending: true });

obj = pane.addItem(obj);
obj = pane.addItem(obj, {});
obj = pane.addItem(obj, { pending: true });
obj = pane.addItem(obj, { index: 42 });
obj = pane.addItem(obj, { pending: true, index: 42 });

objs = pane.addItems(objs);
objs = pane.addItems(objs, 42);

pane.moveItem(obj, 42);
pane.moveItemToPane(obj, pane, 42);
pane.destroyActiveItem();

pane.destroyItem(obj);
pane.destroyItem(obj, true);
...
pane = pane.splitLeft({ items: objs });
pane = pane.splitLeft({ copyActiveItem: true, items: objs });

pane = pane.splitRight();
pane = pane.splitRight({});
pane = pane.splitRight({ copyActiveItem: true });
pane = pane.splitRight({ items: objs });
pane = pane.splitRight({ copyActiveItem: true, items: objs });

pane = pane.splitUp();
pane = pane.splitUp({});
pane = pane.splitUp({ copyActiveItem: true });
pane = pane.splitUp({ items: objs });
pane = pane.splitUp({ copyActiveItem: true, items: objs });

pane = pane.splitDown();
pane = pane.splitDown({});
pane = pane.splitDown({ copyActiveItem: true });
pane = pane.splitDown({ items: objs });
pane = pane.splitDown({ copyActiveItem: true, items: objs });
...
//// Task =====================================================================
let task: AtomCore.Task = Task.once("File.path", {}, () => {});
...
bool = editor.selectionIntersectsBufferRange(range);

// Searching and Replacing
editor.scan(/r/, (match, matchText, range, stop, replace) => { range.start &&
	stop() && replace(); });
editor.scan(/r/, {}, () => {});
...
editor.backwardsScanInBufferRange(/r/, range, (match, matchText, range, stop,
	replace) => { range.start && stop() && replace; });
...
//// ViewRegistry =============================================================
atom.views.addViewProvider(obj, () => element);
...
bool = atom.workspace.hide(obj);

atom.workspace.toggle("https://test");
atom.workspace.toggle(obj);
