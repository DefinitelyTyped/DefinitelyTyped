...
new Nightmare().goto('http://www.google.com/').click('a').back().forward().run(done);
...
new Nightmare().goto('http://yahoo.com').screenshot('test/test.png', { x: 10, y: 5, width: 10, height: 10 }).run(done);
...
new Nightmare().goto('http://www.whattimeisit.com/').wait(seconds, 1, 1500).run(done);
...
function search(term: string) {
    return function (nightmare: Nightmare) {
        nightmare.goto('http://yahoo.com').type('.input-query', term).click('.searchsubmit').wait();
    };
}
...
new Nightmare({ waitTimeout: 1000 }).goto('https//google.com').wait('.really-not-real');

new Nightmare({ gotoTimeout: 10000 }).goto('https://45.56.178.93/');

new Nightmare({ executionTimeout: 1000 }).goto('https//google.com').evaluate(() => {
