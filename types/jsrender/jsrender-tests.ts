...
(function() {
    ...
    assert.ok(
        html === 'Jo ',
        '$.templates("myTmpl", "#myTmpl"); $.render.myTmpl(data)',
        'Named template as expando on $.render'
    );
    ...
    assert.ok(
        html === 'Jo ',
        '$.templates("myTmpl", "#myTmpl"); $.templates.myTmpl(data)',
        'Named template as expando on $.templates'
    );
    ...
    assert.ok(
        tmpl.tmplName + tmpl.markup + tmpl.useViews === 'tmplFromString{{:~title}} {{:name}}. true',
        'tmpl.tmplName + tmpl.markup + tmpl.useViews',
        'tmpl properties'
    );
    ...
    assert.ok(
        html === '3  Jo  Amy  Bob ',
        '$.views.helpers("title", null); ...tmpl(array, true)',
        'Unregister named helper, then render array, no iteration'
    );
    ...
    tmpl = $.templates({
        markup:
            "{{:~title}}{{:~title2}}{{:~title3}} {{upper:name}} {{full/}} {{include tmpl='inner2'/}}{{include tmpl='inner'/}}",
        converters: {
            // private converter
            upper: function(val) {
    ...
    assert.ok(
        html ===
            'MrSirMs JO MR Jo Inner2Inner: Mr Jo MR Jo jo MrSirMs AMY MR Amy Inner2Inner: Mr Amy MR Amy amy MrSirMs BOB MR Bob Inner2Inner: Mr Bob MR Bob bob ',
        'tmpl = $.templates({markup: ..., converters: tags ... etc',
        'Compile template with resources'
    );
    ...
    assert.ok(
        ...
        '$.templates({ ... useViews: true, ...})',
        'useViews forced to true'
    );

    assert.ok(
        $.templates('{{for}}{{:#parent}}{{/for}}').useViews,
        '$.templates("{{for}}{{:#parent}}{{/for}}").useViews',
        'useViews defaults to true'
    );

    /*<<<<<<<<<<<<<*/ assert.testGroup('$.views.tags()'); /*>>>>>>>>>>>>>*/

    let tag = $.views.tags('add', function(val1, val2) {
        return val1 + '|' + val2;
    });
    ...
    assert.equal(
        test + tmpl({ first: 'A', last: 'B' }),
        'tagtagA|B A!!B',
        '$.views.tags("add", function() { ... })',
        'create tag from function, public or private'
    );
    ...
    assert.equal(
        ...
        'A&B A$$B',
        '$.views.tags("add", "...")',
        'create tag from string, public or private'
    );
    ...
    assert.equal(
        ...
        'a==B:FOO?myctx none',
        '$.views.tags("add", {...})',
        'create tag from tagOptions hash, public or private'
    );

    /*<<<<<<<<<<<<<*/ assert.testGroup('$.views.converters()'); /*>>>>>>>>>>>>>*/

    let converter = $.views.converters('add', function(val1, val2) {
        return val1 + '|' + val2 + ', ';
    });
    ...
    converter = $.views.converters(
        {
            privateadd: function(val1, val2) {
                return val1 + '!!' + val2 + ', ';
            },
        },
        tmpl
    ...
    assert.equal(
        test + '--' + tmpl({ first: 'A', last: 'B' }),
        'a|b, c!!d, --A|B,  A!!B, ',
        '$.views.converters("add", function() { ... })',
        'register converter, public or private'
    );

    converter = $.views.converters('add', null);

    assert.ok(
        converter === null && $.views.converters.add === undefined,
        '$.views.converters("...", null)',
        'unregister converter'
    );

    /*<<<<<<<<<<<<<*/ assert.testGroup('$.views.helpers()'); /*>>>>>>>>>>>>>*/

    let helper = $.views.helpers('add', function(val1: string, val2: string) {
        return val1 + '|' + val2 + ', ';
    });
    ...
    helper = $.views.helpers(
        {
            privateadd: function(val1: string, val2: string) {
                return val1 + '!!' + val2 + ', ';
            },
        },
        tmpl
    ...
    assert.equal(
        test + '--' + tmpl({ first: 'A', last: 'B' }),
        'a|b, c!!d, --A|B,  A!!B, ',
        '$.views.helpers("add", function() { ... })',
        'register helper, public or private'
    );

    helper = $.views.helpers('add', null);

    assert.ok(
        helper === null && $.views.helpers.add === undefined,
        '$.views.helpers("...", null)',
        'unregister helper'
    );
    ...
    let Book = $.views.viewModels({
        getters: ['title', 'price'],
        extend: {
            nameAndPrice: function(reverse?: boolean) {
    ...
    assert.ok(
        book1.title() + ': ' + book1.price() === 'Hope: $1.50' &&
            book1.nameAndPrice() === 'Hope: $1.50' &&
            book1.nameAndPrice(true) === '$1.50 for Hope',
        'VM=$.views.viewModels(vmOptions)',
        'Create VM, instantiate and access members'
    );
    ...
    assert.ok(
        html === 'Title: Hope, Price: $1.50, PriceName: $1.50 for Hope',
        'tmpl.render(book1)',
        'Render vm instance with template'
    );
    ...
    assert.ok(
        html === 'Title: Hope+, Price: $1.50+, PriceName: $1.50+ for Hope+',
        'book1.title(newValue)',
        'Modify vm instance, with setters, then render template'
    );
    ...
    assert.ok(
        Book === MyVMs.Bk,
        '$.views.viewModels("Bk", vmOptions, MyVMs)',
        'Register named VM on local MyVMs collection'
    );
    ...
    assert.ok(
        test === 'Hope',
        '$.views.viewModels({Bk: vmOptions}, MyVMs)',
        'Register one or more named VMs on local myVms collection'
    );
    ...
    assert.ok(
        test.title === 'Hope2' && test.price === '$1.50',
        'book.unmap()',
        'Round-trip data changes back to data, using unmap()'
    );

    let bookDataArray1 = [
        // book array (plain objects)
    ...
    booksArray1.merge([{ title: 'Hope2', price: '$1.50' }, { title: 'Courage', price: '$222.50' }]);

    test = booksArray1.unmap();

    assert.ok(
        test[1].title === 'Courage' && test[1].price === '$222.50',
        'bookArray = Book.map(dataArray) bookArray.merge(...) bookArray.unmap()',
        'Round-trip data array to array of book vm instances'
    );

    tmpl = $.templates(
        'Name: {{:name()}}, Street: {{:address().street()}}, Phones:' +
    ...
    $.views.viewModels(
        {
            Person: {
                ...
                id: function(vm, plain) {
                    // Callback function to determine 'identity'
        ...
        myVmCollection
    ); // Store View Models (typed hierarchy) on myVmCollection

    // Override generated name() get/set
    function myNameGetSet(this: Person, val: string) {
        if (!arguments.length) {
            // This is standard compiled get/set code
    ...
    function addPhone(this: Person, phoneNo: string) {
        // Uses myVmCollection.Phone() to construct new instance
    ...
    function defaultAddress(this: { name: string }) {
        // Function providing default address if undefined in data
    ...
    let peopleData = [
        ...
        },
        {
            personId: '2',
            name: 'Pete',
            phones: [{ number: '333 333 3333', phoneId: '2a' }],
        },
    ];

    // Second version of data - JSON string (e.g. new JSON request):
    let peopleData2 =
        '[{"personId":"2","name":"Peter","address":{"street":"11 1st Ave"},' +
    ...
    assert.equal(
        html,
        'Name: No name, Street: 2nd Ave, Phones:Name: Pete, Street: No street for "Pete", Phones: 333 333 3333 (Pete) ',
        'Person.map(peopleData)',
        'map data to full VM hierarchy'
    );
    ...
    assert.equal(
        html,
        'Name: Peter, Street: 11 1st Ave, Phones: 111 111 9999 (Peter)  333 333 9999 (Peter) ',
        'people.merge(peopleData2)',
        'Merge data on full hierarchy'
    );
    ...
    assert.equal(
        ...
        'Name: No name, Street: 2nd Ave, Phones:Name: Pete, Street: No street for "Pete", Phones: 333 333 3333 (Pete) ',
        'people.merge(peopleData)',
        'Merge back to previous data on full hierarchy'
    );
    ...
    assert.equal(
        html,
        'Name: newName, Street: 2nd Ave, Phones:Name: Pete, Street: No street for "Pete", Phones: 333 333 3333 (Pete) ',
        'people[0].name("newName")',
        'Change a property, deep in hierarchy'
    );
    ...
    assert.equal(
        html,
        'Name: newName, Street: 2nd Ave, Phones: xxx xxx xxxx (newName) Name: Pete, Street: No street for "Pete", Phones: 333 333 3333 (Pete) ',
        'people[0].addPhone("xxx xxx xxxx")',
        'Insert instance, deep in hierarchy'
    );

    let updatedPeopleData = people.unmap();

    assert.ok(
        updatedPeopleData[0].name === 'newName' && updatedPeopleData[0].phones[0].number === 'xxx xxx xxxx',
        'updatedPeopleData = people.unmap()',
        'Round-trip back to data'
    );
    ...
    $.views.tags('mytag', {
        ...
        show: function(view) {
            test +=
                (view.parent.views._1 === view && view.parent.parent === this.tagCtx.view) +
    ...
    tmpl = $.templates(
        "{{if true ~foo='FOO'}}{{for start=0 end=1}}{{mytag a 'b' 33}} in tag {{:~tag.show(#view)}} {{/mytag}}{{/for}}{{/if}}"
    );
    ...
    assert.equal(
        test,
        'trueFOO(00)true in tag {{:~tag.show(#view)}} includemytagdata555',
        'view.get(), view.parent, view.data, view.root etc etc',
        'View APIs tested'
    );
    ...
    assert.equal(
        ...
        'tagCtx.ctxPrm(), tagCtx.view, tagCtx.bndArgs(), tagCtx.params etc etc',
        'TagCtx APIs tested, {{myTag ...}}...{{/myTag}}'
    );
    ...
    assert.equal(
        test,
        'falseundefineddataundefinedundefinedundefinedtrue0trueundefinedundefinedtruemytagfalsefalsefalse',
        'tagCtx.ctxPrm(), tagCtx.view, tagCtx.bndArgs(), tagCtx.params etc etc',
        'TagCtx APIs tested, {{myTag/}}'
    );
    ...
    assert.equal(
        '' + $.views.settings.delimiters() + $.views.settings.allowCode(),
        '<%,%>,&true',
        'settings.delimiters(), settings.allowCode()',
        'get/set settings'
    );
