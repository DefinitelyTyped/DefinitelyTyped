...
const dialOpt: CodeMirror.OpenDialogOptions = {
    ...
    closeOnEnter: true,
    closeOnBlur: true,
    onKeyDown: (event: KeyboardEvent, value: string, close: () => void) =>
        false,
...
const close1: CodeMirror.CloseFunction = editor.openNotification(
    template,
    notifOpt
);
const close2: CodeMirror.CloseFunction = editor.openConfirm(
    template,
