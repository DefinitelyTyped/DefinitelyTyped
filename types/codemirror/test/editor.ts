...
editor.on(
    "keyHandled",
    (instance: CodeMirror.Editor, name: string, event: Event) => {}
);
editor.on(
    "inputRead",
    (
...
editor.on(
    ...
    (instance: CodeMirror.Editor, selection: CodeMirror.Selection) => {
...
editor.on(
    "swapDoc",
    (instance: CodeMirror.Editor, oldDoc: CodeMirror.Doc) => {}
);
editor.on(
    "gutterClick",
    (
...
editor.on(
    ...
    (instance: CodeMirror.Editor, event: Event) => {}
);
...
const posH: { line: number; ch: number; hitSide?: boolean } = editor.findPosH(
    position,
    0,
    "char",
    true
);
const posV: { line: number; ch: number; hitSide?: boolean } = editor.findPosV(
    position,
    0,
    "page"
);
