...
api.Users.updateKycDocument('user-id', { Status: 'VALIDATION_ASKED' }).then((data) => {
    const d = data; // $ExpectType KycDocumentData
});
...
api.Users.createKycPageFromFile('user-id', 'kyc-document-id', 'path/to/file').then((data) => {
    const d = data; // $ExpectType KycDocumentData
});
...
api.Users.createUboDeclaration('user-id', { DeclaredUBOs: ['user1'] }).then((data) => {
...
api.Transfers.createRefund('transfer-id', { AuthorId: 'user-id' }).then((data) => {
    const d = data; // $ExpectType RefundData
});
...
api.DisputeDocuments.createDisputeDocumentConsult('dispute-doc-id').then((data) => {
    const d = data; // TODO unsure of expected type
});
...
api.Disputes.updateDisputeDocument('dispute-id', { Tag: 'update' }).then((data) => {
    const d = data; // $Expect DisputeDocumentData
});
...
api.Disputes.createDisputeDocumentPageFromFile('dispute-id', 'dispute-doc-id', 'path/to/file').then((data) => {
    const d = data; // $Expect DisputeData
});
...
api.Mandates.getMandatesForBankAccount('user-id', 'bank-account-id').then((data) => {
    const d = data; // $ExpectType MandateData[]
});
...
api.Hooks.update({ Id: 'hook-id', Url: 'https://new-hook.com/hooks' }).then((data) => {
    const d = data; // $ExpectType HookData
});
