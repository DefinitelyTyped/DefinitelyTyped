import "./test/GetIntrinsic.test";
import "./test/es5.test";
import "./test/es2015.test";
import "./test/es2016.test";
import "./test/es2017.test";
import "./test/es2018.test";
import "./test/es2019.test";
import "./test/index.test";

// #region ./test/helpers/
import "./test/helpers/DefineOwnProperty.test";
import "./test/helpers/OwnPropertyKeys.test";
import "./test/helpers/assertRecord.test";
import "./test/helpers/assign.test";
import "./test/helpers/callBind.test";
import "./test/helpers/callBound.test";
import "./test/helpers/every.test";
import "./test/helpers/forEach.test";
import "./test/helpers/getInferredName.test";
import "./test/helpers/getIteratorMethod.test";
import "./test/helpers/getOwnPropertyDescriptor.test";
import "./test/helpers/getProto.test";
import "./test/helpers/getSymbolDescription.test";
import "./test/helpers/isFinite.test";
import "./test/helpers/isNaN.test";
import "./test/helpers/isPrefixOf.test";
import "./test/helpers/isPrimitive.test";
import "./test/helpers/isPropertyDescriptor.test";
import "./test/helpers/isSamePropertyDescriptor.test";
import "./test/helpers/maxSafeInteger.test";
import "./test/helpers/mod.test";
import "./test/helpers/padTimeComponent.test";
import "./test/helpers/regexTester.test";
import "./test/helpers/setProto.test";
import "./test/helpers/sign.test";
import "./test/helpers/timeConstants.test";
// #endregion

// #region ./test/operations/
import "./test/operations/2015.test";
import "./test/operations/2016.test";
import "./test/operations/2017.test";
import "./test/operations/2018.test";
import "./test/operations/2019.test";
// #endregion
