...
function nodeJS(session: autobahn.Session) {
    var sessionObservable = Rx.Observable.fromConnection({ url: 'ws://localhost:9000', realm: 'realm1' });
}
...
function new_in_version3(onResult: Rx.Observer<autobahn.ISubscription>) {
    Rx.Observable.subscribeAsObservable(
        Rx.Observable.fromConnection({ url: 'ws://myconnectionurl:9090', realm: 'realm1' }),
        'wamp.my.foo',
        undefined,
        onResult
    );
...
function or_use_them_together(session: autobahn.Session, observer: Rx.Observer<IWampEvent>) {
    ...
    Rx.Observable.generateWithRelativeTime(0, x => x < 42, x => x + 1, x => ({ args: [x] }), x => 15).subscribe(topic);
}

function registering_methods(session: autobahn.Session, endpoint: autobahn.RegisterEndpoint) {
    var registration = Rx.Observable.registerAsObservable(session, 'wamp.my.add', endpoint, {})
...
function new_in_version3_registration(myUrl: string, endpoint: autobahn.RegisterEndpoint) {
    ...
    Rx.Observable.registerAsObservable(
        ...
        'wamp.my.add',
        (args, kwargs, details) => {
            if (args === undefined || args.length < 1) throw new autobahn.Error('No values to sum!');
...
function weather_station_monitor(session: autobahn.Session) {
    ...
    var desiredTemperature = Rx.Observable.subscribeAsObservable(session, 'temperature.indoors.desired').map(
        x => x.args[0] as number
    );
