...
export function parseCMSSigned() {
    ...
    while (certificatesTable.rows.length > 1) certificatesTable.deleteRow(certificatesTable.rows.length - 1);
        ...
        for (let cert of cmsSignedSimpl.certificates) {
            if (cert instanceof Certificate) {
                ...
                for (let i = 0; i < cert.issuer.typesAndValues.length; i++) {
                    ...
                    if (typeof typeval === 'undefined') typeval = cert.issuer.typesAndValues[i].type.toString();
        ...
        for (let crl of cmsSignedSimpl.crls) {
            if (crl instanceof CertificateRevocationList) {
                ...
                for (let i = 0; i < crl.issuer.typesAndValues.length; i++) {
                    ...
                    if (typeof typeval === 'undefined') typeval = crl.issuer.typesAndValues[i].type.toString();
...
export function createCMSSigned(buffer: ArrayBuffer) {
    ...
    sequence = sequence.then(() => {
        ...
        if ('hash' in algorithm.algorithm) (algorithm.algorithm as any).hash.name = hashAlgorithm;
        // endregion
    ...
    if ((document.getElementById('add_ext') as HTMLInputElement).checked) {
        ...
        sequence = sequence.then(() => crypto.digest({ name: hashAlgorithm }, new Uint8Array(buffer)));
        // endregion
    ...
    sequence.then(
        () => {
            ...
            for (let i = 0; i < view.length; i++) signedDataString = signedDataString + String.fromCharCode(view[i]);
            // endregion
...
function verifyCMSSigned() {
    ...
    cmsSignedSimpl
        ...
        .then(result => alert(`Verification result: ${result}`), error => alert(`Error during verification: ${error}`));
    // endregion
}
