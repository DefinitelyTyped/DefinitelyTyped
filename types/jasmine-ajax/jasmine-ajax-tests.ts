...
describe('FakeRequest', () => {
    beforeEach(() => {
        ...
        this.FakeRequest = getJasmineRequireObj().AjaxFakeRequest(this.eventBusFactory)(
            this.fakeGlobal,
            this.requestTracker,
            this.stubTracker,
            this.paramParser
    ...
    it('skips XMLHttpRequest attributes that IE does not want copied', () => {
        ...
        const FakeRequest = getJasmineRequireObj().AjaxFakeRequest(this.eventBusFactory)(
            window,
            this.requestTracker,
            this.stubTracker,
            this.paramParser
    ...
    it('retrieves a combined response header', () => {
        ...
        request.respondWith({
            status: 200,
            responseHeaders: [{ name: 'X-Header', value: 'foo' }, { name: 'X-Header', value: 'bar' }],
        });
        ...
        describe('response with unique header names using an object', () => {
            ...
            it('getAllResponseHeaders should return all values', () => {
                expect(response.getAllResponseHeaders()).toBe(
                    ['X-Header1: header 1 value', 'X-Header2: header 2 value', 'X-Header3: header 3 value'].join(
                        '\r\n'
                    ) + '\r\n'
                );
            });
...
function sharedAjaxResponseBehaviorForZepto_Failure(context) {
    describe('the failure response', () => {
        ...
        it('should have the expected xhr2 response', () => {
            const expected =
                context.response || xhr.responseType === 'json' ? JSON.parse(xhr.responseText) : xhr.responseText;
            expect(xhr.response).toEqual(expected);
        });
