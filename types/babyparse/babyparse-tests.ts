...
var UNPARSE_TESTS: UnparserTest[] = [
    ...
    },
    {
        ...
        notes:
            'An array of arrays, even if just a single row.<br>Baby should add empty fields property to prevent errors.',
        input: { data: [['a', 'b', 'c']] },
        expected: 'a,b,c',
    },
    {
...
function displayResults(test: ParserTest, actual: BabyParse.ParseResult, results: ParserTestCompareResult): void {
    var testId = testCount++;

    var testDescription = test.description || '';

    var tr = testId + ',' + testDescription + ',' + results.data.passed + ',' + results.errors.passed;
    // + '\t' + results.data
    // + '\t' + results.errors
...
function compare(actualData: any[], actualErrors: any[], expected: ParserTestExpectedResult): ParserTestCompareResult {
    ...
    function compareData(actual: any[], expected: any[]): ParserTestPassFail {
        ...
        if (actual.length != expected.length) passed = false;
        else {
        ...
        if (passed)
            // final check will catch any other differences
...
function runUnparseTests() {
    ...
    function runTest(test: UnparserTest) {
        ...
        var tr = testId + ',' + testDescription + ',' + results.passed;
        // + '\t' + JSON.stringify(test.config, null, 2)
