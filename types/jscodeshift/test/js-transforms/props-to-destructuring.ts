        ...
        .replaceWith((p) => {
            ...
            const isFromProps = (name: string, resolvedScope: any) => {
                return resolvedScope.getBindings()[name].every((p: any) => {
                    ...
                    if (p.parentPath.value.type === 'Property' && p.parentPath.value.key.name !== name) return false;
            ...
            return j.functionExpression(p.value.id, p.value.params, j.blockStatement([decl].concat(p.value.body.body)));
        })
        .toSource();
};
