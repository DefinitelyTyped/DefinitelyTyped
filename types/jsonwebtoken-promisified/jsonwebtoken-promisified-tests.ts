...
// sign asynchronously
jwt.sign(testObject, cert, { algorithm: 'RS256' }, (err: Error, token: string) => {
    console.log(token);
});

jwt.signAsync(testObject, cert, { algorithm: 'RS256' }).then((token: string) => console.log(token));
/**
 * jwt.verify
...
jwt.verify(
    token,
    'shhhhh',
    (err: jwt.JsonWebTokenError | jwt.NotBeforeError | jwt.TokenExpiredError, decoded: object | string) => {
