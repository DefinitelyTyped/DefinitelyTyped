...
function CombiningMultipleStreamsAndProperties() {
    {
        var property = Bacon.constant(1),
            stream = Bacon.once(2),
            constant = 3;
        Bacon.combineAsArray(property, stream, constant).log(); // > returns [1, 2, 3]
    }
...

    {
        ...
        Bacon.zipAsArray(x, y, z).log(); // > returns values `[1, 10, 100]`, `[2, 20, 200]` and `[3, 30, 300]`
    }
...
function Errors() {
    ...
    Bacon.fromArray<string, number>([1, 2, 3, 4]).flatMapError<number>(error => {
        var isNonCriticalError = (error: string) => Math.random() < 0.5,
...
function JoinPatterns() {
    {
        ...
        Bacon.when([tick, keyEvent], (_: number, timestamp: number) => handleKeyEvent(timestamp), [tick], handleTick);
    ...
    }
    {
        ...
        Bacon.update(
            initial,
            [x, y, z],
            (previous: number, x: number, y: number, z: number) => previous + x + y + z,
            [x, y],
            (previous: number, x: number, y: number) => previous + x + y
        );
    ...
    }
    {
        ...
        var scoreMultiplier = Bacon.constant(1),
            ...
            score = Bacon.update(
                0,
                [hitUfo, scoreMultiplier],
                (score: number, _: number, multiplier: number) => score + 100 * multiplier,
                [hitMotherShip],
                (score: number, _: number) => score + 2000
            );
    ...
    }
    {
        ...
        let oxygen = Bacon.interval(1e3, 'O'),
            ...
            makeWater = (oxygen: string, hydrogen1: string, hydrogen2: string) =>
                `${hydrogen1}${[hydrogen1, hydrogen2].length}${oxygen}`,
...
function JoinPatternsAndBaconBus() {
    {
        ...
        let chopsticks = [new Bacon.Bus(), new Bacon.Bus(), new Bacon.Bus()],
            ...
            dining = Bacon.when(
                [hungry[0], chopsticks[0], chopsticks[1]],
                eat(0),
                [hungry[1], chopsticks[1], chopsticks[2]],
                eat(1),
                [hungry[2], chopsticks[2], chopsticks[0]],
                eat(2)
            ).log('dining');
