...
declare namespace chrome.declarativeNetRequest {
    ...
    /** Time interval within which MAX_GETMATCHEDRULES_CALLS_PER_INTERVAL getMatchedRules calls can be made, specified
     * in minutes. Additional calls will fail immediately and set runtime.lastError. Note: getMatchedRules calls
     * associated with a user gesture are exempt from the quota. */
    const GETMATCHEDRULES_QUOTA_INTERVAL: number;

    /** The number of rules that an extension can specify across its enabled static rulesets that will not count
     * towards the global total. Up to this number of rules can be enabled regardless of the number of rules that
     * count towards the global total. */
    ...
    const MAX_GETMATCHEDRULES_CALLS_PER_INTERVAL: number;

    /** The maximum number of combined dynamic or session scoped rules an extension can add.*/
    const MAX_NUMBER_OF_DYNAMIC_OR_SESSION_RULES: number;

    /** The maximum number of dynamic rules that an extension can add. */
    const MAX_NUMBER_OF_DYNAMIC_RULES: number;

    /** The maximum number of regular expression rules that an extension can add. This limit is evaluated separately
     * for the set of dynamic rules and those specified in the rule resources file. */
    const MAX_NUMBER_OF_REGEX_RULES: number;

    /** The maximum number of rules that an extension can specify across its enabled static rulesets. Any excess rules
     * will be ignored and an install warning will be raised. */
    const MAX_NUMBER_OF_RULES: number;
    ...
    /** This describes whether the request is first or third party to the frame in which it originated. A request is
     * said to be first party if it has the same domain (eTLD+1) as the frame in which the request originated. */
    ...
    export interface RequestDetails {
        ...
        /** The origin where the request was initiated. This does not change through redirects. If this is an opaque
         * origin, the string 'null' will be used. */
        initiator?: string;
    ...
    export interface RuleCondition {
        ...
        /** The rule will only match network requests originating from the list of domains. If the list is omitted, the
         * rule is applied to requests from all domains. An empty list is not allowed.
         *
         * Notes:
         *
        ...
        /** The rule will not match network requests originating from the list of excludedDomains. If the list is empty
         * or omitted, no domains are excluded. This takes precedence over domains.
         *
         * Notes:
         *
         * Sub-domains like "a.example.com" are also allowed.
        ...
        /** List of resource types which the rule won't match. Only one of resourceTypes and excludedResourceTypes
         ...
         *
         * Note: The regexFilter must be composed of only ASCII characters. This is matched against a url where the host
         * is encoded in the punycode format (in case of internationalized domains) and any other non-ascii characters
         ...
         *
         * Note: this must be specified for allowAllRequests rules and may only include the sub_frame and main_frame
         * resource types.
         */
         ...
         *
         * '|' : Left/right anchor: If used at either end of the pattern, specifies the beginning/end of the url
         * respectively.
         *
         * '||' : Domain name anchor: If used at the beginning of the pattern, specifies the start of a (sub-)domain
         * of the URL.
         *
         ...
         *
         * Therefore urlFilter is composed of the following parts: (optional Left/Domain name anchor) + pattern +
         ...
         * Note: Only one of urlFilter or regexFilter can be specified.
         *
         * Note: The urlFilter must be composed of only ASCII characters. This is matched against a url where the host
         * is encoded in the punycode format (in case of internationalized domains) and any other non-ascii characters
         * are url encoded in utf-8. For example, when the request url is http://abc.рф?q=ф, the urlFilter will be
         * matched against the url http://abc.xn--p1ai/?q=%D1%84.
         */
        urlFilter?: string;
    }
    ...
    export interface MatchedRule {
        ...
        /** ID of the Ruleset this rule belongs to. For a rule originating from the set of dynamic rules, this will be
         * equal to DYNAMIC_RULESET_ID. */
        rulesetId: string;
    }
    ...
    export interface MatchedRuleInfo {
        ...
        /** The time the rule was matched. Timestamps will correspond to the Javascript convention for times, i.e.
    ...
    export interface URLTransform {
        /** The new fragment for the request. Should be either empty, in which case the existing fragment is cleared;
         *  or should begin with '#'. */
        fragment?: string;
        ...
        /** The new query for the request. Should be either empty, in which case the existing query is cleared; or
         * should begin with '?'. */
        query?: string;
    ...
    export interface RegexOptions {
        ...
        /** Whether the regex specified requires capturing. Capturing is only required for redirect rules which specify
         * a regexSubstition action. The default is false. */
    ...
    export interface IsRegexSupportedResult {
        ...
        reasion?: UnsupportedRegexReason;
    }
    ...
    export interface ExtensionActionOptions {
        /** Whether to automatically display the action count for a page as the extension's badge text. This preference
    ...
    export interface Redirect {
        ...
        /** Substitution pattern for rules which specify a regexFilter. The first match of regexFilter within the url
         * will be replaced with this pattern. Within regexSubstitution, backslash-escaped digits (\1 to \9) can be
         * used to insert the corresponding capture groups. \0 refers to the entire matching text. */
    ...
    export interface Ruleset {
        ...
        /** A non-empty string uniquely identifying the ruleset. IDs beginning with '_' are reserved for
         * internal use. */
        id: string;
    ...
    export function getAvailableStaticRuleCount(callback: (count: number) => void);

    /** Returns the current set of dynamic rules for the extension.
     *
     * @param callback Called with the set of dynamic rules. An error might be raised in case of transient internal
     * errors.
     */
    export function getDynamicRules(callback: (rules: Rule[]) => void);

    /** Returns the ids for the current set of enabled static rulesets.
     *
     * @param callback Called with a list of ids, where each id corresponds to an enabled static Ruleset. */
    export function getEnabledRulesets(callback: (rulesetIds: string[]) => void);

    /** Returns all rules matched for the extension. Callers can optionally filter the list of matched rules by
     * specifying a |filter|. This method is only available to extensions with the declarativeNetRequestFeedback
     * permission or having the activeTab permission granted for the tabId specified in filter. Note: Rules not
     * associated with an active document that were matched more than five minutes ago will not be returned.
     *
     * @param filter An object to filter the list of matched rules.
     * @param callback Called once the list of matched rules has been fetched. In case of an error, runtime.lastError
     * will be set and no rules will be returned. This can happen for multiple reasons, such as insufficient
     * permissions, or exceeding the quota.
     * */
    export function getMatchedRules(filter: MatchedRulesFilter | undefined, callback: (details: RulesMatchedDetails) => void);

    /** Returns the current set of session scoped rules for the extension.
     *
     * @param callback Called with the set of session scoped rules.
     **/
    export function getSessionRules(callback: (rules: Rule[]) => void);
     ...
     * reason if not.
     * */
    export function isRegexSupported(regexOptions: RegexOptions, callback: (result: IsRegexSupportedResult) => void);

    /** Configures if the action count for tabs should be displayed as the extension action's badge text and provides
     * a way for that action count to be incremented. */
    export function setExtensionActionOptions(options: ExtensionActionOptions, callback: Function);

    /** Modifies the current set of dynamic rules for the extension. The rules with IDs listed in options.removeRuleIds
     * are first removed, and then the rules given in options.addRules are added. Notes:
     *
     * This update happens as a single atomic operation: either all specified rules are added and removed, or an error
     * is returned.
     *
     ...
     *
     * @param callback Called once the update is complete or has failed. In case of an error, runtime.lastError will be
     * set and no change will be made to the rule set. This can happen for multiple reasons, such as invalid rule
     * format, duplicate rule ID, rule count limit exceeded, internal errors, and others.
     */
    export function updateDynamicRules(options: UpdateRuleOptions, callback: Function);

    /** Updates the set of enabled static rulesets for the extension. The rulesets with IDs listed in
     * options.disableRulesetIds are first removed, and then the rulesets listed in options.enableRulesetIds are added.
     ...
     *
     * @param callback Called once the update is complete. In case of an error, runtime.lastError will be set and no
     * change will be made to set of enabled rulesets. This can happen for multiple reasons, such as invalid ruleset
     * IDs, rule count limit exceeded, or internal errors.
     */
    export function updateEnabledRulesets(options: UpdateRulesetOptions, callback: Function);

    /** Modifies the current set of session scoped rules for the extension. The rules with IDs listed in
     * options.removeRuleIds are first removed, and then the rules given in options.addRules are added.
     *
     * Notes:
     *
     * This update happens as a single atomic operation: either all specified rules are added and removed, or an error
     * is returned.  These rules are not persisted across sessions and are backed in memory.
     * MAX_NUMBER_OF_DYNAMIC_OR_SESSION_RULES is the maximum number of combined dynamic or session rules an extension
     * can add.
     *
     * @param callback Called once the update is complete or has failed. In case of an error, runtime.lastError will
     * be set and no change will be made to the rule set. This can happen for multiple reasons, such as invalid rule
     * format, duplicate rule ID, rule count limit exceeded, and others.
     */
    export function updateSessionRules(options: UpdateRuleOptions, callback: Function);
    ...
    /** Fired when a rule is matched with a request. Only available for unpacked extensions with the
     * declarativeNetRequestFeedback permission as this is intended to be used for debugging purposes only. */
