...
if (transition) {
    ...
    const to: NavigationDestination = transition.to;
    const committed: Promise<void> = transition.committed;
    const finished: Promise<void> = transition.finished;
}
const activation: NavigationActivation | null = navigation.activation;
if (activation) {
    const entry: NavigationHistoryEntry = activation.entry;
    const from: NavigationHistoryEntry | null = activation.from;
    const type: NavigationTypeString = activation.navigationType;
}
...
const navigateListener = (e: NavigateEvent) => {
    ...
    const sourceElement: Element | null = e.sourceElement;
    ...
    e.intercept({
        precommitHandler: async (controller) => {
            controller.redirect("/", { history: "auto" });
            controller.redirect("/", { history: "push" });
            controller.redirect("/", { history: "replace" });
            controller.redirect("/");
