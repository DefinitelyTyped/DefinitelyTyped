...
const getList2: Promise<User[]> = frappe.db.get_list('User', { fields: ['name', 'age', 'foo'], filters: { active: true }, });
...
const getValue2: Promise<FrappeResponse<Pick<User, 'name' | 'active'>>> = frappe.db.get_value('User', { age: 20 }, ['name', 'active']);
