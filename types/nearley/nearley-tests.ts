...
compiledRules.ParserStart; // $ExpectType number
...
compiledRules.ParserRules[0].postprocess; // $ExpectType ((data: any[], reference: number, wantedBy: any) => void) | undefined
...
rule.postprocess; // $ExpectType ((data: any[], reference: number, wantedBy: any) => void) | undefined
...
const parser = new Parser(compiledRules, parserStart);
new Parser(compiledRules, parserStart, {lexer});
new Parser(compiledRules, parserStart, {keepHistory: false});
new Parser(grammar);
