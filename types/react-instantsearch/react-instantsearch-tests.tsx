...
() => {
    const Content = connectStateResults(({ error }) => (error ? <div>Some error</div> : <div>No error</div>));
};
...
() => {
    ...
    const AllResults = connectStateResults(({ allSearchResults, children }) => {
        const hasResults = allSearchResults && values(allSearchResults).some((results) => results.nbHits > 0);
...
() => {
    const RefinementListWithSearchBox = connectRefinementList((props) => {
        const values = props.items.map((item) => {
            const label = item._highlightResult ? <Highlight attribute="label" hit={item} /> : item.label;
