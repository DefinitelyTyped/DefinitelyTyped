...
{
    const library = Yadda.localisation.English.library().given('a user called (\\w+)', (name: string) => {
        // Code goes here
    });
...
{
    const library = Yadda.localisation.English.library().given('a user called $name', (name: string) => {
        // Code goes here
    });
...

{
    ...
    const dictionary = new Yadda.Dictionary().define('csv', /([^\u0000]*)/, converter);

    const library = Yadda.localisation.English.library(dictionary).given('some csv\n$csv', (csv: string) => {
        // Code goes here
    });
...

{
    ...
    const library = Yadda.localisation.English.library(dictionary).given('A $gender user', (gender: string) => {
        // Code goes here
    });
...

{
    ...
    const library = Yadda.localisation.English.library(dictionary).given('A whole number $num', (number: string) => {
...
function quantity_converter(
    amount: string,
    units: string,
    cb: (err: Error | null, quantity: { amount: string; units: string }) => void,
) {
    cb(null, { amount, units });
}

{
    const dictionary = new Yadda.Dictionary().define('quantity', /(\d+) (\w+)/, quantity_converter);

    const library = Yadda.localisation.English.library(dictionary).given('a delay of $quantity', (quantity: string) => {
