...
var f = (x: string) => (y: string) => (x.length > y.length ? 1 : x.length < y.length ? -1 : 0);
...
var objToListsRes = prelude.objToLists({ a: 1, b: 2, c: 3 });
//=> [["a", "b", "c"], [1, 2, 3]]
...
var fixRes: number = prelude.fix((fib: (n: number) => number) => (n: number) => (n <= 1 ? 1 : fib(n - 1) + fib(n - 2)))(
    9,
); //=> 55
