...
spotifyApi.getAlbumTracks('41MnTivkwTO3UUJ8DrqEJJ', { limit: 5, offset: 1 }).then(
    (data) => {
...
spotifyApi.getAudioFeaturesForTracks(['4iV5W9uYEdYUVa79Axb7Rh', '3Qm86XLflmIXVm1wcwkgDK']).then(
    (data) => {
        console.log(data.body);
    },
    (err) => {
...
spotifyApi.createPlaylist('thelinmichael', 'My Cool Playlist', { public: false }).then(
    (data) => {
...
spotifyApi
    .removeTracksFromPlaylist('5ieJqeLJjjI8iJWaxeBLuK', [{ uri: 'spotify:track:4iV5W9uYEdYUVa79Axb7Rh' }], {
...
spotifyApi.reorderTracksInPlaylist('5ieJqeLJjjI8iJWaxeBLuK', 0, 10, { range_length: 2 }).then(
    (data) => {
...
spotifyApi.uploadCustomPlaylistCoverImage('5ieJqeLJjjI8iJWaxeBLuK', 'longbase64uri').then(
    (data) => {
...
spotifyApi.areFollowingPlaylist('thelinmichael', '5ieJqeLJjjI8iJWaxeBLuK', ['thelinmichael', 'ella']).then(
    (data) => {
...
spotifyApi
    .getFeaturedPlaylists({ limit: 3, offset: 1, country: 'SE', locale: 'sv_SE', timestamp: '2014-10-23T09:00:00' })
    .then(
        (data) => {
            console.log(data.body);
        },
