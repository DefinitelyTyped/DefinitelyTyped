...
_.filter(
    [
        { name: 'larry', relation: 'father' },
        { name: 'moe', relation: 'uncle' },
    ],
    isUncleMoe,
);
...
_.chain({ name: 'moe', age: 50, userid: 'moe1' }).pick(['name', 'age']).value().age = 5;
_.chain({ name: 'moe', age: 50, userid: 'moe1' })
    .pick((value, key) => {
        return key === 'name' || key === 'age';
    })
    .value().age = 5;
...
function chain_tests() {
    ...
    var firstVal: number | undefined = _.chain([1, 2, 3]).first().value();

    var firstVal2: number | undefined = _.chain([]).first().value();
    ...
    let valuePerYear: number[] = _.chain(yearObject).values().value();

    const arr1: string[] = ['z', 'x', 'y'];
    const query = 'z';
    let arr2: string[] = ['a', 'b', 'c'];
    arr2 = _.chain(arr1).union(arr2).without(query).value();
}
