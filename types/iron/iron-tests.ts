...
Iron.encrypt('password', Iron.defaults.encryption, 'data').then((obj: { data: Buffer; key: Iron.Key }) => {
    console.log(obj);
});

Iron.decrypt('password', Iron.defaults.encryption, 'data').then((buffer: Buffer) => {
    console.log(buffer);
});

Iron.hmacWithPassword('password', Iron.defaults.integrity, 'data').then((value: Iron.HMacResult) => {
    console.log(value);
});
