...
class EditCellClassNameTable extends React.Component {
    ...
    onBeforeSaveCellAsync = <K extends keyof Job>(
        row: Job,
        cellName: K,
        cellValue: Job[K],
...
class MultiSortAndFiltering extends React.Component {
    ...
    cleanSort = () => {
        this.tableRef.cleanSort();
    };

    cleanSelected = () => {
        this.tableRef.cleanSelected();
    };
    ...
    getNameCaret = (direction: SortOrder | null, fieldName: string) =>
        direction === 'asc' ? <span> up</span> : direction === 'desc' ? <span> down</span> : <span> up/down</span>;
            ...
            <div>
                ...
                >
                    <TableHeaderColumn
                        ...
                        sortHeaderColumnClassName="sorted"
                    >
                        Product ID
                    </TableHeaderColumn>
                    <TableHeaderColumn
...
class CustomPagination extends React.Component {
    renderPagination = (props: PaginationPanelProps) => (
        <div>
            <SizePerPageDropDown
                variation="dropup"
                options={props.sizePerPageList}
                btnContextual="primary"
            ...
            <div>
                ...
                <button onClick={() => props.changePage(3)} className="btn btn-default">
                    Jump to page 3
                </button>
            </div>
        </div>
...
class DefaultCustomInsertModalHeaderFooterTable extends React.Component {
    ...
    createCustomModalBody = (
        ...
        validateState: { [dataField: string]: string },
        ignoreEditable: boolean,
    ) => <MyCustomBody columns={columns} validateState={validateState} ignoreEditable={ignoreEditable} />;
    ...
    createCustomToolBar = (props: ToolBarProps) => {
        return (
            <div style={{ margin: '15px' }}>
                ...
                <div className="col-xs-8 col-sm-4 col-md-4 col-lg-2">{props.components.searchPanel}</div>
            </div>
        );
    };
...
class ExpandRowExample extends React.Component<{}, { expanding: number[] }> {
    ...
    selectedRowClass = (row: Product, isSelect: boolean) =>
        isSelect ? (row.id >= 3 ? 'bigger-than-three-select-row' : 'less-than-three-select-row') : '';
    ...
    render() {
        ...
        const selectRow: SelectRow<Product> = {
            ...
            bgColor: (row: Product, isSelect: boolean) =>
                isSelect ? (row.id < 2 ? 'blue' : row.id < 4 ? 'red' : 'yellow') : null,
...
class MouseEventTable extends React.Component {
    render() {
        const options: Options<Product> = {
            ...
            onMouseEnter: () => {
                console.log('mouse entered table');
            },
...
class HideOnInsertTable extends React.Component {
    handleAddRowWithASyncError = (
        row: any,
        colInfo: ReadonlyArray<ColumnDescription<Job>>,
    ...
    render() {
        return (
            <BootstrapTable data={extendedJobs}>
                ...
                <TableHeaderColumn dataField="active" dataFormat={this.activeFormatter}>
                    Active
                </TableHeaderColumn>
            </BootstrapTable>
        );
        ...
        return (
            <BootstrapTable ref="table" data={products}>
                <TableHeaderColumn dataField="id" isKey={true}>
                    Product ID
                    <br />
                    <a onClick={this.handlerClickCleanFiltered} style={{ cursor: 'pointer' }}>
                        clear filters
                    </a>
                </TableHeaderColumn>
                <TableHeaderColumn
                ...
                <TableHeaderColumn
                    ref={(node) => {
                        this.satisfaction = node;
                    }}
                    dataField="satisfaction"
                    filter={{ type: 'NumberFilter', options: satisfaction }}
                >
                    Buyer Satisfaction
                </TableHeaderColumn>
                <TableHeaderColumn
                    ref={(node) => {
                        this.inStockDate = node;
                    }}
...
class ProgrammaticallyArrayFilter extends React.Component {
    ...
    render() {
        return (
            <div>
                <button onClick={this.handleBtnClick} className="btn btn-default">
