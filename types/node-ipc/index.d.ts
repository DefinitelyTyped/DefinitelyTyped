...
declare namespace NodeIPC {
    class IPC {
         ...
         * @param id is the string id of the socket being connected to. The socket with this id is added to the ipc.of object when created.
         * @param path is the path of the Unix Domain Socket File, if the System is Windows, this will automatically be converted to an appropriate pipe with the same information as the Unix Domain Socket File. If not set this will default to ipc.config.socketRoot+ipc.config.appspace+id
         ...
         * @param id is the string id of the socket being connected to. For TCP & TLS sockets, this id is added to the ipc.of object when the socket is created with a reference to the socket
         ...
         * @param callback 	this is the function to execute when the socket has been created
         */
        connectToNet(id: string, host: string, callback?: () => void): void;
        /**
         * https://www.npmjs.com/package/node-ipc#connecttonet
         * Used to connect as a client to a TCP or TLS socket via the network card. This can be local or remote, if local, it is recommended that you use the Unix and Windows Socket Implementaion of connectTo instead as it is much faster since it avoids the network card altogether.
         * For TLS and SSL Sockets see the node-ipc TLS and SSL docs. They have a few additional requirements, and things to know about and so have their own doc.
         ...
         * @param callback 	this is the function to execute when the socket has been created
         */
        connectToNet(id: string, port: number, callback?: () => void): void;
        /**
         ...
         * @param path This is the path of the Unix Domain Socket File, if the System is Windows, this will automatically be converted to an appropriate pipe with the same information as the Unix Domain Socket File. If not set this will default to ipc.config.socketRoot+ipc.config.appspace+id
         * @param callback This is a function to be called after the Server has started. This can also be done by binding an event to the start event like ipc.server.on('start',function(){});
         */
         ...
         * @param host If not specified this defaults to the first address in os.networkInterfaces(). For TCP, TLS & UDP servers this is most likely going to be 127.0.0.1 or ::1
         * @param port The port on which the TCP, UDP, or TLS Socket server will be bound, this defaults to 8000 if not specified
         * @param UDPType If set this will create the server as a UDP socket. 'udp4' or 'udp6' are valid values. This defaults to not being set. When using udp6 make sure to specify a valid IPv6 host, like ::1
        ...
        /**
         * This is where socket connection refrences will be stored when connecting to them as a client via the ipc.connectTo or iupc.connectToNet. They will be stored based on the ID used to create them, eg : ipc.of.mySocket
         */
        of: any;
        /**
    ...
    interface IPCClient {
        /**
         * triggered when a JSON message is received. The event name will be the type string from your message and the param will be the data object from your message eg : { type:'myEvent',data:{a:1}}
         */
         ...
         */
        on(event: "connect", callback: () => void): IPCClient;
        /**
         * triggered by client when socket has disconnected from server
         */
        on(event: "disconnect", callback: () => void): IPCClient;
        /**
        ...
        /**
         * triggered when socket has been totally destroyed, no further auto retries will happen and all references are gone
         */
        on(event: "destroy", callback: () => void): IPCClient;
        /**
    ...
    interface IPCServer extends IPCClient {
        /**
        ...
        emit(event: string, value: any): IPCClient;
        emit(socket: Socket, event: string, value?: any): IPCServer;
        emit(socket: Socket, value: any): IPCServer;
        emit(socketConfig: SocketConfig, event: string, value?: any): IPCServer;
        emit(socketConfig: SocketConfig, value?: any): IPCServer;
    }
    ...
    interface IPCConfig {
        /**
         * Default: 'app.'
         * Used for Unix Socket (Unix Domain Socket) namespacing. If not set specifically, the Unix Domain Socket will combine the socketRoot, appspace, and id to form the Unix Socket Path for creation or binding. This is available incase you have many apps running on your system, you may have several sockets with the same id, but if you change the appspace, you will still have app specic unique sockets
         */
        appspace: string;
        /**
         * Default: '/tmp/'
        ...
        /**
         * Default: 'utf8'
         * the default encoding for data sent on sockets. Mostly used if rawBuffer is set to true. Valid values are : ascii utf8 utf16le ucs2 base64 hex
         */
        ...
        /**
         * Default: 100
         * This is the max number of connections allowed to a socket. It is currently only being set on Unix Sockets. Other Socket types are using the system defaults
         */
        maxConnections: number;
        /**
         * Default: 500
         * This is the time in milliseconds a client will wait before trying to reconnect to a server if the connection is lost. This does not effect UDP sockets since they do not have a client server relationship like Unix Sockets and TCP Sockets
         */
        retry: number;
        /*  */
        /**
         * Default: false
         * if set, it represents the maximum number of retries after each disconnect before giving up and completely killing a specific connection
         */
        maxRetries: boolean;
        /**
         * Default: false
         * Defaults to false meaning clients will continue to retry to connect to servers indefinitely at the retry interval. If set to any number the client will stop retrying when that number is exceeded after each disconnect. If set to true in real time it will immediately stop trying to connect regardless of maxRetries. If set to 0, the client will NOT try to reconnect
         */
        stopRetrying: boolean;
        /**
         * Primarily used when specifying which interface a client should connect through. see the socket.connect documentation in the node.js api https://nodejs.org/api/net.html#net_socket_connect_options_connectlistener
         */
        interfaces: {
            /**
             * Default: false
...
interface RootIPC extends NodeIPC.IPC {
    IPC: new () => NodeIPC.IPC;
}
declare const RootIPC: RootIPC;
export = RootIPC;
