...
declare module "@service.wxpay" {
     ...
     * |返回值|备注|
     * |:-:|:-:|
     * |APP|微信 app 调用方式，服务端向微信支付下单时，trade_type 需要是 APP，参考微信 app 支付|
     * |MWEB|微信网页调用方式，服务端向微信支付下单时，trade_type 需要是 MWEB，参考微信网页支付|
     * |none|微信未安装|
     */
    function getType(): "APP" | "MWEB" | "none";
    ...
    interface MwebPayExtraOptions {
        /**
         * 在微信的支付服务器下单以后，微信返回的 MWEB_URL，在 CP 用于微信支付的 h5 页面中，直接将 mweb_url 取出后跳转过去即可，但这个做法并不是强制的，您也可以通过其他自定义键值向您自己的服务器换取 MWEB_URL。
         *
         * 从 1040 版本开始，若使用直接设置 referer 的方式拉起微信客户端支付，则此参数必选。
         */
        mweb_url?: string;
        /**
