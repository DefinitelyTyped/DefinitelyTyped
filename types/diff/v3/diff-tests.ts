...
function verifyPatchMethods(oldStr: string, newStr: string, uniDiff: jsdiff.IUniDiff) {
    const verifyPatch = jsdiff.parsePatch(
        jsdiff.createTwoFilesPatch('oldFile.ts', 'newFile.ts', oldStr, newStr, 'old', 'new', { context: 1 }),
    );
...
function verifyApplyMethods(oldStr: string, newStr: string, uniDiff: jsdiff.IUniDiff) {
    const verifyApply = [jsdiff.applyPatch(oldStr, uniDiff), jsdiff.applyPatch(oldStr, [uniDiff])];
...
const uniDiffPatch = jsdiff.structuredPatch('oldFile.ts', 'newFile.ts', one, other, 'old', 'new', { context: 1 });
verifyPatchMethods(one, other, uniDiffPatch);

const uniDiffStr = jsdiff.createPatch('file.ts', one, other, 'old', 'new', { context: 1 });
