...
export class OperationOutcomeError extends Error {
    ...
    constructor(outcome: fhir.OperationOutcome) {
        super(
            (outcome.issue && outcome.issue.length > 1 && outcome.issue[0].diagnostics) ||
...
function rejectOnOperationOutcome<ResultType extends fhir.ResourceBase>(
    promise: fhir.OperationOutcome | ResultType | Promise<fhir.OperationOutcome | ResultType>,
): Promise<ResultType> {
...
client.smartAuthMetadata().then((s) => {
    if (s && s.authorizeUrl) {
