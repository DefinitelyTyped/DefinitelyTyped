...
import { channel, createTheming, themeListener, ThemeProvider, withTheme } from 'theming';
...
>(
    component: C
) => React.ComponentType<Omit<JSX.LibraryManagedAttributes<C, PropsOf<C>>, keyof InjectedProps> & AdditionalProps>;
...
export interface CSSProperties<Props> {
    ...
    [k: string]: DynamicCSSRule<Props> | CSSProperties<Props>;
}
...
export type WithSheet<S extends string | Styles | StyleCreator<keyof S, any>, GivenTheme = undefined, Props = {}> = {
    classes: ClassNameMap<
        S extends string | number | symbol
            ? S
            : S extends StyleCreator<infer C, any, Props>
            ? C
            : S extends Styles<infer C, Props>
            ? C
            : never
    >;
