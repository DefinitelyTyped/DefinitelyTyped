...
interface BuildDocFromHashReturn<T extends FirestoreRecord> {
    exists: boolean;
    id: string;
    data(): Omit<T, 'id'>;
}

interface BuildQuerySnapshotReturn<T extends FirestoreRecord> {
    empty: boolean;
...
export class FakeFireStore<T = {}> {
    ...
    where(): this;
    get<R = {}>(): Promise<BuildDocFromHashReturn<R> | { exists: boolean } | BuildQuerySnapshotReturn<R>>;
    getAll<R = {}>(): Promise<Array<R>>;
    batch(): {
        delete(): void;
        set(): void;
        update(): void;
