    ...
    WL.fileDialog({ mode: 'save' }).then(
        function (response) {
            ...
            WL.upload<Microsoft.Live.IFile>({
                ...
                function (response) {
                    log('You saved to ' + response.source + '. ' + 'Below is the result of the upload.');
                    log('');
...
function onGetFilesComplete(response: Microsoft.Live.IObjectCollection<Microsoft.Live.IObject>) {
    ...
    for (var i = 0; i < items.length; i++) {
        if (items[i].type === 'folder') {
            log('Found a folder with the following information: ' + JSON.stringify(items[i]).replace(/,/g, ',\n'));
            foundFolder = 1;
            break;
        }
    }
...
function createContact() {
    ...
    WL.api({
        ...
        function (response) {
            log('Cannot create contact: ' + JSON.stringify(response.error).replace(/,/g, ',\n'));
        },
    );
}
