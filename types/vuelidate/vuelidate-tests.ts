...
const contains = (param: string) => (value: string) => !helpers.req(value) || value.indexOf(param) >= 0;
...
const mustHaveLength = (minLen: number) =>
    helpers.withParams({ type: 'mustHaveLength' }, (value: any) => helpers.len(value) > minLen);
