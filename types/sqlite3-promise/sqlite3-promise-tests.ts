...
function createDb(): sqlite3.Database {
    return new sqlite3.Database(':memory:', sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE);
}
...
async function runExamples() {
    ...
    let sum = 0;
    const eachResult = await db.eachAsync('SELECT rowid AS id, info FROM lorem', (err, row) => {
        sum += row.id;
    });
    ...
    db = new verbose.Database(':memory:', verbose.OPEN_CREATE | verbose.OPEN_READWRITE);
