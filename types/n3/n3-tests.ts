...
function test_doc_rdf_to_triples_1() {
    ...
    parser.parse(
        ...
        (error: Error, quad: RDF.Quad, prefixes: N3.Prefixes) => {
            if (quad) console.log(quad.subject, quad.predicate, quad.object, quad.graph, '.');
...
function test_doc_from_triples_to_rdf_stream() {
    const writer: N3.N3Writer = new N3.Writer(process.stdout, {
        end: false,
...
function test_doc_from_triple_stream_to_rdf_stream() {
    ...
    const streamWriter: N3.N3StreamWriter = new N3.StreamWriter({
        prefixes: { c: N3.DataFactory.namedNode('http://example.org/cartoons#') },
    });
