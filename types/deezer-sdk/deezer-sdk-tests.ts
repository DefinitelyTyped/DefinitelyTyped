...
DZ.Event.subscribe('track_end', (trackPosition) => print('Track end. Position = ' + trackPosition));
DZ.Event.subscribe('player_position', ([positionSecondsFloat, _]) => print('Position = ' + positionSecondsFloat));

DZ.api('/user/123', (response) => print('User name = ' + response.name));
DZ.api('user/me/playlists', 'POST', { title: 'my title' }, (response) => print('My new playlist ID = ' + response.id));
