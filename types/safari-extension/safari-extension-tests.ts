...
function respondToMessage(messageEvent: SafariExtensionMessageEvent) {
    if (messageEvent.name === 'gotIt') document.getElementById('textField').innerHTML = messageEvent.message;
}
...
function performCommand(event: SafariCommandEvent) {
    if (event.command === 'reload-page') {
        ...
        if (currentURL) (<SafariExtensionToolbarItem>event.target).browserWindow.activeTab.url = currentURL;
    }
}
...
function validateCommand(event: SafariValidateEvent) {
    if (event.command === 'reload-page') {
        ...
        (<SafariExtensionToolbarItem>event.target).disabled = !(<SafariExtensionToolbarItem>event.target).browserWindow
            .activeTab.url;
    }
}
