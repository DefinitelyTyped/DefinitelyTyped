...
function ondeviceup(host: string): void {
  ...
  client.connect(host, () => {
    ...
    const connection = client.createChannel("sender-0", "receiver-0",
        "urn:x-cast:com.google.cast.tp.connection", "JSON");
    const heartbeat  = client.createChannel("sender-0", "receiver-0",
        "urn:x-cast:com.google.cast.tp.heartbeat", "JSON");
    const receiver   = client.createChannel("sender-0", "receiver-0",
