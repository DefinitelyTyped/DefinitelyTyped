...
() => httpAssert(false);
() => httpAssert(false, status, options);
() => httpAssert(false, status, message, options);

// Test assertion functions

(foo: any): number => { httpAssert(typeof foo !== 'string'); return foo; };
(foo: any): number => { httpAssert.ok(foo === 3); return foo; };
(foo: any): number => { httpAssert.strictEqual(foo, 3); return foo; };
