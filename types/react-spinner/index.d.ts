...
export default class ReactSplit extends React.Component<SplitWrapper.DefaultProps, any> { }
...
declare namespace SplitWrapper {
  export interface DefaultProps {
    ...
    When the split is created, if expandToMin is true, the minSize for each element overrides the percentage value from the sizes option. Example: The first element (#one) is set to 25% width of the parent container. However, it's minSize is 300px. Using expandToMin: true means that the first element will always load at at least 300px, even if 25% were smaller. */
    expandToMin?: boolean;
    /** Default: 10
    ...
    Possible options are 'start', 'end' and 'center'. Determines how the gutter aligns between the two elements. 'start' shrinks the first element to fit the gutter, 'end' shrinks the second element to fit the gutter and 'center' shrinks both elements by the same amount so the gutter sits between. Added in v1.5.3. */
    ...
    Drag this number of pixels at a time. Defaults to 1 for smooth dragging, but can be set to a pixel value to give more control over the resulting sizes. Works particularly well when the gutterSize is set to the same size. Added in v1.5.3.  */
    dragInterval?: number;
    /**  Default: 'horizontal'

    Direction to split in. Can be 'vertical' or 'horizontal'. Determines which CSS properties are applied (ie. width/height) to each element and gutter */
