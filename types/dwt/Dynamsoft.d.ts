...
import { ISettings } from "./Addon.OCRPro";
import { FileUploader } from "./Dynamsoft.FileUploader";

export declare namespace DynamsoftStatic {
  let Lib: IDynamsoftLib;
  let MSG: IMessages;
  let WebTwainEnv: IWebTwainEnv;
  let managerEnv: IManagerEnv;
  let FileUploader: FileUploader;
  namespace WebTwain {
    namespace Addon {
      namespace OCRPro {
        function NewSettings(): ISettings;
      }
    }
  }
}
export interface IDWTInitialConfig {
  WebTwainId: string;
  Host?: string;
  Port?: string;
  SSLPort?: string;
}
export interface IDynamsoftLib {
  /**
...
export interface IWebTwainEnv {
   ...
   * @param failureCallback A callback function that is executed if the request fails.
   */
  CreateDWTObjectEx(dwtInitialConfig: IDWTInitialConfig, successCallBack: (DWObject: WebTwain) => void, failureCallBack: (errorString: string) => void): void;
  /**
   * Define the display info.
   */
  CustomizableDisplayInfo: IDisplayInfo;
  /**
