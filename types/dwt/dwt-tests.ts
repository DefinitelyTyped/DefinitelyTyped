...
function editImage() {
    const DWObject = Dynamsoft.WebTwainEnv.GetWebTwain('dwtcontrolContainer');
    if (DWObject) {
        if (DWObject.HowManyImagesInBuffer > 0) DWObject.RotateLeft(DWObject.CurrentImageIndexInBuffer);

        if (DWObject.HowManyImagesInBuffer > 0) DWObject.RotateRight(DWObject.CurrentImageIndexInBuffer);

        if (DWObject.HowManyImagesInBuffer > 0) DWObject.Mirror(DWObject.CurrentImageIndexInBuffer);

        if (DWObject.HowManyImagesInBuffer > 0) DWObject.Flip(DWObject.CurrentImageIndexInBuffer);
    }
}
...
function saveImage() {
    ...
    if (DWObject) {
        ...
        DWObject.SaveAllAsMultiPageTIFF(
            'DynamicWebTWAIN.tiff',
            () => {},
            (errorCode: number, errorString: string) => {}
        );
