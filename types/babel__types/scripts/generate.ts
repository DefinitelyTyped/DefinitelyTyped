...
output.push("");
output.push("export declare const TYPES: (NodeName | AliasName)[];");
output.push("");
output.push("export declare const ALIAS_KEYS: Record<NodeName, AliasName[]>;");
...
output.push("export declare const VISITOR_KEYS: Record<NodeName, string[]>;");
NodeNames.forEach(name => {
    assert(
...
output.push("export declare const BUILDER_KEYS: Record<NodeName, string[]>;");
NodeNames.forEach(name => {
    assert(
...
const ConstantUnions = [
    ...
    output.push("export declare const " + unionName + ": " + typeName + "[];");
    return {
...
output.push(
    "export declare const INHERIT_KEYS: " +
        JSON.stringify(types.INHERIT_KEYS) +
        ";"
);
output.push("export declare const BLOCK_SCOPED_SYMBOL: unique symbol;");
assert(typeof types.BLOCK_SCOPED_SYMBOL === "symbol");
output.push("export declare const NOT_LOCAL_BINDING: unique symbol;");
assert(typeof types.NOT_LOCAL_BINDING === "symbol");

// ["NODE_FIELDS"].forEach(exportName => {
//     output.push("export declare const " + exportName + ": {");
//     Object.keys(types[exportName]).forEach(name => {
//         output.push(
//             "    " +
//                 name +
//                 ": " +
//                 (Array.isArray(types[exportName][name])
//                     ? types[exportName][name].length
//                       ? JSON.stringify(types[exportName][name])
//                       : "string[]"
//                     : JSON.stringify(types[exportName][name])) +
//                 ","
//         );
...
AliasNames.forEach(type => {
    ...
    output.push(
        `export declare const ${type.toUpperCase()}_TYPES: NodeName[];`
    );
});
...
NodeNames.sort().forEach(name => {
    ...
    if (reservedNames.includes(n)) {
        ...
        output.push(`export declare function ${n}${definition}`);
        if (/^TS/.test(name)) {
...
output.push("");
output.push(`export declare function isNode(node: object): node is BabelNode;`);
assert(typeof types.isNode === "function");
types.TYPES.filter(name => !types.DEPRECATED_KEYS[name])
    .sort()
    .forEach(name => {
        output.push(
            `export declare function is${name}(node?: object, opts?: object): node is ${name};`
        );
        assert(typeof types[`is${name}`] === "function");
    });
output.push("");
output.push(`export declare function assertNode(node: object): void;`);
assert(typeof types.assertNode === "function");
types.TYPES.filter(name => !types.DEPRECATED_KEYS[name])
    .sort()
    .forEach(name => {
        output.push(
            `export declare function assert${name}(node: object, opts?: object): void;`
        );
        assert(typeof types[`assert${name}`] === "function");
    });
output.push("");

fs.writeFileSync(__dirname + "/../generated.d.ts", output.join("\n"));
...
Object.keys(types)
    ...
    .forEach(name => {
        testOutput.push(`import {${name}} from '@babel/types';`);
    });
testOutput.push("");
