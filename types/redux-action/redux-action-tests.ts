...
actionCreator0(1)(dispatch, () => {}).then((action) => {
...
actionCreator1(1)(dispatch, () => {}).then((action) => {
    const type: string = action.type;
    const payload: Payload = action.payload;
});
...
actionCreator2(1)(dispatch, () => {}).then((action) => {
    const type: string = action.type;
...
actionCreator3(1)(dispatch, () => {}).then((action) => {
    const type: string = action.type;
    const payload: string = action.payload;
});
...
actionCreator4(1)(dispatch, () => {}).then((action) => {
    const type: string = action.type;
    const payload: string = action.payload;
});
...
actionCreator5('items')(dispatch, () => {}).then((action) => {
    const type: string = action.type;
...
actionCreator6('items', false)(dispatch, () => {}).then((action) => {
    const type: string = action.type;
    const payload: number = action.payload;
});
...
const reducer = ReduxAction.createReducer<State>(
    { name: 'name' },
    {
        'get items': () => {
