...
() => {
    ...
    const Stateless = connectStateResults(({ searchResults, additionalProp }) => ( // $ExpectError
        <div>
...
() => {
    ...
    const CustomHighlight = connectHighlight<MyDoc>(({ highlight, attribute, hit }) => {
        ...
        return (
            <>
                {highlights.map((part) => (part.isHighlighted ? <mark>{part.value}</mark> : <span>{part.value}</span>))}
            </>
        );
    });
