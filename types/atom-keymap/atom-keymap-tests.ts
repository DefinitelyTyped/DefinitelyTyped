...
// Event Subscription
sub = manager.onDidMatchBinding(
    (event): void => {
        event.keystrokes;
    }
);
sub = manager.onDidPartiallyMatchBindings(
    (event): void => {
        event.partiallyMatchedBindings;
    }
);
sub = manager.onDidFailToMatchBinding(
    (event): void => {
        event.keystrokes;
    }
);
sub = manager.onDidFailToReadFile(
    (event): void => {
        event.stack;
    }
);
