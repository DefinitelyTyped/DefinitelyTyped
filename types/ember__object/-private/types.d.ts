...
export type ExtractPropertyNamesOfType<T, S> = { [K in keyof T]: T[K] extends S ? K : never }[keyof T];
...
export type UnwrapComputedPropertyGetters<T> = { [P in keyof T]: UnwrapComputedPropertyGetter<T[P]> };

export type UnwrapComputedPropertySetter<T> = T extends ComputedPropertyMarker<any, infer V> ? V : T;
export type UnwrapComputedPropertySetters<T> = { [P in keyof T]: UnwrapComputedPropertySetter<T[P]> };
