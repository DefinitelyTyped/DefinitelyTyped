...
import { default as persistState, mergePersistedState, transformState, actionTypes } from 'redux-localstorage';
...
const createStoreAsyncStorage = compose(persistState(storageAsyncStorage, 'foo'))(createStore)(reducer);

const createStoreLocalStorage = compose(persistState(storageLocalStorage, 'foo'))(createStore)(reducer);

const createStoreSessionStorage = compose(persistState(storageSessionStorage, 'foo'))(createStore)(reducer);

const storage = compose(transformState([JSON.stringify, btoa], [atob, JSON.parse]))(storageLocalStorage);
