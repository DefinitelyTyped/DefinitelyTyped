function test_cordova_plugin_ms_adal() {
    ...
    Microsoft.ADAL.AuthenticationContext.createAsync(testConfiguration.authority).then(
        (context) => {
            context.tokenCache.readItems().then((cacheItems) => {
                if (cacheItems.length >= 1) {
                    ...
                    context
                        .acquireTokenSilentAsync(testConfiguration.resourceUrl, testConfiguration.clientId, testUserId)
                        .then(
                            (authResult) => {
