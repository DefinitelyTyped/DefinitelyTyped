...
function test_table2() {
    ...
    ] as any[]).forEach((col: sql.IColumn) =>
...
function test_promise_returns() {
    ...
    connection
        .batch<Entity>('create procedure #temporary as select * from table;select 1 as value')
        .then((recordset) => {});
    ...
    request.batch<
        Entity
    >`create procedure #temporary as select * from table;select ${1} as value`.then((recordset) => {});
...
async function test_msnodesqlv8() {
    const connection = new msnodesqlv8.ConnectionPool({
        server: 'localhost',
        database: 'master',
