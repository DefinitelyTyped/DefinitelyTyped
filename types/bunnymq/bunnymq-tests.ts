...
// RPC Support
instance
    .publish('queue:name', { message: 'content' }, { routingKey: 'my-routing-key', rpc: true, timeout: 1000 })
    .then((consumerResponse: string) => 'response');
instance.producer.produce(
    'queue:name',
    { message: 'content' },
    { rpc: true, routingKey: 'my-routing-key', timeout: 1000 },
);
