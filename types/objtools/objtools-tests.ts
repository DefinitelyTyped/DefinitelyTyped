...
let maskOrTrue: ObjectMask | true = ObjectMask.addMasks({ foo: true }, { _: true, bar: false });
maskOrTrue = ObjectMask.addMasks(new ObjectMask({ foo: true }), new ObjectMask({ _: true, bar: false }));

mask = ObjectMask.subtractMasks({ foo: true }, { _: true, bar: false });
mask = ObjectMask.subtractMasks(new ObjectMask({ foo: true }), new ObjectMask({ _: true, bar: false }));
...
let maskOrFalse: ObjectMask | false = ObjectMask.andMasks({ foo: true }, { _: true, bar: false });
maskOrFalse = ObjectMask.andMasks(new ObjectMask({ foo: true }), new ObjectMask({ _: true, bar: false }));
...
const filter: (obj: Record<string, any>) => Record<string, any> = mask.createFilterFunc();
...
obj = objtools.syncObject(
    ...
    { foo: 'bar' },
    {
        onField(field: string, toVal: any, fromVal: any, parentObj: Record<string, any>): boolean {
            return false;
        },
        onChange(): void {},
    },
...
const customizer: objtools.MergeCustomizer = (objectValue: any, sourceValue: any, key: string, object: any) => {};
...
const dottedDiff: string[] = objtools.dottedDiff({ foo: 'bar' }, { foo: 'baz' });
