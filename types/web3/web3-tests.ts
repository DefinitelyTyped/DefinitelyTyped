...
const sendSignedTransactionTxReceipt1: PromiEvent<TransactionReceipt> = web3.eth
    .sendSignedTransaction('')
    .on('transactionHash', (txHash: string) => {});
const sendSignedTransactionTxReceipt2: PromiEvent<TransactionReceipt> = web3.eth
    .sendSignedTransaction('')
    .on('receipt', (txReceipt: TransactionReceipt) => {
        const { status }: { status: boolean } = txReceipt;
    });
const sendSignedTransactionTxReceipt3: PromiEvent<TransactionReceipt> = web3.eth
    .sendSignedTransaction('')
...
const sendTransactionTxReceipt0: PromiEvent<TransactionReceipt> = web3.eth.sendTransaction(
    { to: '0x1' },
    (error: Error, txHash: string) => {}
);
const sendTransactionTxReceipt1: PromiEvent<TransactionReceipt> = web3.eth
    .sendTransaction({ to: '0x1' })
    .on('transactionHash', (txHash: string) => {});
const sendTransactionTxReceipt2: PromiEvent<TransactionReceipt> = web3.eth
    .sendTransaction({ to: '0x1' })
    .on('receipt', (txReceipt: TransactionReceipt) => {});
const sendTransactionTxReceipt3: PromiEvent<TransactionReceipt> = web3.eth
    .sendTransaction({ to: '0x1' })
    .on('confirmation', (confNumber: number, receipt: TransactionReceipt) => {});
