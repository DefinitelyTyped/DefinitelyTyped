...
const storageWait = compose(debounce(100))(adapter(window.localStorage));
const enhancerWait = persistState(storageWait, 'test');

const storageMax = compose(debounce(100, 200))(adapter(window.localStorage));
const enhancerMax = persistState(storageMax, 'test');

const storageOpts = compose(debounce(100, { maxWait: 200, foo: 'bar' }))(adapter(window.localStorage));
