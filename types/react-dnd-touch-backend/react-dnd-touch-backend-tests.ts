...
const dndComponentDelayMouseStart = createDragDropManager(
    TouchBackend({ enableMouseEvents: true, delayMouseStart: 100 }),
    context
);
...
const dndComponentAllCurrentEvents = createDragDropManager(
    TouchBackend({ enableKeyboardEvents: true, enableMouseEvents: true, delayMouseStart: 100, delayTouchStart: 200 }),
    context
);
const dndComponentWithScrollAngleRanges = createDragDropManager(
    TouchBackend({ scrollAngleRanges: [{ start: 0, end: 0 }, { start: 0 }, { end: 0 }] }),
    context
);
