...
// An EditOp's `op` is defined entirely by the document's OT type, so its shape
// varies: an array for json0/text ops, or an object for OT types whose ops
// serialize to a delta (e.g. rich-text / quill-delta, sent as `{ ops: [...] }`).
// It is therefore typed `any` rather than `any[]`.
