...
myApp.config((RestangularProvider: restangular.IProvider) => {
    ...
    RestangularProvider.setFullRequestInterceptor(function(
        element,
        operation,
        route,
        url,
...
myApp.controller('TestCtrl', ($scope: MyAppScope, Restangular: restangular.IService) => {
    ...
    baseAccounts.getList().then(
        function(accounts) {
            ...
            firstAccount.post('Buildings', myBuilding).then(
                function() {
