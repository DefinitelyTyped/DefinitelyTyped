import { BotsInfoResponse, MessageMetadata, UsersInfoResponse, UsersListResponse, ChatPostMessageArguments, WebAPICallResult, ConversationsInfoResponse } from '@slack/web-api';
import { Adapter, User as HubotUser, Robot, Message, TextMessage, Envelope } from 'hubot';
...
declare namespace HubotSlack {
    ...
    class ReactionMessage extends Message {
        constructor(type: string, user: HubotUser, reaction: string, item_user: string, item: { type: 'message', channel: string, ts: string }, event_ts: string);
    }
    ...
    class SlackTextMessage extends TextMessage {
        ...
        constructor(user: HubotUser, text: string | undefined, rawText: string | undefined, rawMessage: RawSlackMessage, reaction: string, item_user: string, item: { type: 'message', channel: string, ts: string }, event_ts: string);
        ...
        makeSlackTextMessage(user: HubotUser, text: string | undefined, rawText: string | undefined, rawMessage: RawSlackMessage, channel_id: string, robot_name: string, robot_alias: string, client: SlackClient, cb: (result: unknown | SlackTextMessage) => void): SlackTextMessage;
    }
    ...
    class SlackClient {
        ...
        private updateUserInBrain(eventOrUser: { type: 'user_change', user: UsersInfoResponse['user'] } | UsersInfoResponse['user']): void;
