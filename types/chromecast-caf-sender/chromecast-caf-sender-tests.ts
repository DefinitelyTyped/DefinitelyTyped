...
context.removeEventListener(cast.framework.CastContextEventType.SESSION_STATE_CHANGED, handleSessionStateChange);
...
context.requestSession().then((err) => {
    ...
    session.getActiveInputState() === cast.framework.ActiveInputState.ACTIVE_INPUT_STATE_UNKNOWN;
    ...
    const req = new chrome.cast.media.LoadRequest(new chrome.cast.media.MediaInfo('id', 'type'));
    ...
    session.addEventListener(cast.framework.SessionEventType.ACTIVE_INPUT_STATE_CHANGED, ({ activeInputState }) => {});
    session.addEventListener(cast.framework.SessionEventType.APPLICATION_METADATA_CHANGED, ({ metadata }) => {});
    session.addEventListener(cast.framework.SessionEventType.APPLICATION_STATUS_CHANGED, ({ status }) => {});
    session.addEventListener(cast.framework.SessionEventType.MEDIA_SESSION, ({ mediaSession }) => {});
    session.addEventListener(cast.framework.SessionEventType.VOLUME_CHANGED, ({ volume }) => {});
    session.removeEventListener(cast.framework.SessionEventType.VOLUME_CHANGED, ({ volume }) => {});
});
...
playerCtrl.addEventListener(cast.framework.RemotePlayerEventType.ANY_CHANGE, () => {});
playerCtrl.removeEventListener(cast.framework.RemotePlayerEventType.ANY_CHANGE, () => {});
