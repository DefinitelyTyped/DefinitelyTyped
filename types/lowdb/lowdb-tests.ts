...
const result: ArrayLike<Post> = db.get('posts').push({ title: 'hello', views: 123 }).value();
...
low(adapterAsync).then((dbAsync) => {
    ...
    const test: Promise<Post | undefined> = dbAsync.get('posts').find({}).write();
...
async () => {
    ...
    const adapterAsync: low.AdapterAsync<ExampleSchema> = new FileAsync('db.json', {
        defaultValue: {
            posts: [{ name: 'baz' }],
        },
    });
    ...
    const xSync: ExampleSchema = dbSync.defaults({ posts: [{ name: 'baz' }] }).write();

    const xAsync: Promise<ExampleSchema> = dbAsync.defaults({ posts: [{ name: 'baz' }] }).write();
    ...
    const result2: Promise<OtherSchema['nested']['x']> = db.get('nested').get('x').write();
};

declare const lodashChain: _.ObjectChain<ExampleSchema>;

// let's also ensure we didn't break lodash.chain through extension
const weDidNotBreakLodash: ArrayLike<{ name: string }> = lodashChain.get('posts').sortBy('').value();
...
() => {
    ...
    // Find post by id
    const post: Post | undefined = postsFP(
        find<Post>({ id: 1 }),
    );

    // Find top 5 fives posts
    const popular: Post[] = postsFP([sortBy<Post>('views') as PostsAction, take(5) as PostsAction]);

    const filtered: Post[] = dbFP('posts')(
        filter<Post>({ published: true }),
    );
    const writeAction: Post[] = dbFP('posts').write(
        concat<Post>({ id: 123 }),
    );
    ...
    async () => {
        ...
        const func: Promise<Post[]> = postsWithDefault.write((post) => [...post, { title: 'another' }]);
    };
