...
bananas.updateByExample(bananas.any(), { shape: { type: 'round' } }, { mergeObjects: true });
bananas.ensureIndex({
    type: 'geo',
    fields: ['latLng'],
});
...
router.use((req, res, next) => {
    if (!req.auth || !req.auth.basic) {
        res.throw(401);
    } else if (req.auth.basic.username !== 'admin' || req.auth.basic.password !== 'hunter2') {
        res.throw(403);
    }
    next();
});
...
console.log(
    query`
        FOR u IN users
        ${aql.literal(Math.random() < 0.5 ? 'FILTER u.admin' : 'FILTER !u.admin')}
        RETURN u
    `.toArray(),
);
