...
Model.populate([{ name: 'User hoge' }, { name: 'User fuga' }], { path: 'weapon' }, (err: any, users: IActor[]) => {});
...
query3
    .where('loc')
    .within()
    .geometry({ type: 'Polygon', coordinates: [[[10, 20], [10, 40], [30, 40], [30, 20]]] });
query3
    .find()
    .where('age')
    .gt(21);
...
query3
    .where('path')
    .intersects()
    .geometry({ type: 'LineString', coordinates: [[180.0, 11.0], [180, 9.0]] });
