// Type definitions for geojson-vt 3.2.0
...
export interface GeoJSONVTInterface {
...
declare const enum FeatureTypes {
    Unknown = 0,
    Point,
    Linestring,
    Polygon,
...
export type GeoJSONTiles = Record<`${number}`, GeoJSONTile>;
export interface TileCoord {
    x: number;
    y: number;
    z: number;
}
export type GeoJsonVTData = GJ.GeoJSON;
export type GeoJSONVTOptions = GeoJSONVTInterface;
declare class GeoJSONVT {
    constructor(data: GeoJsonVTData, options: GeoJSONVTOptions);
     ...
     */
    splitTile(features: GeoJSONTile, z: number, x: number, y: number, cz: number, cx: number, cy: number): void;
     ...
     */
    getTile(z: number | string, x: number | string, y: number | string): null | GeoJSONTile;
}

declare function geojsonvt(data: GeoJsonVTData, options: GeoJSONVTOptions): GeoJSONVT;
