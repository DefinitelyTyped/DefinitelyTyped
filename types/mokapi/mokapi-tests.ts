...
// @ts-expect-error
on("http", () => {})
on("http", (): boolean => { return false })
// @ts-expect-error
on("http", (s: string): boolean => { return false })
on("http", (req: HttpRequest, res: HttpResponse): boolean => { return false })
on("kafka", (): boolean => { return false })
// @ts-expect-error
on("kafka", (s: string): boolean => { return false })
on("kafka", (r: KafkaRecord): boolean => { return false })
on("ldap", (): boolean => { return false })
// @ts-expect-error
on("ldap", (s: string): boolean => { return false })
on("ldap", (req: LdapSearchRequest, res: LdapSearchResponse): boolean => { return false })
// @ts-expect-error
on("http", (): boolean => { return false }, "")
on("http", (): boolean => { return false }, {})
on("http", (): boolean => { return false }, { tags: { foo: "bar" }})
...
// @ts-expect-error
h = (s: LdapSearchRequest) => { return false }
h = (req: HttpRequest) => { return false }
// @ts-expect-error
h = (req: HttpRequest, s: string) => { return false }
h = (req: HttpRequest, res: HttpResponse) => { return false }
