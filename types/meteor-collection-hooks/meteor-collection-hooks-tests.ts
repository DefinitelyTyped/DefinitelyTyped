...
Collection.before.find(
    (userId: string, selector: Mongo.Selector, options: { multi?: boolean; upsert?: boolean }) => {},
);
...
Collection.after.find(
    (
        ...
        selector: Mongo.Selector,
        options: { multi?: boolean; upsert?: boolean },
        cursor: Mongo.Cursor<Model>,
    ) => {},
);
Collection.after.findOne(
    (userId: string, selector: Mongo.Selector, options: { multi?: boolean; upsert?: boolean }, doc: Model) => {},
);
...
cursor = Collection.direct.find(
    { _id: doc._id },
    { sort: { createdAt: -1 }, skip: 5, limit: 5, fields: { createdAt: 1 }, reactive: true, transform: () => {} },
);
...
doc = Collection.direct.findOne(
    { _id: doc._id },
    { sort: { createdAt: -1 }, skip: 5, fields: { createdAt: 1 }, reactive: true, transform: () => {} },
);
...
let { numberAffected, insertedId }: { numberAffected?: number; insertedId?: string } = Collection.direct.upsert(
    doc._id,
    { $inc: { votes: 2 } },
    { multi: true },
    () => {},
