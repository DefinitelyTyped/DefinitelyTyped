...
declare namespace jwplayer {
    ...
    interface JWPlayer {
        ...
        playAd(tag: string): JWPlayer;
        playlistItem(index: number): JWPlayer;
        registerPlugin(id: string, target: string, jsPlugin: () => void, swfURL?: string): JWPlayer;
        remove(): JWPlayer;
        removeButton(id: string): JWPlayer;
        resize(width: number, height: number): JWPlayer;
        seek(position: number): JWPlayer;
        setControls(controls: boolean): JWPlayer;
        setCurrentAudioTrack(index: number): JWPlayer;
        setCurrentCaptions(index: number): JWPlayer;
        setCurrentQuality(index: number): JWPlayer;
        setPlaybackRate(rate: number): JWPlayer;
        setFullscreen(state: boolean): JWPlayer;
