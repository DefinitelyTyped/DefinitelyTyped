...
puppeteer.launch().then(async (browser) => {
    ...
    // of the page class.
    const html = await page.evaluate((body: HTMLElement) => body.innerHTML, bodyHandle);
});
...
puppeteer.launch().then(async (browser) => {
    ...
    await page.exposeFunction('md5', (text: string) => crypto.createHash('md5').update(text).digest('hex'));
    ...
    page.waitForSelector('img', { visible: true }).then(() =>
        console.log('First URL with image by selector: ' + currentURL),
    );

    page.waitForXPath('//img', { visible: true }).then(() =>
        console.log('First URL with image by xpath: ' + currentURL),
    );
...
(async () => {
    ...
    if (!button) throw new Error('Unable to select myButton');

    if (!input) throw new Error('Unable to select myInput');
...
(async () => {
    ...
    await page.waitFor(
        (stuff: string) => !!document.querySelector(stuff),
        {
            hidden: true,
        },
        'asd',
...
async () => {
    ...
    const page = await browser.newPage();
    await page
        .evaluate(() => Promise.resolve(document.body.innerHTML))
        .then((s) => {
            console.log('body html has length', s.length);
        });
};
...
async () => {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    const link: puppeteer.JSHandle = await page.evaluateHandle(() => document.body.querySelector('a'));
    const linkEl: puppeteer.ElementHandle | null = link.asElement();
    if (linkEl !== null) {
        const href = await page.evaluate((el: HTMLElement): string | null => el.getAttribute('href'), linkEl);
...
async () => {
    ...
    const paragraphContents: string[] = await page.$$eval('p', (ps: Element[]): string[] =>
...
async () => {
    ...
    const reHandle: puppeteer.JSHandle = await page.evaluateHandle(() => /\s*bananas?\s*/i);
    const numMatchingEls: number = await page.$$eval(
        'p',
        (els: Element[], re: RegExp) => els.filter((el) => el.textContent && re.test(el.textContent)).length,
        reHandle,
    );
