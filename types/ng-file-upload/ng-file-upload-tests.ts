...
class UploadController {
    ...
    onFileSelect(files: Array<File>) {
        ...
        this.Upload.base64DataUrl(files[0]).then((file: string) => {
            console.log(file);
        });

        this.Upload.dataUrl(files[0], true).then((result: string) => {
            console.log(result);
        });

        this.Upload.imageDimensions(files[0]).then((imageDimensions) => {
