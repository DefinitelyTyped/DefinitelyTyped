...
import { Emitter, Event, Observable, Pool, Property, Stream } from "kefir";
...
// Modify an observable
{
    ...
    let observable09: Stream<number, void> = Kefir.sequentially(100, [1, 2, 2.1, 3, 1]).skipDuplicates((a, b) =>
    ...
    let observable16: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3, 0, 0, 0, 4, 5, 6]).filter(x => x > 0)
        .debounce(250);
    let observable17: Stream<void, number> = Kefir.sequentially(100, [0, -1, 2, -3]).valuesToErrors<number>(x => {
        return { convert: x < 0, error: x * 2 };
    });
    let observable18: Stream<number, void> = Kefir.sequentially(100, [0, -1, 2, -3]).valuesToErrors().errorsToValues<
        number
    >((x: number) => {
        return { convert: x >= 0, value: x * 2 };
    });
    let observable19: Stream<void, number> = Kefir.sequentially(100, [0, 1, 2, 3]).valuesToErrors().mapErrors((
        x: number,
    ) => x * 2);
    let observable20: Stream<void, number> = Kefir.sequentially(100, [0, 1, 2, 3]).valuesToErrors().filterErrors((
        x: number,
    ) => (x % 2) === 0);
...
// Combine observables
{
    ...
    let observable10: Stream<number, void> = Kefir.sequentially(100, [1, 2]).valuesToErrors().flatMapErrors(x =>
        Kefir.interval(40, x).take(2)
    );
...
{
    {
        ...
        let bar: Property<boolean, void> = Kefir.sequentially(200, [false, true, false]).delay(40).toProperty(() =>
            true
        );
