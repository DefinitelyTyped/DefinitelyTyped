...
//Create a stream
{
    ...
    let stream08: Stream<number, Error> = Kefir.fromNodeCallback<number, Error>(callback =>
...
// Convert observables
{
    ...
    let stream: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3])
        .toProperty(() => 0)
        .changes();
}
...
// Subscribe / add side effects
{
    ...
    Kefir.sequentially(1000, [1, 2])
        .valuesToErrors()
        .onValue(x => console.log('error:', x));
    Kefir.sequentially(1000, [1, 2])
        .valuesToErrors()
        .offValue(x => console.log('error:', x));
    ...
    Kefir.sequentially(1000, [1, 2])
        .toPromise()
        .then((x: number) => console.log('fulfilled with:', x));
    ...
    Kefir.sequentially(1000, [1, 2]).observe();
    const subscription = Kefir.sequentially(1000, [1, 2]).observe(_ => {}, _ => {}, () => {});
...
// Modify an observable
{
    ...
    let observable09: Stream<number, void> = Kefir.sequentially(100, [1, 2, 2.1, 3, 1]).skipDuplicates(
    ...
    let observable13: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3, 4]).flatten<number>(x =>
        x % 2 === 0 ? [x * 10] : []
    );
    ...
    let observable16: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3, 0, 0, 0, 4, 5, 6])
        .filter(x => x > 0)
        .debounce(250);
    let observable17: Stream<void, number> = Kefir.sequentially(100, [0, -1, 2, -3]).valuesToErrors<number>(x => {
        return { convert: x < 0, error: x * 2 };
    });
    let observable18: Stream<number, void> = Kefir.sequentially(100, [0, -1, 2, -3])
        .valuesToErrors()
        .errorsToValues<number>((x: number) => {
            return { convert: x >= 0, value: x * 2 };
        });
    let observable19: Stream<void, number> = Kefir.sequentially(100, [0, 1, 2, 3])
        .valuesToErrors()
        .mapErrors((x: number) => x * 2);
    let observable20: Stream<void, number> = Kefir.sequentially(100, [0, 1, 2, 3])
        .valuesToErrors()
        .filterErrors((x: number) => x % 2 === 0);
    let observable21: Stream<void, number> = Kefir.sequentially(100, [0, -1, 2, -3])
        .valuesToErrors(x => {
            return { convert: x < 0, error: x };
        })
        .endOnError();
    let observable22: Stream<void, number> = Kefir.sequentially(100, [0, -1, 2, -3])
        .valuesToErrors(x => {
            return { convert: x < 0, error: x };
        })
        .ignoreValues();
    let observable23: Stream<void, void> = Kefir.sequentially(100, [0, -1, 2, -3])
        .valuesToErrors(x => {
            return { convert: x < 0, error: x };
        })
        .ignoreErrors();
    ...
    let observable31: Stream<number[], void> = Kefir.sequentially(100, [1, 2, 3, 4, 5]).bufferWithTimeOrCount(330, 10);
    {
        ...
        let observable28: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3, 4, 5, 6]).transduce<number>(
            myTransducer
        );
    }
    let observable28: Stream<number | string, void> = Kefir.sequentially(100, [0, 1, 2, 3]).withHandler<
        number | string,
        void
    >((emitter: Emitter<string | number, void>, event: Event<number, void>) => {
        if (event.type === 'end') {
            emitter.emit('bye');
            emitter.end();
        }
    ...
    type First = 'first';
    type Second = 'second';
    let observable32: Stream<First, void> = Kefir.sequentially<First | Second>(100, ['first', 'second']).filter(
        (value): value is First => value === 'first'
    );
}
...
// Combine observables
{
    ...
    }
    {
        ...
        let observable02: Observable<number, void> = Kefir.combine<number, void, number>(
            [a, b],
            [c],
            (a, b, c) => a + b + c
        );
    }
    {
        ...
        let c: Property<number, void> = Kefir.sequentially(100, [8, 9])
            .delay(260)
            .toProperty(() => 7);
    ...
    let observable05: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3]).flatMap(x =>
        Kefir.interval(40, x).take(4)
    );
    let observable06: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3]).flatMapLatest(x =>
        Kefir.interval(40, x).take(4)
    );
    let observable07: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3]).flatMapFirst(x =>
        Kefir.interval(40, x).take(4)
    );
    let observable08: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3]).flatMapConcat(x =>
        Kefir.interval(40, x).take(4)
    );
    let observable09: Stream<number, void> = Kefir.sequentially(100, [1, 2, 3]).flatMapConcurLimit(
        x => Kefir.interval(40, x).take(6),
        2
    );
    let observable10: Stream<number, void> = Kefir.sequentially(100, [1, 2])
        .valuesToErrors()
        .flatMapErrors(x => Kefir.interval(40, x).take(2));
}
...
{
    {
        ...
        let bar: Property<boolean, void> = Kefir.sequentially(200, [false, true, false])
            .delay(40)
            .toProperty(() => true);
    ...
    }
    {
        ...
        let b: Stream<number, void> = Kefir.interval(100, 0)
            .delay(40)
            .take(5);
