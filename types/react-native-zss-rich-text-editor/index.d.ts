...
declare module "react-native-zss-rich-text-editor" {
    ...
    type FunctionWithZeroArgs = () => void;

    type LinkHandler = (url: string, title: string) => void;

    type ImageHandler = (attributes: ImageProps) => void;

    type ColorHandler = (color: string) => void;

    type LinkDialogHandler = (optionalTitle: string, optionalUrl: string) => void;

    type PlaceHolderHandler = (placeholder: string) => void;

    type ContentStylesHandler = (styles: RichTextStyles) => void;

    type ContentSetHandler = (html: string) => void;

    type ContentGetHandler = () => Promise<string>;

    type FocusHandler = (callback: FunctionWithZeroArgs) => void;
    ...
    // or cursor position:
    interface RichTextEditorReference {
    ...
    interface RichTextToolbarOptionalProps {
    ...
    interface RichTextToolbarProps extends Partial<RichTextToolbarOptionalProps> {
        // Must provide a function that returns a ref to a RichTextEditor component.
        getEditor: () => RichTextEditorReference;
    }

    /* tslint:disable:max-classes-per-file */
    class RichTextEditor extends Component<Partial<RichTextEditorProps>>{ }
    ...
    class RichTextToolbar extends Component<Partial<RichTextToolbarProps>>{ }
