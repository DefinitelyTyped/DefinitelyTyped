...
var friendCount = 0;
oboe('friends.json').path('friend', function (name) {
    friendCount++;
});

oboe('resource.json').on('done', function (parsedJson) {
    console.log('Request complete', parsedJson);
});

oboe('resource.json').on('start', function (status, headers) {
    console.log('Resource cached for', headers.Age, 'secs');
});

oboe('/content').fail(function (errorReport) {
    if (404 == errorReport.statusCode) {
        console.error('no such content');
    }
});

oboe('friends.json').node('friend', function (parsedJson) {
    console.log('friend parsed', parsedJson);
});

oboe('friends.json').node({
