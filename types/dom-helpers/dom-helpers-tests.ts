...
const win = helpers.ownerWindow(element || null || undefined) || ownerWindow(element || null || undefined);
...
const match: boolean = query.matches(element, '.class') || matches(element, '.class');
const h: number = query.height(element) || height(element);
const w: number = query.width(element) || width(element);
const _offset: { top: number; left: number; bottom: number; right: number } = query.offset(element) || offset(element);
const _offsetParent: Element = query.offsetParent(element) || offsetParent(element);
const _position: { top: number; left: number; bottom: number; right: number } =
    query.position(element) || position(element);
const _contains: boolean = query.contains(element, element) || contains(element, element);
const _scrollParent: Element = query.scrollParent(element) || scrollParent(element);
...
const _querySelectorAll: HTMLElement[] =
    (query.querySelectorAll(element, '*') as HTMLElement[]) || (querySelectorAll(element, '*') as HTMLElement[]);
const _closest: Element = query.closest(element, '*', element) || closest(element, '*', element);

const _getComputedStyle: string = getComputedStyle(element).getPropertyValue('padding');
