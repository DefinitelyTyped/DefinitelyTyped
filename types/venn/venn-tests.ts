...
vennDiagram.orientationOrder((a: { setid: string }, b: { setid: string }) => a['setid'].localeCompare(b['setid']));

function lossFunction(sets: { [key: string]: venn.Circle }, overlaps: venn.Overlap[]): number {
    let output = 0;
    for (const area of overlaps) {
        ...
        if (area.sets.length === 1) {
            ...
            if (area.sets.length === 2) {
                ...
                overlapSize = venn.circleOverlap(left.radius, right.radius, venn.distance(left, right));
            } else {
                overlapSize = venn.intersectionArea(area.sets.map((i: string) => sets[i]));
            }
        }
...
vennDiagram.layoutFunction((areas: venn.Area[], parameters: venn.LayoutParameter) => {
