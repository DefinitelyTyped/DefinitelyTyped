...
declare module '@mapbox/s3urls' {
    function fromUrl(url: string): { Bucket: string | undefined; Key: string | undefined };
    ...
    function convert(url: string, to: 's3' | 'bucket-in-path' | 'bucket-in-host'): string;

    function signed(url: string, expires: number, cb: (err: Error | undefined, url: string) => void): void;
