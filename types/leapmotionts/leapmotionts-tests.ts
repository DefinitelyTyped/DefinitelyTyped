...
controller.addEventListener(Leap.LeapEvent.LEAPMOTION_FRAME, (event: Leap.LeapEvent) => {
    ...
    console.log(
        "Frame id: " + frame.id + ", timestamp: " + frame.timestamp + ", hands: " + frame.hands.length + ", fingers: "
            + frame.fingers.length + ", tools: " + frame.tools.length + ", gestures: " + frame.gestures().length,
    );
    ...
    if (frame.hands.length > 0) {
        ...
        console.log(
            "Hand pitch: " + Leap.LeapUtil.toDegrees(direction.pitch) + " degrees, " + "roll: "
                + Leap.LeapUtil.toDegrees(normal.roll) + " degrees, " + "yaw: " + Leap.LeapUtil.toDegrees(direction.yaw)
                + " degrees\n",
        );
    }
    ...
    for (var i: number = 0; i < gestures.length; i++) {
        ...
        switch (gesture.type) {
            case Leap.Gesture.TYPE_CIRCLE:
                ...
                if (circle.state != Leap.Gesture.STATE_START) {
                    ...
                    if (previousGesture.isValid()) {
                        var previousUpdate: Leap.CircleGesture = <Leap.CircleGesture> controller.frame(1).gesture(
                            circle.id,
                        );
                ...
                console.log(
                    "Circle id: " + circle.id + ", " + circle.state + ", progress: " + circle.progress + ", radius: "
                        + circle.radius + ", angle: " + Leap.LeapUtil.toDegrees(sweptAngle) + ", " + clockwiseness,
                );
                break;
            case Leap.Gesture.TYPE_SWIPE:
                var swipe: Leap.SwipeGesture = <Leap.SwipeGesture> gesture;
                console.log(
                    "Swipe id: " + swipe.id + ", " + swipe.state + ", position: " + swipe.position + ", direction: "
            ...
            case Leap.Gesture.TYPE_SCREEN_TAP:
                ...
                console.log(
                    "Screen Tap id: " + screenTap.id + ", " + screenTap.state + ", position: " + screenTap.position
            ...
            case Leap.Gesture.TYPE_KEY_TAP:
                ...
                console.log(
                    "Key Tap id: " + keyTap.id + ", " + keyTap.state + ", position: " + keyTap.position
