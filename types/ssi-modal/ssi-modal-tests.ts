...
[$backdrop, $buttonsAll, $buttonsLeft, $buttonsRight, $modalElement, $content, $icons, $title, $window, $wrapper].forEach(jq => jq.addClass('touched'));
...
// dialog
ssi_modal.dialog({ title: 'Dialog', content: 'dialog content' }, (e, m) => { m.close(); });
// confirm
ssi_modal.confirm({ title: 'Confirm', content: 'confirm content' }, (e, m) => { m.close(); });
// notify 所有类型
...
ssi_modal.show({
    content: 'callbacks',
    beforeShow: (modal) => { if (!modal) return false; return true; },
    beforeClose: (modal) => { if (Math.random() > 0.5) return false; return true; },
    onShow: (modal) => { modal.changePreviewState(); },
