...
const eventPlugin: Plugin = {
    ...
    onClick: (event, editor, next) => {
        [event.nativeEvent, event.clientX];
    },
    onCompositionEnd: (event, editor, next) => {
        [event.nativeEvent, event.data];
    },
    ...
    onCopy: (event, editor, next) => {
        [event.nativeEvent, event.clipboardData];
    },
    onCut: (event, editor, next) => {
        [event.nativeEvent, event.clipboardData];
    },
    onDragEnd: (event, editor, next) => {
        [event.nativeEvent.dataTransfer, event.clientX];
    },
    onDragEnter: (event, editor, next) => {
        [event.nativeEvent.dataTransfer, event.clientX];
    },
    onDragExit: (event, editor, next) => {
        [event.nativeEvent.dataTransfer, event.clientX];
    },
    onDragLeave: (event, editor, next) => {
        [event.nativeEvent.dataTransfer, event.clientX];
    },
    ...
    onInput: (event, editor, next) => {
        [event.nativeEvent];
    },
    onKeyDown: (event, editor, next) => {
        [event.nativeEvent, event.key];
    },
...
const decorations = Decoration.createList([
    { anchor: Point.create({ key: 'a', offset: 0 }), focus: Point.create({ key: 'a', offset: 0 }) },
]);
