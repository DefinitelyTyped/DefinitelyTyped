...
function runTest() {
    StreamTest.versions.forEach((version) => {
        ...
        const errorChunkStream1 = StreamTest[version].fromErroredChunks(new Error('bad thing'), [
        ...
        const errorObjectStream1 = StreamTest[version].fromErroredObjects(new Error('bad thing'), fromObjectsTestArray);
        ...
        try {
            StreamTest[version].syncError(StreamTest[version].writable(), new Error('bad thing'), ['a chunk']);
        } catch (err) {
