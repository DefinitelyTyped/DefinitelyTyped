...
const { connectionType: ShipConnection } = connectionDefinitions({ nodeType: shipType });
const factionType = new GraphQLObjectType({
    ...
    fields: () => ({
        ships: {
            type: ShipConnection,
            args: connectionArgs,
            resolve: (faction, args) => connectionFromArray(faction.ships.map((id: any) => id), args),
        },
    }),
});
// Object Identification
...
mutationWithClientMutationId({
    ...
    description: 'D',
    inputFields: gifcm,
    mutateAndGetPayload: (object: any, context: any, info: GraphQLResolveInfo) => {
