...
declare namespace oracledb {
	...
	interface IResultSet {
         ...
         * @param {number} numRows number of rows to fetch at this time
		 * @param  {(err:any,row:Array<any>)=>void} callback Callback called when the rows is available or when some error occurs.
         */
        getRows(numRows: number, callback: (err: any, row: Array<any>) => void): void;
         ...
         * Different values of numRows may alter the time needed for fetching data from Oracle Database. The value of fetchArraySize has no effect on getRows() performance or internal buffering.
         * @param {number} numRows number of rows to fetch at this time
		 * @returns Promise when the rows is available or when some error occurs.
         */
        getRows(numRows: number): IPromise<Array<any>>;
     ...
     * @since The property was introduced in node-oracledb version 2.0. It replaces prefetchRows.
     * @example
     * ```
     * var oracledb = require('oracledb');
     ...
     * @since This property was added in node-oracledb 1.13.
     * @example
     * ```
     * var oracledb = require('oracledb');
