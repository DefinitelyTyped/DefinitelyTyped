...
export interface AddMethodCallback {
    /**
     * Accept a callback
     * @param callback
     */
...
export interface OnCallbackEvent {
    message: string;
    task: string;
    err: any;
    duration?: number;
...
export interface OnAllCallbackEvent extends OnCallbackEvent {
    src: string;
}
...
declare class Orchestrator {
     ...
     */
    on(event: string, cb: (e: OnCallbackEvent) => any): Orchestrator;

    /**
     * Listen to all orchestrator events from one callback
     * @param cb Passes single argument: e: event details
     */
    onAll(cb: (e: OnAllCallbackEvent) => any): void;
}
