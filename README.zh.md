...
#### 旧版本的 TypeScript（3.8 和更早版本）

Definitely Typed 仅在发布时间小于 2 年的 TypeScript 版本上测试软件包。当前已测试 3.9 及更高版本。如果您使用的是 TypeScript 2.0 到 3.8，仍然可以尝试安装 @types 软件包，大多数软件包都不使用 TypeScript 的新特性。但是不能保证它们会起作用，这是支持窗口：
