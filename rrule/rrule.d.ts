/*
Unlike `index.d.ts`, this uses `export = RRule` instead of exporting a property `RRule`.
This is necessary so we can `export as namespace RRule;` correctly. Globally there is no need to use `RRule.RRule`.
When imported it must be accessed through `import { RRule } from "rrule";` instead of just `import RRule = require("rrule");`.
*/
